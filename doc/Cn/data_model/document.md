...
+----------------+--------------------------------------------------------+----------------+---------------------------------------------------------------+
| 长整数         | 整数，范围 -9223372036854775808 至 9223372036854775807 | 10             | { "key" : 3000000000 } 或                                     |
|                | 如果用户指定的数值无法适用于整数，则 SequoiaDB 自动将  |                | { "key" : { "$numberLong" : "3000000000" } }                  |
|                | 其转化为浮点型                                         |                |                                                               |
+----------------+--------------------------------------------------------+----------------+---------------------------------------------------------------+
| 浮点数         | 浮点数，范围 -1.7E+308 至 1.7E+308                     | 10             | { "key" : 123.456 } 或 { "key" : 123e+50 }                    |
+----------------+--------------------------------------------------------+----------------+---------------------------------------------------------------+
| 高精度数       | 高精度数，范围 小数点前最高131072位;小数点后最高16383位| 10             | { "key" : { $decimal:"123.456" } }                            |
+----------------+--------------------------------------------------------+----------------+---------------------------------------------------------------+
...
SequoiaDB 中的日期使用 YYYY-MM-DD的形式存取，在存储时将其转换为8字节的整数。
...
<pre class="prettyprint lang-diy">
{ "$decimal" : "<数据>" }
{ "$decimal" : "<数据>", "$precision" : [<总精度>,<小数精度>] }</pre>

有两种表式方法：第一种不指定数字的精度范围，可以支持最大的精度范围，第二种可以指定数字的总精度和小数精度。
