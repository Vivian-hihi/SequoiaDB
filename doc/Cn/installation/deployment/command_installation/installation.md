...
-   运行安装程序
	...
	> ./sequoiadb-2.0-linux-x86_64-installer.run --mode text --SMS false</pre>
...
-   输入安装路径后按回车（默认安装在 /opt/sequoiadb ），此时系统提示输入用户名和用户组，该用户名用于运行 SequoiaDB 服务
	...
	用户名 [sdbadmin]:
	用户组 [sdbadmin_group]:</pre>

-   输入用户名、用户组后按回车（默认创建 sdbadmin 用户和 sdbadmin_group 用户组），此时系统提示输入该用户的密码和确认密码
