...
void CLeftView::OnVScroll( UINT nSBCode, UINT nPos, CScrollBar * pScrollBar )
{
   ...
   //GetDocument()->GetNextView( pos )->SetScrollPos( SB_VERT, 0, 1 ) ;

   CMarkdownEditorView *pView = (CMarkdownEditorView *)( GetDocument()->GetNextView( pos ) ) ;

   percent = (float)y / (float)max ;
   pView->setHtmlScroll( percent ) ;
