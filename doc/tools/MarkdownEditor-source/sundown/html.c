...
struct buf *_navOb = NULL ;
static int *_navNum = NULL ;

static int handler( void *pCtrl, const char *pSection, const char *pKey, const char *pValue )
{
   Config *pConfig = (Config*)pCtrl ;
   if( strcmp( pSection, "config" ) == 0 && strcmp( pKey, "md" ) == 0 )
   {
      pConfig->pMd = strdup( pValue ) ;
   }
   else if( strcmp( pSection, "config" ) == 0 && strcmp( pKey, "img" ) == 0 )
   {
      pConfig->pImg = strdup( pValue ) ;
   }
   else
   {
      return 0 ;
   ...
   if( text )
   {
      ...
      if( level == 2 )
      {
		  if (text->size > 0)
		  {
			  ...
			  bufput(_navOb, text->data, text->size);
   ...
   if( link && link->size )
   {
      if( link->size >= 4 &&
         ( link->data[0] != 'h' ||
           ...
           link->data[2] != 't' ||
           link->data[3] != 'p' ) &&
          link->data[link->size - 3] == '.' &&
          link->data[link->size - 2] == 'm'  &&
          link->data[link->size - 1] == 'd' )
      {
         bufput( ob, _pRootPath, strlen( _pRootPath ) );
         bufput( ob, _Config.pMd, strlen( _Config.pMd ) );
         BUFPUTSL( ob, "/" );
         escape_href( ob, link->data, link->size - 2 );
         BUFPUTSL( ob, "html" );
      }
      else
      {
...
rndr_image( struct buf *ob, const struct buf *link, const struct buf *title, const struct buf *alt, void *opaque )
{
   ...
   if( link->size > 0 )
   {
      ...
      {
         bufput( ob, _pRootPath, strlen( _pRootPath ) );
         bufput( ob, _Config.pImg, strlen( _Config.pImg ) );
         BUFPUTSL( ob, "/" );
         escape_href( ob, link->data, link->size );
      }
      else
   ...
   bufputs( ob, USE_XHTML( options ) ? "\"/></div>" : "\"></div>" );
   return 1;
}
...
void
sdhtml_renderer( struct sd_callbacks *callbacks, struct html_renderopt *options, unsigned int render_flags, const char *pRootPath, struct buf *ob, int *navNum )
{
   static const struct sd_callbacks cb_default = {
      rndr_blockcode,
      rndr_blockquote,
