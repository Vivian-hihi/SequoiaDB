...
- **KEY(\<column_list\>)**
    ...
    由于 SequoiaDB 引擎有独立的 hash 算法，`PARTITION BY LINEAR KEY` 与 `PARTITION BY KEY` 效果等同，均是使用 SequoiaDB 的算法。而例子中 `PARTITION 4` 参数是无意义的。分区实际是按 SequoiaDB 的规则自动切分到对应的数据组中。上述示例与以下语句对应。
