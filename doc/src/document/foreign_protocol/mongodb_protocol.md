...
| logout            | 是 | 是 | 是 |
  ...
  >
  > * fapmongo2 支持 MONGODB-CR 鉴权算法，fapmongo3 支持 SCRAM-SHA-1 鉴权算法，fapmongo4 支持 SCRAM-SHA-256 鉴权算法。
  > 
  > * “createUser” 命令在mongodb-3.x对应的驱动上无法使用，请改用 “db.runCommand({createUser:"myuser",pwd:"mypwd"})” 的方式去创建。
  >
  > * 删除用户：只能删除当前会话鉴权过的用户。
