...
```lang-javascript
foo=# create foreign table test (name text, id numeric) server sdb_server options ( collectionspace 'chen', collection 'test', decimal 'on' ) ;
```
...
>
>直接在 pg_shell 中修改配置，只能在当前 pg_shell 中生效，重新登录 pg_shell 需要重新设置。
