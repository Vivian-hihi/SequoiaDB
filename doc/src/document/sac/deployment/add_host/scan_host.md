...
1. 扫描主机，在 **IP地址/主机名** 输入要扫描主机的IP地址或主机名。
...
2. 用户名必须是root，安装需要root权限，如果系统禁止root登录，需要手工去开启。

3. 输入密码，SSH端口根据实际端口修改（默认22）。代理端口在这里不修改。

4. 点击 **扫描** 按钮。扫描成功后，点击底部的 **下一步** 按钮，进入添加主机页面。

   ![扫描主机](sac/deployment/add_host/scan_host_6.jpg)

5. 开始添加主机，[点击查看](sac/deployment/add_host/add_host.md)。