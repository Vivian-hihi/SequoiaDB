...
- 默认情况下，复制组中的主节点在处理完一个写请求后会立即返回，即 W =1。数据同步会在后台异步完成（[同步日志](database_management/log_synchronization.md)并达到最终一致。此时外部的读请求获得的数据可能不是最新的。在对数据一致性要求不高的场景中，这种方式可以提供最优的写入性能。

- 当我们[创建集合](reference/Sequoiadb_command/SdbCS/createCL.md)时，可以通过ReplSize 属性指定集合的 W 值。
