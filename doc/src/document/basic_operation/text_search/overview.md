...
全文检索实现的是“近实时”的搜索能力，即一个新的文档从被索引到可被搜索，会有一定的延迟，对于典型的搜索引擎来说，这个值从秒级到分钟级不等。在 SequoiaDB 的实现中，其延迟更多地取决于索引的速度。主要分两种情况：
...
##相关内容##
* [全文检索环境部属](basic_operation/text_search/deploy.md)
