...
然而，对于写事务操作，若在操作过程发生错误，[数据库配置](database_management/runtime_configuration.md)中的 transautorollback 配置项可以决定当前会话中，所有未提交的写事务是否自动回滚。transautorollback 的描述如下：
...
默认情况下，transautorollback 配置项的值为 true。所以，当写事务操作过程出现失败时，当前事务所有未提交的写操作都将被回滚。
...
* 事务提交或者回滚的范围仅仅局限于单个操作。当单个操作成功时，该操作将自动被提交；当单个操作失败时，该操作将自动被回滚。
