...
[数据库配置](database_management/runtime_configuration.md)中，如下两个配置项的设置将决定事务的提交与回滚方式：

| 配置项 | 描述 | 取值 | 默认值 |
| ------ | ------ | --- | ------ |
| transautocommit | 表示是否开启事务自动提交（操作成功）与自动回滚（操作失败）功能。 | true/false | false |
| transautorollback | 表示是否开启事务自动回滚（操作失败）功能。 | true/false | true |

**注意**：这两个配置项只有在事务功能开启（即 transactionon 为 true ）的情况下才生效。

根据 transautocommit 与 transautorollback 的设置组合，事务的提交与回滚有如下 3 种使用方式：

1. 当 transautocommit 为 false，transautorollback 为 false 时，用户需要手工控制事务的提交与回滚。其使用方式如下：
	...
	使用这种设置组合，用户必须显式调用 "transBegin" 和 "transCommit" 或者 "transRollback" 方法来控制事务的开启、提交或者回滚。

2. 当 transautocommit 为 false，transautorollback 为 true 时(默认的设置组合)，当某个事务操作失败时，当前会话所有未提交的事务将自动被回滚。

3. 当 transautocommit 为 true 时，transautorollback 的值将被忽略。在这种设置组合下，使用事务存在以下两点不同：
	...
	* 事务提交或者回滚的范围仅仅局限于单个事务操作。当单个事务操作成功时，该操作将自动被提交；当单个事务操作失败时，该操作将自动被回滚。
	...
	当 transautocommit 设置为 true 时，批插 1、批插 2、批插 3 分别为独立的事务操作。假设批插 1 和 批插 2 操作成功，而批插 3 失败。那么批插 1 和 批插 2 的记录将全部被自动提交。而批插 3 批次的所有记录将被自动回滚。
