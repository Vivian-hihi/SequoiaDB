...
| --help      | -h   | 返回基本帮助和用法文本 |
| --version   | -v   | 返回工具的编译版本 |
| --meta      | -m   | 指定在解析日志文件时，也解析日志文件的元数据信息，不可单独使用 |
| --type      | -t   | 指定只解析日志文件中指定类型的日志：<br> 0：空白填充<br> 1：数据插入<br> 2：数据更新<br> 3：数据删除<br> 4：创建集合空间<br> 5：删除集合空间<br> 6：创建集合<br> 7：删除集合<br> 8：创建索引<br> 9：删除索引<br> ... 更多类型，请查看：dpsDef.hpp 中的 DPS_LOG_TYPE 类型枚举 |
