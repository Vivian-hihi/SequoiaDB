...
适配器本身是无状态的，除使用配置文件外，不持久化存储任何数据。
...
##日志##
适配器的日志存放在 SequoiaDB 安装目录下的 conf/log/sdbseadapterlog 下，在适配器启动的时候按连接的数据节点服务端口号分别自动创建子目录进行存放，日志文件名为 sdbseadapter.log。日志文件最大100MB，写满之后将创建下一个文件，最多保留20个循环诊断日志文件。第20个文件写满后，最老的日志文件将被删除，并创建一个新的文件。