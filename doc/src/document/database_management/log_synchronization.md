...
3. 进入该节点的配置文件所在位置，重新配置参数。将logfilesz 设为70， logfilenum设为30。如果没有 logfilesz 和 logfilenum ，请添加logfilesz =70， logfilenum =30这两行。
  ...
  ```
  svcname=20000
  dbpath=/opt/trunk/20000/
  diaglevel=3
  clustername=xxx
  businessname=yyy
  role=data
  catalogaddr=localhost:30003,localhost:30013,localhost:30023
  logfilesz=70
  logfilenum=30
  ```
...
5. 使用快照查看该节点的配置参数。
  ...
  ```lang-javascript
  > var db=new Sdb("localhost",20000)
  > db.snapshot(SDB_SNAP_CONFIGS,{},{"NodeName":"","role":"","logfilesz":"","logfilenum":""})
  {
    "NodeName": "localhost:20000",
    "role": "data",
    "logfilesz": 70,
    "logfilenum": 30
  }
