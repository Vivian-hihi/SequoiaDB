...
鉴权能够大大保障数据库的安全性。从 3.4.1 版本开始，SequoiaDB 支持新的鉴权机制--SCRAM-SHA256，而在之前版本，SequoiaDB 使用是 MD5 鉴权。 

##MD5 鉴权##

###创建用户###

在创建用户之后，用户系统表会保存该用户的用户名以及密码的 MD5 值。
...
> 建议用户使用 SSL 连接创建用户。关于 SSL 连接详细可参考 [网络安全](database_management/security/network_security.md)。
...
![第一次步骤](database_management/authStep1.png)

####第二次步骤####

![第二次步骤](database_management/authStep2.png)

第一次步骤之后，客户端从服务端获取盐值和计算的迭代数，按照已经协商好的算法分别计算出 ClientKey，StoredKey 和 ServerKey 。
...
![第三次步骤](database_management/authStep3.png)

客户端利用在第一次认证中计算出来的 ServerKey 计算出服务端的身份证明，然后同服务端发送过来的身份证明作比较，如果相同则说明服务器是合法的。
