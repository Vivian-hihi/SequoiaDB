...
6. 按 y 表示使用强制安装（默认不使用）。非强制安装时发现有相关进程存在，就会报错退出。强制安装会尝试停止进程：
...
11. 询问是否安装OM服务，默认为2不安装。输入1安装om，输入3只安装om不重启集群
  ...
  >**Note:**  
  > true表示安装om，如果安装之前存在db集群则重启集群。

  > only表示只安装om，如果安装之前存在db集群则不会重启集群。
