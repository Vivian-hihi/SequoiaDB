...
> * 索引页大小为 4096 / 8192 / 16384 / 32768 / 65536 字节，分别对应最大的索引值大小为 1024 / 2048 / 4096 / 4096 / 4096 字节。例如，索引页大小 4096 字节， a 为索引字段，当插入记录含有 a 字段 { a: "fjsd..." }，并且 a 字段值大小超过 1011 个字符时（另外的13个字节用于额外的存储）时，无法插入该记录。
> * 索引字段的数量最多为32个。
