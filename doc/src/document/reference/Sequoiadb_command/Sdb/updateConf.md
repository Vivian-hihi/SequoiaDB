...
> **Note:**
>
> * 参照[数据库配置](database_management/runtime_configuration.md)页面获取配置的动态生效、重启生效和禁止修改信息。
...
* 配置数据节点 20000 上的 diaglevel 参数。
 ...
 > db.updateConf( { diaglevel:3 }, { GroupName:"db1", Svcname:"20000" } )
 ```
...
* 报错时获取详细错误信息。
 ...
 > db = new Sdb( "localhost", 11810 )
 // 进行参数配置，报错
 > db.updateConf( { transactionon:'true' }, { Svcname:"20000" } )
