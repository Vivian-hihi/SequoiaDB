...
4. MySQL Shell 连接到 myinst_01 实例

   ```lang-bash
   $ bin/mysql --socket=/opt/mysql/database/3306/mysqld.sock -uroot
   ```

5. 在实例 myinst_01 上进行建库、建表等元数据操作
...
6. MySQL Shell 连接到 myinst_02 实例

   ```lang-bash
   $ bin/mysql --socket=/opt/mysql/database/3307/mysqld.sock -uroot
   ```

7. 验证实例 myinst_01 的操作是否同步到实例 myinst_02
...
8. 新增实例 myinst_03 并加入实例组
 
   ```lang-bash
   $ bin/sdb_sql_ctl addinst myinst_03 -D database/3308 -p 3308 -g mysql -k test
   ```
 
9. MySQL Shell 连接到实例 myinst_03

   ```lang-bash
   $ bin/mysql --socket=/opt/mysql/database/3308/mysqld.sock -uroot
   ```
 
10. 查询元数据是否同步到实例 myinst_03
