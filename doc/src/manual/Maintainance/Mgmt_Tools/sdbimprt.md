...
| string        | 字符串，可以指定最小长度和最大长度，最大长度如果为 0，表示不设限制 <br>指定最大长度，如：```string(128)``` 最大长度 128 个字符，超出则截断<br>指定最小长度，如：```string(10, 0)``` 最小长度 10 个字符，少于 10 个字符则用默认值代替，如果没有默认值则用 null 代替 <br> 指定最小长度和最大长度，如：```string(10, 128)``` 最小长度 10 个字符，并且最大长度 128 个字符 |
| null          | 空值 |
