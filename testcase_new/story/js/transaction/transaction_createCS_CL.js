/* *****************************************************************************
@Description: after insert 1000 record, and the normal connection read the max record
@modify list:
...
var CONNECTNUM = 2 ;
var TRANCSNAME = CHANGEDPREFIX + "trans_1";
try
{
...
function createcs_cl__index_test( db )
{
   ...
   var cs = commCreateCS( db, TRANCSNAME, true ) ;
   var cl = commCreateCL( db, TRANCSNAME, COMMCLNAME, 0, true, false, true ) ;
   ...
   commDropCL( db, TRANCSNAME, COMMCLNAME, false, false ) ;
   try
   {
      db.transCommit() ;
   }
   ...
   commDropCS( db, TRANCSNAME, false ) ;
   try
   {
      db.transCommit() ;
   }
