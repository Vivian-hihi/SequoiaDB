/* *****************************************************************************
@Description: after insert 1000 record, sdb transactional connection read the min record ,and the normal connection read the same record
@modify list:
...
function dbReadDifferent( db )
{
   ...
   if( cl2.find( {"transTest":0} ).count() != 1 )
   {
...
{
   println( "transaction read same test failed: " + e ) ;
   throw e ;
}
