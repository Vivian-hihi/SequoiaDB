/* *****************************************************************************
@Description: step one :insert data and commit transaction . step two: insert data and rollback transaction 
@modify list:
...
function insertCommit_insertRollbackTest( db )
{
   ...
   while( i < loop )
   {
      ...
      for( j = 0 ; j < amount ; ++j )
      {
         ...
         {
            println( " failed to insert data when rollback : { transaction: " + j + " }" ) ;
            println( " loop : " + i ) ;
            throw e ;
         }
      }
...
function main( db )
{
   insertCommit_insertRollbackTest( db ) ;
}
...
{
   println( "transaction insertCommit_insertRollbackTest test failed: " + e ) ;
   throw e ;
}
