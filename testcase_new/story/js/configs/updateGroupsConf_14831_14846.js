 ...
 ******************************************************************************/
 ...
 function main()
 {
   ...
   createGroupAndNode( db, hostName, groupName1, svcName1, dbPath );
   createGroupAndNode( db, hostName, groupName2, svcName2, dbPath );
   createGroupAndNode( db, hostName, groupName3, svcName3, dbPath );
   
   println("update config");
   //test update config in more group
   svcName = Number(RSRVPORTBEGIN) + 10 * (Math.floor(Math.random()*3));
   updateGroupConf( db, hostName, groupArr, svcName );
   
   deleteGroupConf( db, hostName, groupArr, svcName );
 ...
 function createGroupAndNode( db, hostName, groupName, svcName, dbPath )
 {
   ...
   rg.createNode( hostName, svcName, dbPath + svcName );
   rg.start();
 }
