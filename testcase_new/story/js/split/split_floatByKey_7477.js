...
function main() {
   ...
   try{
      ...
      //createCL , split in a
      var varCL = varCS.createCL(COMMCLNAME,{ShardingKey:{a:1}, ShardingType:"range",
   ...
   if( 2 == arrGroupName.length ) {
      for(var i=0; i !=arrGroupName.length ;++i){
         if( PGname != arrGroupName[i][0] ){
            try{
               varCL.split( PGname,arrGroupName[i][0],{a:100+0.001} );
         ...
         if( PGname != arrGroupName[i][0] ){
            if( _PGname != arrGroupName[i][0] ){
               try{
                  ...
                  varCL.split( PGname,arrGroupName[i][0],{a:100*t+0.001} );
                  while( catadb.SYSCAT.SYSTASKS.find().count() != 0 );
   ...
   for(var i = 0 ; i != SLgroupID.length ; ++i){
      ...
      //var gdb = new Sdb(COORDHOSTNAME,60000);
      try{
      ...
      if( len != 100 ){
         println( "expect is: 100, actual is: " + len );
         throw "NotExpect";
      }
   }
   //@ clean end
