...
function main( db )
{
   try
   {
      var FILENAME = LocalPath + "/" + CHANGEDPREFIX + "_notExistCL_jira466.file",
...
/*******************************************************************************
*@【Test Point1】: export json data when the collection not exist
*******************************************************************************/
      var expCmdOption = "--hostname "+COORDHOSTNAME+" --svcname "+COORDSVCNAME+
                         " -c "+COMMCSNAME+" -l NotExistCL --file "+FILENAME+
                         " --type json" ;
      try
      {
...
*******************************************************************************/
      ...
      {
         cmdToolRun( expCmd, expCmdOption ) ;
         println( "success to export csv format data by assign not exist CL" ) ;
         throw "ErrorCsvRun" ;
      }
      catch( e )
      {
