...
function main()
{
   ...
   var clOption = {Capped:true, Size:128, AutoIndexId:false, OverWrite:false};
   var dbcl = commCreateCLByOption( db, csName, clName, clOption, true, true );
   
   var insertNum = 131068;
   var expectNum = 0;
   var min = 0;
   var max = insertNum -1;
   ...
   for(var j = 0 ; j< repeatNum; j++)
   {
      ...
      //如果direction=1，则pop记录后，无法继续插入
      //如果direction=-1，则pop记录后，可继续插入记录
      if(1 == direction)
      {
         dbcl.truncate();
         insertNum = max + 1;
