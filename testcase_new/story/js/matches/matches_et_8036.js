...
function findRecs( cl, rawData, dataType )
{
   ...
   if( dataType === "int" )
   {
      var rc = cl.find( {b:{$et: rawData[0]["int"][0]}, 
                         c:{$et: rawData[0]["int"][1]}} ).sort({a:1});
   }
   else if( dataType === "long" )
   {
      var rc = cl.find( {b:{$et:{$numberLong: rawData[1]["long"][0]}}, 
                         c:{$et:{$numberLong: rawData[1]["long"][1]}}} ).sort({a:1});
   }
   else if( dataType === "double" )
   {
      var rc = cl.find( {b:{$et: rawData[2]["double"][0]}, 
                         c:{$et: rawData[2]["double"][1]}} ).sort({a:1});
   }
