...
function main()
{
   ...
   for(var k=0; k<coordNum; k++ )
   {
      ...
      {
         doc.push({a:i});
         expR.push({a:i, id1:currentValue1 + increment1 -1 + Math.ceil(100/acquireSize1)*acquireSize1*increment1*k + increment1*i,id2:currentValue2 + increment2 -1 + Math.ceil(100/acquireSize2)*acquireSize2*increment2*k + increment2*i});
      }
      cl.insert(doc);
      coord.close();
   }
