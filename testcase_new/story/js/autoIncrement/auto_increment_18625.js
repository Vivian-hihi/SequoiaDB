...
function main()
{
   ...
   for (var i in autoIncres){
      ...
      clAutoNames.push(clAutoName);
   }
   
   for(var i in clAutoNames){
      var tmpName = clAutoNames[i];
      if(autoIncreNames.indexOf(tmpName) == -1){
         throw buildException("main", "LIST_SEQUENCES ERROR", "indexOf(tempName) == -1", autoIncreNames, clAutoNames);
      }
   }
}
