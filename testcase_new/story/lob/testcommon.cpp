...
void getUniqueName( const CHAR *modName, CHAR getName[] )
{
   ...
   pid_t pid ;
   pid = getpid() ;
   sprintf( getName, "%s_%d_%s", uniqName, (unsigned int)pid, modName ) ;
}
                   ...
                   bson_oid_t *oids, sdbCSHandle *_cs, CHAR _clName[] )
{
   ...
   *oids = oid ;
   ...
   //sdbReleaseConnection( db ) ; // never release db handle before the other handles
                                  // are released
}
...
//Modify : 2015-12-8   Ting YU   Init
INT32 lobGetRgNum( sdbConnectionHandle db, UINT32 *dataRgNum )
{ 
   sdbCursorHandle cursor = SDB_INVALID_HANDLE ;
   INT32 rc = SDB_OK ;
   *dataRgNum = 0;
   ...
   while( SDB_OK == sdbNext( cursor, &lrgObj ) )
   {
      bson_iterator_init( &it, &lrgObj ) ;
      rgName = bson_iterator_string( &it ) ;
      if( 0 != strcmp( "SYSCatalogGroup", rgName ) &&
