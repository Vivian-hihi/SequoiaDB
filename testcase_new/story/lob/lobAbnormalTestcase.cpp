                    ...
                    const UINT64 lobSize, UINT64 putNum, bson_oid_t oids[] )
{
   ...
   printf( "begin to open lob \n" ) ;
   bson_oid_gen( &oid ) ;
   rc = sdbOpenLob( cl, &oid, SDB_LOB_CREATEONLY, &lob ) ;
   ASSERT_TRUE( rc  == SDB_OK || rc == SDB_DMS_NOTEXIST || rc == SDB_DMS_CS_DELETING )<< " rc = " << rc;
   ...
   if( SDB_OK == rc )
   {
      ...
      {        
         ...
         ASSERT_TRUE( rc  == SDB_OK || rc == SDB_DMS_NOTEXIST || rc == SDB_RTN_CONTEXT_NOTEXIST ) 
            << " i = " << i << ", rc = " << rc;
      }
      printf( "begin to close lob \n" ) ;
      rc = sdbCloseLob( &lob ) ;
      ASSERT_EQ( SDB_OK, rc ) ;
   }
   ...
   printf( "over to excute splitLobWrite1() \n" ) ;
}
                   ...
                   CHAR md5[] )
{
   ...
   ASSERT_TRUE( rc  == SDB_OK || rc == SDB_DMS_NOTEXIST )<< " rc = " << rc;
   ...
   if( SDB_OK == rc )
   {
      ...
      for( i = 0 ; i < putNum ; ++i )
      {
         ...
         ASSERT_TRUE( rc  == SDB_OK || rc == SDB_DMS_NOTEXIST || rc == SDB_RTN_CONTEXT_NOTEXIST || rc == SDB_LOB_SEQUENCE_NOT_EXIST ) 
            << " i = " << i << ", rc = " << rc;
      }
      printf( "begin to close lob \n" ) ;
      rc = sdbCloseLob( &lob ) ;
   ...
   printf( "over to excute splitLobRead1() \n" ) ;   
}
...
void dropCSandCL( const CHAR *clName )
{
   ...
   // allow drop cs when open lob
   ASSERT_TRUE( SDB_OK == rc || SDB_LOCK_FAILED == rc ) << "rc is: " << rc ;
