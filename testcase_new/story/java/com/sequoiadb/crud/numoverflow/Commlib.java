...
public class Commlib {
     ...
     * @param: cl
     *         operSymbol:      $inc           
     *         updateValue: update value,rg:{a:1}
     */
    public static void updateIsStrictDataType(DBCollection cl, BSONObject updateValue ){
		try{
			BSONObject modifier = new BasicBSONObject();					
			modifier.put("$inc",updateValue);			
			cl.update(null,modifier,null, DBCollection.FLG_UPDATE_KEEP_SHARDINGKEY);
			...
			if( e.getErrorCode() != -318 ){ 
				Assert.assertTrue(false,"update is overflow should be failed,"+e.getErrorCode()+e.getMessage());
            }			
		}		
	}
    ...
    public static void checkDataType(DBCollection cl,BSONObject sValue,int matcherValue,String selectorName, 
		...
		try{
			...
			Assert.assertTrue(false,"check the data type failed,"+e.getMessage());
		}		
	}
     ...
     * @param: cl:rg:"db.cs.cl"
     *         matcherName:   matcher field name
     *         matcherValue:  matcher operator,rg:{"$abs":1,"$et":123}     *         
     */
    public static void matcherOper(DBCollection cl,String matcherName,BSONObject matcherValue, String []expRecords){
		try{			
			DBQuery query = new DBQuery();				
			BSONObject selector = new BasicBSONObject();	
		    ...
		    cursor1.close(); 
		    Assert.assertEquals(actualList1, actualList,"index and table scans query is different!"); 
		    dropIndex(cl, indexName);
		}catch(BaseException e){			
			Assert.assertTrue(false,"operator is used as matcher oper failed,"+e.getMessage());
		}		
	}
    ...
    public static String createIndex(DBCollection cl,String keyName) {
        ...
        try { 
            ...
            Assert.fail("create index fail:"+e.getErrorCode()+e.getMessage());
        }
        return indexName;
    }
    ...
    public static void dropIndex(DBCollection cl,String indexName) {
        ...
        try { 
            ...
            Assert.fail("dropIndex fail:"+e.getErrorCode()+e.getMessage());
        }
        
    }

    public static void multiFieldOperAsMatcher(DBCollection cl,String matcher, String[] expRecords, String indexKey){
		try{			
	        long skipRows   = 0;
	        long returnRows = -1;  
	        String selector = "{_id:{$include:0}}";
            DBCursor cursor =  cl.query(matcher, selector, null, null, skipRows, returnRows);
	        ...
	        try {	        	
	        	int sortBufferSize = 64;
	        	cl.createIndex(indexName, indexKey, false, false, sortBufferSize);           
	        } catch (BaseException e) {
	            Assert.fail("create index fail:"+e.getErrorCode()+e.getMessage());
	        }
	        ...
	        DBCursor cursor1 =  cl.query(matcher, selector, null, hint, skipRows, returnRows);
	        List<BSONObject> actualList1= new ArrayList<BSONObject>(); 			
	        while( cursor1.hasNext() ) {
	            BSONObject object1 = cursor1.getNext();	        
	            actualList1.add(object1);
     ...
     *         isVerifyDataType: if true validation data type as required,if false not validation data type 
     */
    public static void updateOper(DBCollection cl,int matcherValue,BSONObject updateValue,String operate){
		try{			
			BSONObject matcher = new BasicBSONObject();		
			BSONObject modifier = new BasicBSONObject();
			matcher.put("test",matcherValue);
			...
			if ( operate == "updateShardingKey"){
				cl.update(matcher,modifier,null, DBCollection.FLG_UPDATE_KEEP_SHARDINGKEY);
			}else if(operate == "update"){				
				cl.update(matcher,modifier,null);				
			}
        }catch(BaseException e){			
			Assert.assertTrue(false,"update is oper failed,"+e.getMessage());
		}		
	}
     ...
     * @param: cl:rg:"db.cs.cl"
     *         matcher:  used to match a record,rg:{a:1}
     *         updateValue:  rg:{"b":3}     *         
     */
    public static void upsertOper(DBCollection cl,BSONObject matcher,BSONObject updateValue,int setInsertValue,
    		String operate){
		try{			
			BSONObject modifier = new BasicBSONObject();
			...
			if ( operate == "upsert"){				
				...
				cl.upsert(matcher,modifier,null,setOnInsert, DBCollection.FLG_UPDATE_KEEP_SHARDINGKEY);	
			}
        }catch(BaseException e){			
			Assert.assertTrue(false,"update is oper failed,"+e.getMessage());
		}		
     ...
     *         matcherValue:  used to match a record     
     */
    public static void checkUpdateResult(DBCollection cl,int matcherValue, String[] expRecords) {
		try{
			DBQuery query = new DBQuery();			
			BSONObject selector = new BasicBSONObject();
			BSONObject sValue = new BasicBSONObject();			
			...
			Assert.assertTrue(false,"query data of update failed,"+e.getMessage());
		}		
	}
     ...
     * @param: 
     *         matcherValue:  used to match a record  
     *         updateName:   update fieldName
     *         expTypeToSdb: query expected data type by sdb,check the data Type
     *         isVerifyTypeToJava:is or not check the data type by java Client,true/false
     *         expTypeToJava: query expected data type by java Client
     */
    public static void checkUpdateDataType(DBCollection cl,int matcherValue,String updateName, 
    		String expTypeToSdb,Boolean isVerifyTypeToJava,String expTypeToJava) throws Exception{
		try{
			if(!updateName.contains("$")){
		        ...
		        if(isVerifyTypeToJava){
	    	        ...
	    	        while( cursor1.hasNext() ) {
	    	            object1 = cursor1.getNext();	    	            
	    	            typeOfJava = object1.get(updateName).getClass().toString(); 	    	            
	    	        } 	        
	    	        cursor1.close();             	
	            	Assert.assertEquals(typeOfJava, expTypeToJava,"the numtype from java is error");
	            }
     ...
     * @param: 
     *         sql: SQL,rg:"select a+1 from cs.cl where test=1"     *         
     */
    public static void sqlOper(Sequoiadb sdb,String sql,String[] expRecords){
