...
/**
* FileName: Pop12382.java
* test content:test pop interface for cappedCL
* @author liuxiaoxuan
    * @Date    2017.8.14
*/
public class Pop12382 extends SdbTestBase{
	...
	public void testPop() {
		...
		Assert.assertEquals(popValidRecords(isHasDirection), true, "pop valid records failed");
		Assert.assertEquals(popValidRecords(!isHasDirection),true, "pop valid records failed");
		Assert.assertEquals(popInvalidLogicalID(), -6,  "pop invalid records success");
		Assert.assertEquals(popHasNotLogicalID(), -6, "pop invalid records success");
		Assert.assertEquals(popInvalidDirection(), -6, "pop invalid records success");
    ...
    public boolean popValidRecords(boolean isHasDirection) {
		...
		try {
			...
			for(Iterator<BSONObject> it = insrtObjs.iterator(); it.hasNext();) {
				it.next();
				it.remove();
				break;
			}
			...
			System.out.println("actBsonSize: " + actBsonSize + " " + "eptBsonSize: " +eptBsonSize);
			...
			while(cursor.hasNext() && i < insrtObjs.size()) {
				...
				System.out.println("act.toString(): " + act.toString() + " " + "exp.toString(): " + exp.toString());
			...
			System.out.println("pop valid records error: " + e.getErrorCode() + " "  + e.getMessage());			
			return false;
		}finally {
			if(cursor != null) {
				cursor.close();
	...
	public int popInvalidLogicalID() {
		...
		try {
			...
			System.out.println("logicalId 99999 error: " + e.getErrorCode() + " "  + e.getMessage());
	...
	public int popHasNotLogicalID() {
		...
		try {
			...
			System.out.println("logicalId none error: " + e.getErrorCode() + " "  + e.getMessage());
			return e.getErrorCode();
		}
	...
	public int popInvalidDirection() {
		...
		try {
			...
			System.out.println("direction 1.1 error: " + e.getErrorCode() + " "  + e.getMessage());
			return e.getErrorCode();
		}finally {
			if(cursor != null) {
				cursor.close();
