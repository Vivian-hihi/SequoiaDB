...
public class Split10529A extends SdbTestBase {
	...
	public void alterCL() {
		...
		try {
			...
			if (!splitThread.isSuccess()) {
				...
				checkGroupData(db, 9000, "{sk:{$gte:1000,$lt:10000}}", 9000, destGroupName);
				checkGroupData(db, 1000, "{sk:{$gte:0,$lt:1000}}", 1000, srcGroupName);
			}
			if (db != null) {
				db.disconnect();
			}
	...
	private void checkGroupData(Sequoiadb sdb, int expectedCount, String macher, int expectTotalCount,
			String groupName) {
	...
	class Split extends SdbThreadBase {
		...
		public void exec() throws Exception {
			...
			try {
				...
				cl.split(srcGroupName, destGroupName,  (BSONObject) JSON.parse("{sk:1000}"), // 切分
