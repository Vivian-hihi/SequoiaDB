...
public class CrudSplitTest10481 extends SdbTestBase {
	...
	private String[] subclNames = {"maincl_10481_subcl_0","maincl_10481_subcl_1"};
	...
	public void split() {
		try {
			DBCollection subcls_1 = sdb_other.getCollectionSpace(SdbTestBase.csName).getCollection(subclNames[1]);
			subcls_1.splitAsync(replicaGroupNames.get(1), replicaGroupNames.get(2), (BSONObject)JSON.parse(" {a:150} "), (BSONObject)JSON.parse(" {a:200} "));
		} catch (BaseException e) {
			e.printStackTrace();
			Assert.assertTrue(false,"split data faild: "+e.getMessage());
		}
	}
	...
	private void createCL() {
		...
		try {
			...
			cs.createCollection(subclNames[0], (BSONObject) JSON.parse(subclOptions[0]));
			cs.createCollection(subclNames[1], (BSONObject) JSON.parse(subclOptions[1]));
