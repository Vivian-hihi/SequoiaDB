...
/*
 * 1、多个连接多线程并发如下操作： 
 *    （1）重新打开lob，指定范围锁定数据段（lockAndSeek），写入lob 
 *    （2）truncate删除lob（分别验证truncate删除范围包括写入lob范围、truncate删除范围不包含写入lob范围） 
 * 2、检查写入和truncatelob结果
 */

public class TruncateWhenWrite13393 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        try {
            ...
            // TODO: skip for bug, JIRA-3080
            CommLib comm = new CommLib();
            if (comm.isStandAlone(sdb)) {
                throw new SkipException("temperarily skip for a bug");
            }
