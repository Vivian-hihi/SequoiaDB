...
public class InsertRecords14372 extends SdbTestBase {
    ...
    public void test() {
        ...
        FullTextUtils.checkFullSyncToES(esClient, sdb, SdbTestBase.csName, this.clName, this.fullIndexName, FullTextUtils.INSERT_NUMS * 2);
    }
        ...
        try {
            for(int i = 0; i < 100; i++) {
                for(int j = 0; j < insertNums/100; j++) {
                    BSONObject record = (BSONObject) JSON.parse("{a: 'test_14372_" + i*j + "', b: '" + FullTextUtils.getRandomString(64)
                                      + "', c: '" + FullTextUtils.getRandomString(64) + "', d: '" + FullTextUtils.getRandomString(64)
                                      + "', e: '" + FullTextUtils.getRandomString(128) + "', g: '" + FullTextUtils.getRandomString(256) + "'}");
                    records.add(record);
                }
