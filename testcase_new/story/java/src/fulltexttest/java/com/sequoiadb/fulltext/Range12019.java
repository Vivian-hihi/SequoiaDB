...
/**
* FileName: Range12019.java
* test content: 插入记录并执行range切分再创建全文索引
* @author liuxiaoxuan
    * @Date    2018.11.20
*/
public class Range12019 extends SdbTestBase{
      ...
      public void setUp() {
           ...
           cl = cs.createCollection(clName, (BSONObject) JSON
                     .parse("{ShardingKey:{a:1},ShardingType:'range',Group:'" + srcGroupName + "'}"));      }
           ...
           try {
                for(int i = 0; i < 100; i++){
                    for (int j = 0; j < insertNums/100; j++) {
                         insertObjs.add((BSONObject) JSON.parse("{a: 'test_range12019_" + i*j + "', b: '" + FullTextUtils.getRandomString(32)
