...
public class SplitAndInsert12020 extends SdbTestBase {
    ...
    public void test() {
        ...
        cl.createIndex(fullTextIndexName, (BSONObject)JSON.parse("{a : 'text', b : 'text', c : 'text', d : 'text', e : 'text', f : 'text'}"), false, false);
        ...
        try {
            for(int i = 0; i < 100; i++) {
                for(int j = 0; j < FullTextUtils.INSERT_NUMS/100; j++) {
                    BSONObject record = (BSONObject) JSON.parse("{a: 'test_hash12020_" + i*j + "', b: '" + FullTextUtils.getRandomString(32)
