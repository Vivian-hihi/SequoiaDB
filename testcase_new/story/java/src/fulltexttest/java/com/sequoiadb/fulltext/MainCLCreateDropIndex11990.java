...
/**
* FileName: MainCLCreateDropIndex11990.java
* test content: 主子表中创建/删除全文索引    
* @author liuxiaoxuan
    * @Date    2018.11.27
*/
public class MainCLCreateDropIndex11990 extends SdbTestBase{
      ...
      public void setUp() {
           ...
           DBCollection subcl = cs.createCollection(subCLName2, (BSONObject)JSON
                     .parse("{ShardingKey:{a0:1}, ShardingType:'hash', Group:'" + srcGroupName + "'}"));
      ...
      public void test() {
           ...
           // insert one group
           boolean isSuccess = insertDataOneGroup(maincl, FullTextUtils.INSERT_NUMS);
           if(!isSuccess) {
           ...
           // insert more groups
           isSuccess = insertDataMoreGroups(maincl, FullTextUtils.INSERT_NUMS);
           if(!isSuccess) {
                throw new SkipException("---insert has an err:SEQUOIADBMAINSTREAM-3827---");
           }
      ...
      public boolean insertDataOneGroup(DBCollection cl, int insertNums) {
      ...
      public boolean insertDataMoreGroups(DBCollection cl, int insertNums) {
           List<BSONObject> insertObjs = new ArrayList<>();
           try {
                for(int i = 0; i < 100; i++){
