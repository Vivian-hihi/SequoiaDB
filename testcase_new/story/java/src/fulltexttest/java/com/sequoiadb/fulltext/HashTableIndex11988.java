...
public class HashTableIndex11988 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        this.cl = cs.createCollection(clName, (BSONObject)JSON.parse("{ShardingKey:{a:1},ShardingType:'hash',AutoSplit:true}"));
    ...
    public void insertData(int insertNums) {
        ...
        for(int i = 0; i < 100; i++) {
            for(int j = 0; j < insertNums/100; j++) {
                BSONObject record = (BSONObject) JSON.parse("{a: 'test_hash11988_" + i*j + "', b: '" + FullTextUtils.getRandomString(32)
                                      + "', c: '" + FullTextUtils.getRandomString(64) + "', d: '" + FullTextUtils.getRandomString(64)
                                      + "', e: '" + FullTextUtils.getRandomString(128) + "', g: '" + FullTextUtils.getRandomString(128) + "'}");
                records.add(record);
            }
