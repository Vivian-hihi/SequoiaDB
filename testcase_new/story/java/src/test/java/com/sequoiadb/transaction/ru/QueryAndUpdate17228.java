 ...
 */
@Test(groups = "ru")
public class QueryAndUpdate17228 extends SdbTestBase {
     ...
     public void test() {
          ...
          // 事务1读记录走索引扫描
          recordsCursor = cl1.query("{a:{$exists:1}}", null, null, "{'':'textIndex17228'}");
          explainCursor = cl1.explain((BSONObject) JSON.parse("{a:{$exists:1}}"), null, null,
                    (BSONObject) JSON.parse("{'':'textIndex17228'}"), 0, 1, 0, null);
          while (explainCursor.hasNext()) {
               String scanType = (String) explainCursor.getNext().get("ScanType");
          ...
          // 事务2读记录走索引扫描
          recordsCursor = cl2.query("{a:{$exists:1}}", null, null, "{'':'textIndex17228'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 事务3读记录走索引扫描
          recordsCursor = cl3.query("{a:{$exists:1}}", null, null, "{'':'textIndex17228'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 非事务索引扫描
          recordsCursor = cl.query("{a:{$exists:1}}", null, null, "{'':'textIndex17228'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
