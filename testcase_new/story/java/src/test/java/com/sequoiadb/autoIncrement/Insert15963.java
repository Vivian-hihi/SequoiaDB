...
public class Insert15963 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        DBCollection cl = commSdb.getCollectionSpace(csName).createCollection(
        		clName,(BSONObject)JSON.parse("{AutoIncrement:{Field:'id', Increment:"+befIncrement+"}}"));
	}
	...
	public void test() {
		...
		if(!(insertThread.isSuccess() && alterThread.isSuccess())){
            Assert.fail(insertThread.getErrorMsg() + alterThread.getErrorMsg());
        }
    ...
    private class AlterThread extends SdbThreadBase {
		...
		public void exec() throws BaseException{
			try(Sequoiadb sdb = new Sequoiadb(coordUrl, "", "");){
				DBCollection cl = sdb.getCollectionSpace(csName).getCollection(clName);
				cl.setAttributes((BSONObject)JSON.parse("{AutoIncrement:{Field:'id',Increment:"+afIncrement+"}}"));
			}
		}
	}
