...
public class RenameCS_16812 extends SdbTestBase{
	...
	public void setUp(){
		...
		for (int i = 0; i < 5; i++) {
			...
			sdb.analyze(new BasicBSONObject("CollectionSpace", oldCSName + "_" + i));
		}
	}
	
	@Test(dataProvider = "operData")
	public void test(int analyzeMode, String testCSName, String testNewCSName){ 
		RenameCSThread reCSNameThread = new RenameCSThread(testCSName, testNewCSName);
		...
		try(Sequoiadb dataDB = sdb.getReplicaGroup(groupName).getMaster().connect()){
			...
			Assert.assertEquals(num1, 0, "check cs " + testCSName +" be rename, analyze shuold cleanUp");
			BSONObject filter2 = new BasicBSONObject();
			filter2.put("CollectionSpace", testNewCSName);
			long num2 = stat.getCount(filter2);
			Assert.assertEquals(num2, 1, "check new name cs " + testNewCSName +" , analyze shuold create");
		}
	}
