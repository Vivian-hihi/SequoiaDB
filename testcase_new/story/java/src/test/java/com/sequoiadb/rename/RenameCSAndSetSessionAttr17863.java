 ...
 * @content set seesionAttr is "{PreferedInstance:'s'}",priority slave node query,the slave node has 
 *          no synchronous rename CS  *          
...
public class RenameCSAndSetSessionAttr17863 extends SdbTestBase {
    ...
    public void test() {
        ...
        Assert.assertNotEquals(accessNodeBeforeRename, masterNodeName,"accessNode is " + accessNodeBeforeRename 
                + "  masterNode is " + masterNodeName);
        Assert.assertEquals(accessNodeAfterRename, masterNodeName,"accessNode is " + accessNodeAfterRename 
        		                    + "  masterNode is " + masterNodeName);        
    }
    ...
    private String getAccessNode(Sequoiadb sdb, String csName){
    	DBCollection dbcl = sdb.getCollectionSpace(csName).getCollection(clName);
 		DBCursor cursor = dbcl.explain(null, null, null, null, 0, -1, DBQuery.FLG_QUERY_FORCE_HINT, null);		
 		String nodeName = "";
