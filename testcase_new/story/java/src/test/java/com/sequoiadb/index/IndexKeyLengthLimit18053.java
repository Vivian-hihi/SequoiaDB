...
/**
 * FileName: IndexKeyLengthLimit18053.java test content:create index, 
 *           specify multiple index fields,the index fields value exceeds the limit. 
 * testlink case:seqDB-18053 * 
 * @author wuyan
 * @Date 2019.3.28
 * @version 1.00
 */
...
public class IndexKeyLengthLimit18053 extends SdbTestBase {
	...
	public void testIndexInAnyPageSize1(int pageSize, int length1, int length2) {
		try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "");) {
			...
			createIndexFail(cl);
			Assert.assertFalse(cl.isIndexExist("testindex"), "check the index not exist!");
	...
	private void insertData(DBCollection dbcl, int length1, int length2, boolean isSuccessflag) {
		// the keyValue length exceeds maximum:maxlength + 1
		...
		try {
			...
			if (e.getErrorCode() != -39) {
				Assert.fail("insert fail: e=" + e.getErrorCode() + e.getMessage());
			}
		}
	}
	...
	private void createIndexFail(DBCollection dbcl) {
		try {
			dbcl.createIndex("testindex", "{'testa':1,'str':-1}", true, false);
			Assert.fail("createIndex should fail!");
		} catch (BaseException e) {
			// error 39: index key is too large
			if (e.getErrorCode() != -39) {
				Assert.fail("createindex fail: e=" + e.getErrorCode() + e.getMessage());
			}
		}
	}
}
