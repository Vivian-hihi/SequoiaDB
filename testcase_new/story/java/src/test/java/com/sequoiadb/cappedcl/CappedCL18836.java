...
public class CappedCL18836 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        for (int i = 0; i < csNum; i++) {
            ...
            sdb.createCollectionSpace(cappedCSNamei, (BSONObject) JSON.parse("{Capped:true}"));
        }
    ...
    public void test() throws Exception {
        for (int i = 0; i < csNum; i++) {
            for (int j = 0; j < clNum; j++) {
                te.addWorker(new CreateCLThread(cappedCSName + "_" + i, cappedCLName + "_" + j));
            }
        }
        te.run();
        ...
        // 检查集合空间及集合创建成功，记录正确
        for (int i = 0; i < csNum; i++) {
            Assert.assertTrue(sdb.isCollectionSpaceExist(cappedCSName + "_" + i),
                    cappedCSName + "_" + i + " is not exists");
            for (int j = 0; j < clNum; j++) {
                Assert.assertTrue(
                        sdb.getCollectionSpace(cappedCSName + "_" + i).isCollectionExist(cappedCLName + "_" + j),
                        cappedCLName + "_" + j + " is not exists in " + cappedCSName + "_" + i);

                DBCollection cl = sdb.getCollectionSpace(cappedCSName + "_" + i).getCollection(cappedCLName + "_" + j);
                ...
                // 校验主节点id字段
                Assert.assertTrue(CappedCLUtils.checkLogicalID(sdb, cappedCSName + "_" + i, cappedCLName + "_" + j,
                        stringLength));
