...
public class RenameCSAndSplit16139 extends SdbTestBase {
    ...
    public Object[][] generateDatas() {
        return new Object[][] { new Object[] { "renameCS16139", "renameCSNew16139" },
    ...
    public void test(String testCSName, String testNewCSName) {
        ...
        String clOptions = "{ShardingKey:{no:1},ShardingType:'range'," + "ReplSize:0,Compressed:true}, Group:"
                + srcGroupName + "}";
        ...
        RenameCSThread renameCSThread = new RenameCSThread(testCSName, testNewCSName);
        splitCLThread.start();
        ...
        if (renameCSThread.isSuccess()) {
            ...
            if (e.getErrorCode() != -147 && e.getErrorCode() != -172 && e.getErrorCode() != -23
                    && e.getErrorCode() != -190) {
                Assert.fail("split fail:" + splitCLThread.getErrorMsg() + "  e:" + e.getErrorCode());
            }

            int clNums = 1;
            RenameUtil.checkRenameCSResult(sdb, testCSName, testNewCSName, clNums);
    ...
    public class SplitCLThread extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            ...
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                DBCollection dbcl = db.getCollectionSpace(localCSName).getCollection(clName);
                int present = 80;
    ...
    private void checkSplitResult(String testCSName, Sequoiadb sdb) {
        ...
        checkGroupData(testCSName, sdb, srcGroupCount, srcGroupMather, srcGroupName);

        String targetGroupMather = "{no:{$gte:2000,$lt:10000}}";
        long targetGroupCount = 8000;
        checkGroupData(testCSName, sdb, targetGroupCount, targetGroupMather, targetGroupName);
    ...
    private void checkGroupData(String testCSName, Sequoiadb sdb, long expectedCount, String macher, String groupName) {
        try (Sequoiadb dataNode = sdb.getReplicaGroup(groupName).getMaster().connect()) {
            DBCollection dbcl = dataNode.getCollectionSpace(testCSName).getCollection(clName);
