...
public class Fulltext15843B extends FullTestBase {
    ...
    protected void caseFini() throws Exception {
        // 检查固定集合及ES端全文索引无残留
        Assert.assertTrue(FullTextUtils.isIndexDeleted(sdb, cappedName, esIndexName));
    }
        ...
        public void insert() {
            ...
            try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                ...
                for (int i = 0; i < insertRecordNum; i++) {
                    insertObjs.add((BSONObject) JSON.parse("{ a: '" + strA + "', b: 'new_insert_15843B_" + i + "'}"));
                }
        ...
        public void update() {
            ...
            try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                ...
                BSONObject modifier = new BasicBSONObject("$set",
                        new BasicBSONObject("a", "fulltext15843B_after_update"));
                BSONObject matcher = new BasicBSONObject("id", new BasicBSONObject("$lt", 500));
