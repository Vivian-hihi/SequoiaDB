 ...
 * @version 1.00
 */
public class RewriteLobAndSplit13270_19002 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        createMainCLAndAttachCL(cs, SdbTestBase.csName, mainCLName, subCLName);
    ...
    public void testLob(String clName, String splitCLName) {
        try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
            String sourceRGName = RandomWriteLobUtil.getSrcGroupName(sdb, SdbTestBase.csName, splitCLName);
            String targetRGName = RandomWriteLobUtil.getSplitGroupName(sdb, sourceRGName);

            DBCollection dbcl = sdb.getCollectionSpace(SdbTestBase.csName).getCollection(clName);
            ObjectId lobOid = RandomWriteLobUtil.createAndWriteLob(dbcl, testLobBuff);
            ...
            for (int i = 0; i < 20; i++) {
                rewriteLobTasks.add(new LockAndRewriteLobTask(clName, lobOid, offset, rewriteBuff));
            ...
            SplitTask splitTask = new SplitTask(sdb, splitCLName, sourceRGName, targetRGName);
            splitTask.start();
    ...
    public class SplitTask extends SdbThreadBase {
        ...
        public SplitTask(Sequoiadb sdb, String clName, String srcRGName, String targetRGName) {
            this.clName = clName;
    ...
    private class LockAndRewriteLobTask extends SdbThreadBase {
        ...
        public LockAndRewriteLobTask(String clName, ObjectId lobOid, int offset, byte[] rewriteLobBuff) {
            this.clName = clName;
            this.lobOid = lobOid;
            this.offset = offset;
    ...
    private void readLobAndcheckWriteResult(DBCollection cl, ObjectId lobOid, byte[] rewriteBuff) {

        // check the rewrite lob
        int expOffset = 1024 * 1024;
        for (int i = 0; i < 20; i++) {
            byte[] actBuff = RandomWriteLobUtil.seekAndReadLob(cl, lobOid, rewriteBuff.length, expOffset);
    ...
    private void createMainCLAndAttachCL(CollectionSpace cs, String csName, String mainCLName, String subCLName) {
     ...
     */
    private List<CataInfoItem> buildExpectResult(String sourceRGName, String targetRGName) {
    ...
    private void checkSplitResult(Sequoiadb sdb, String clName, String sourceRGName, String targetRGName) {
        String cond = String.format("{Name:\"%s.%s\"}", SdbTestBase.csName, clName);
        DBCursor collections = sdb.getSnapshot(8, cond, null, null);
        List<CataInfoItem> cataInfo = buildExpectResult(sourceRGName, targetRGName);
        while (collections.hasNext()) {
            ...
            for (int i = 0; i < cataInfo.size(); ++i) {
                ...
                Assert.assertTrue(compareResult, cataInfo.get(i).toString() + "actResult:" + "groupName:" + groupName
    ...
    private class CataInfoItem {
        ...
        public String toString() {
            return "groupName : " + groupName + " lowBound: {'':" + lowBound + "}" + " upBound:{'':" + upBound + "}";
        }
    }
}
