...
public class Insert15962 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        DBCollection cl = commSdb.getCollectionSpace(csName).createCollection(clName);
	}
	...
	public void test() {
		...
		if(!(insertThread.isSuccess() && createThread.isSuccess())){
            Assert.fail(insertThread.getErrorMsg() + createThread.getErrorMsg());
        }
    ...
    private class CreateThread extends SdbThreadBase {
		...
		public void exec() throws BaseException{
			try(Sequoiadb sdb = new Sequoiadb(coordUrl, "", "");){
				DBCollection cl = sdb.getCollectionSpace(csName).getCollection(clName);
				cl.createAutoIncrement((BSONObject) JSON.parse("{Field:'" + fieldName + "', AcquireSize : 1}"));
			}
		}
	}
