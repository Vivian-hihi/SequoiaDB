...
/**
* FileName: TestAlterCS14994.java
* test content:Concurrent alter cs ,alter the same field
* testlink case:seqDB-14994
* @author wuyan
* @Date    2016.9.12
* @version 1.00
*/
public class TestAlterCS14994 extends SdbTestBase {
	...
	public void setUp(){
		...
		BSONObject options1 = (BSONObject)JSON.parse("{Groups:[ '"+ dataGroupNames.get(1) +"','" +  dataGroupNames.get(0) + "']}");
		BSONObject options2 = (BSONObject)JSON.parse("{Groups:[ '"+ dataGroupNames.get(1) +"','" +  dataGroupNames.get(2) + "']}");
	...
	private class AlterUseEnable extends SdbThreadBase{
		public void exec() throws BaseException {					
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	...
		    	BSONObject options = (BSONObject)JSON.parse("{Domain:'" + domainName1 + "'}");
	...
	private class AlterUseSet extends SdbThreadBase{
		public void exec() throws BaseException {				
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	CollectionSpace dbcs = sdb.getCollectionSpace(csName);
		    	BSONObject options = (BSONObject)JSON.parse("{Domain:'" + domainName2 + "'}");
	...
	private class AlterSameValue extends SdbThreadBase{
		public void exec() throws BaseException {					
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	CollectionSpace dbcs = sdb.getCollectionSpace(csName);
		    	BSONObject options = (BSONObject)JSON.parse("{PageSize:16384}");
		    	dbcs.setAttributes(options);
		    }
		}
	}
	...
	private class AlterDiffValue extends SdbThreadBase{
		public void exec() throws BaseException {					
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	CollectionSpace dbcs = sdb.getCollectionSpace(csName);
		    	BSONObject options = (BSONObject)JSON.parse("{LobPageSize:16384}");
		    	dbcs.setAttributes(options);
		    }
		}
	}	
	...
	private void checkAlterResult( ){
		...
		try{
	        ...
	        DBCollection dbcl = cataDB.getCollectionSpace("SYSCAT").getCollection("SYSCOLLECTIONSPACES");
			...
			Assert.assertTrue(expDomainsList.contains(actDomain),"domain for one of the modified values!");
		}finally{
			cataDB.close();
		}		
	}
