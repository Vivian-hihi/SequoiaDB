...
public class RenameMainCLAndDetachCL16086 extends SdbTestBase {
    ...
    public void test() {
        ...
        if (renameCLThread.isSuccess()) {
            Assert.assertTrue(!detachCLThread.isSuccess(), detachCLThread.getErrorMsg());
            BaseException e = (BaseException) (detachCLThread.getExceptions().get(0));
            if (e.getErrorCode() != -147 && e.getErrorCode() != -23 && e.getErrorCode() != -190) {
                Assert.fail("detach fail:" + detachCLThread.getErrorMsg() + "  e:" + e.getErrorCode());
            }
            ...
            if (e.getErrorCode() != -33 && e.getErrorCode() != -334) {
                Assert.fail("renameCS fail:" + renameCLThread.getErrorMsg() + "  e:" + e.getErrorCode());
            }
    ...
    public class DetachCLThread extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            ...
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                DBCollection dbcl = db.getCollectionSpace(SdbTestBase.csName).getCollection(mainCLName);
                dbcl.detachCollection(SdbTestBase.csName + subCLName);
            }
        }
    }

    private DBCollection createCLAndAttachCL(String mainCLName, String subCLName) {
        // create maincl and subcl
        CollectionSpace cs = sdb.getCollectionSpace(SdbTestBase.csName);
        String mainCLOptions = "{ShardingKey:{no:1},ShardingType:'range',IsMainCL:true,"
                + "ReplSize:0,Compressed:true}}";
        DBCollection maincl = RenameUtil.createCL(cs, mainCLName, mainCLOptions);
    ...
    private void checkDetachFailResult(String subCLName) {
        try (DBCursor cursor = sdb.getSnapshot(8, "{'Name':'" + SdbTestBase.csName + "." + subCLName + "'}", "", "")) {
            ...
            while (cursor.hasNext()) {
                ...
                if (objInfo.get("MainCLName") == null) {
                    ...
                    String getMainclName = (String) objInfo.get("MainCLName");
                    String expMainclName = SdbTestBase.csName + "." + newMainCLName;
                    if (!getMainclName.equals(expMainclName)) {
                        Assert.fail("the mainclName is error:" + getMainclName);
                    }
                }
            }
        }
