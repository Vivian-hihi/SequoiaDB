 ...
 */
@Test(groups = "ru")
public class QueryAndUpdateIxScan17238 extends SdbTestBase {
     private Sequoiadb sdb = null;
     private String clName = "cl17238ix";
     ...
     public void test() {
          ...
          recordsCursor = cl1.query("{a:{$exists:1}}", null, null, "{'':'textIndex17238'}",
                    DBQuery.FLG_QUERY_FOR_UPDATE);
          explainCursor = cl1.explain((BSONObject) JSON.parse("{a:{$exists:1}}"), null, null,
                    (BSONObject) JSON.parse("{'':'textIndex17238'}"), 0, 1, DBQuery.FLG_QUERY_FOR_UPDATE, null);
          while (explainCursor.hasNext()) {
               String scanType = (String) explainCursor.getNext().get("ScanType");
       ...
       // 事务2 select for update读记录走索引扫描阻塞
          CL2Query cl2Thread = new CL2Query("{a:{$exists:1}}", "{'':'textIndex17238'}");
          cl2Thread.start();
