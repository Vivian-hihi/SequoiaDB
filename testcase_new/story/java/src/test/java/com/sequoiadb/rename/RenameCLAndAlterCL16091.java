...
public class RenameCLAndAlterCL16091 extends SdbTestBase {
    ...
    public void test() {
        ...
        if (renameCLThread.isSuccess()) {
            ...
            if (alterCLThread.isSuccess()) {
                ...
                Assert.assertTrue(!alterCLThread.isSuccess(), alterCLThread.getErrorMsg());
                BaseException e = (BaseException) (alterCLThread.getExceptions().get(0));
                if (e.getErrorCode() != -147 && e.getErrorCode() != -23 && e.getErrorCode() != -190) {
                    Assert.fail("detach fail:" + alterCLThread.getErrorMsg() + "  e:" + e.getErrorCode());
                }
    ...
    private void checkAlterCLResult(String clName, Boolean alterSuccess) {
        try (DBCursor cursor = sdb.getSnapshot(8, "{'Name':'" + SdbTestBase.csName + "." + clName + "'}", "", "")) {
            while (cursor.hasNext()) {
                ...
                if (alterSuccess) {
                    ...
                    Assert.assertTrue(Arrays.asList(expResult).contains(compressionType),
