...
public class RenameCS_16284 extends SdbTestBase{
	...
	public void test(){ 
		...
		if(!putThread.isSuccess()){
			...
			BaseException errorA = (BaseException)putThread.getExceptions().get(0);
		...
		if(!readThread.isSuccess()){
			...
			BaseException errorB = (BaseException)readThread.getExceptions().get(0);
		...
		if(!deleteThread.isSuccess()){
			...
			BaseException errorC = (BaseException)deleteThread.getExceptions().get(0);
		...
		if(!listThread.isSuccess()){
			...
			BaseException errorD = (BaseException)listThread.getExceptions().get(0);
	...
	private List<ObjectId> putLob(DBCollection cl, byte[] data, int lobNum){
