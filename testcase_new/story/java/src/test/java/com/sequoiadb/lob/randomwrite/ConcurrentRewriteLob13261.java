...
/**
* FileName: ConcurrentRewriteLob13261.java
* test content:write lobdata to the locked data segment and write lobdata to the non locked data segment,
* testlink case:seqDB-13261
* @author wuyan
    * @Date    2017.11.8
* @version 1.00
*/
public class ConcurrentRewriteLob13261 extends SdbTestBase {	
	...
	public void testLob(){	
		...
		if(lockAndRewriteLob.isSuccess()){
			if(!seekAndRewriteLob.isSuccess()){
				...
				Assert.assertEquals(-320,e.getErrorCode(),"seekAndRewriteLob must fail:"+seekAndRewriteLob.getErrorMsg());
				readLobAndcheckWriteResult( cl, oid, testLobBuff, lockAndRewriteBuff,offset2);
			}else{
				//can't determine the status of the server, and maybe all operations are sucessful,
			...
			Assert.assertEquals(-320,e.getErrorCode(),"LockAndRewriteLob must fail:"+lockAndRewriteLob.getErrorMsg());
	...
	private void readLobAndcheckWriteResult( DBCollection cl, ObjectId oid, byte[] lobBuff, byte[] rewriteBuff,int offset) {		
