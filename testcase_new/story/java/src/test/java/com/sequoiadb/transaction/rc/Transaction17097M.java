 ...
 */
@Test(groups = "rc")
public class Transaction17097M extends SdbTestBase {
    private Sequoiadb sdb = null;
    private String clName = "cl_17097m";
    ...
    public void setUp() {
        ...
        sdb.getCollectionSpace(csName).createCollection("sub117097");
        sdb.getCollectionSpace(csName).createCollection("sub217097",
                (BSONObject) JSON.parse("{ShardingKey:{b:1}, ShardingType:'hash', AutoSplit:true}"));
        cl.attachCollection(csName + ".sub117097",
                (BSONObject) JSON.parse("{LowBound:{b:{'$minKey':1}}, UpBound:{b:500}}"));
        cl.attachCollection(csName + ".sub217097",
    ...
    public void test(String indexKey) {
        try {
            ...
            // 7 非事务记录读
            expList.clear();
            ArrayList<BSONObject> updateR2s = TransUtils.getIncDatas(startId, stopId, updateValue2);
    ...
    private class UpdateThread extends SdbThreadBase {
        ...
        public void exec() throws BaseException {
            hint = "{\"\":\"a\"}";
            cl2.update("{a: {$gte: " + startId + ", $lt: " + stopId + "}}", "{$inc:{a:" + updateValue2 + "}}", hint);
        }
    }
