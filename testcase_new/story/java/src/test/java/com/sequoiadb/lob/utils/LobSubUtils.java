...
public class LobSubUtils {
    ...
    /**
     * 创建主表并挂载一个子表，默认分区键格式为YYYYMMDD，分区范围默认1-11日
     * 
     * @param db
     * @param csName 集合空间名
     * @param mainCLName 主表名
     * @param subCLName 子表名
     * @return mainCL 返回主表
     */
    public static DBCollection createMainCLAndAttachCL(Sequoiadb db, String csName, String mainCLName,
	    String subCLName) {
	...
	subNames.add(subCLName);
	return createMainCLAndAttachCL(db, csName, mainCLName, subNames, "YYYYMMDD", sf.format(date), 10);
    }
     ...
     * @param csName 集合空间名
     * @param mainCLName 主表名
     * @param subCLNames 子表名列表
     * @param shardingFormat 分区键格式、YYYYMMDD/YYYYMM/YYYY
     * @param beginBound 分区范围开启起始边界值
     * @param scope 分区范围
     * @return mainCL 返回主表
     */
    public static DBCollection createMainCLAndAttachCL(Sequoiadb db, String csName, String mainCLName,
	    List<String> subCLNames, String shardingFormat, String beginBound, int scope) {
	...
	for (int i = 0; i < subCLNames.size(); i++) {
	    ...
	    bound.put("LowBound", new BasicBSONObject("date", Integer.valueOf(beginBound) + i * scope + ""));
	    bound.put("UpBound", new BasicBSONObject("date", Integer.valueOf(beginBound) + (i + 1) * scope + ""));
     ...
     */
    public static List<ObjectId> createAndWriteLob(DBCollection cl, byte[] data) {
	Calendar cal = Calendar.getInstance();
	cal.set(Calendar.DATE, 1);
	Date date = cal.getTime();
	SimpleDateFormat sf = new SimpleDateFormat("yyyyMMdd");
	return createAndWriteLob(cl, data, "YYYYMMDD", 10, 1, sf.format(date));
    }
     ...
     * @param cl 集合
     * @param data 数据
     * @param format 主表分区键日期格式
     * @param lobNum 创建的lob数
     * @param timeInterval lob间隔时间
     * @param beginDate 构造lob的时间起始值
     * @return List<ObjectId> lobIds
     */
    public static List<ObjectId> createAndWriteLob(DBCollection cl, byte[] data, String format, int lobNum,
	    int timeInterval, String beginDate) {
     ...
     * @return List<ObjectId> lobIds
     */
    public static List<ObjectId> getLobIDs(DBCollection cl, BSONObject matcher) {
	List<ObjectId> idList = new ArrayList<ObjectId>();
	DBCursor cursor = cl.listLobs(matcher, null, new BasicBSONObject("Oid", 1), null, 0, -1);
     ...
     * @return booelan
     */
    public static boolean checkLobMD5(DBCollection cl, List<ObjectId> lobIds, byte[] expData) {
	...
	for (ObjectId lobId : lobIds) {
		...
		throw new BaseException(0, "check lob: " + lobId + " md5 error, exp: " + expMD5 + ", act: " + actMD5);
	    }
	}
	return true;
    }
