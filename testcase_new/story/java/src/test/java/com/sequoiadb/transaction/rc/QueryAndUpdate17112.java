 ...
 */
@Test(groups = "rc")
public class QueryAndUpdate17112 extends SdbTestBase {
     ...
     public void setUp() {
          ...
          cl.createIndex("textIndex17112", "{a:1}", false, false);
     ...
     public void test() {
          ...
          recordsCursor = cl1.query("{a:{$exists:1}}", null, null, "{'':'textIndex17112'}",
                    DBQuery.FLG_QUERY_FOR_UPDATE);
          explainCursor = cl1.explain((BSONObject) JSON.parse("{a:{$exists:1}}"), null, null,
                    (BSONObject) JSON.parse("{'':'textIndex17112'}"), 0, 1, DBQuery.FLG_QUERY_FOR_UPDATE, null);
          while (explainCursor.hasNext()) {
               String scanType = (String) explainCursor.getNext().get("ScanType");
          ...
          // 事务2索引扫描
          recordsCursor = cl2.query("{a:{$exists:1}}", null, null, "{'':'textIndex17112'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 非事务索引扫描
          recordsCursor = cl.query("{a:{$exists:1}}", null, null, "{'':'textIndex17112'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 事务3索引扫描
          recordsCursor = cl3.query("{a:{$exists:1}}", null, null, "{'':'textIndex17112'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
     ...
     private class CL3Update extends SdbThreadBase {
          ...
          public void exec() throws Exception {
               cl3.update("{a:1}", "{$set:{a:4}}", "{'':'textIndex17112'}");
          }
     }
}
