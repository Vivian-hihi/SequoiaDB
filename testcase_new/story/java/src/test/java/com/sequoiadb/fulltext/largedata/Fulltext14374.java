...
/**
 * FileName: Fulltext14374.java test content: ES中无提交记录时，插入/修改/删除/查询集合中的记录
 * 
 * @author liuxiaoxuan
 * @Date 2018.11.21
 */
public class Fulltext14374 extends FullTestBase {
    ...
    public void test() throws Exception {
        ...
        Assert.assertTrue(FullTextUtils.isIndexCreated(cl, textIndexName, (int) cl.getCount()));
    }
    ...
    private class UpdateThread extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                DBCollection cl = null;
                cl = db.getCollectionSpace(csName).getCollection(clName);
                BSONObject modifier = new BasicBSONObject("$set", new BasicBSONObject("g", "-1"));
                BSONObject matcher = new BasicBSONObject("g", new BasicBSONObject("$lt", 100000));
    ...
    private class RemoveThread extends SdbThreadBase {
        ...
        public void exec() throws Exception {
            try (Sequoiadb db = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                DBCollection cl = null;
                cl = db.getCollectionSpace(csName).getCollection(clName);
                BSONObject matcher = new BasicBSONObject("g", new BasicBSONObject("$gt", 100000));
                cl.delete(matcher);
            }
        }
    }
