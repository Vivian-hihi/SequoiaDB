...
/**
 * @FileName seqDB-13318: 并发读lob过程中执行切分
 * @Author linsuqiang
 * @Date 2017-11-08
 * @Version 1.00
 */
...
/*
 * 1、多线程并发如下操作: (1)打开已存在lob对象，seek指定偏移范围，并发读取lob数据， 其中读取lob覆盖元数据组和目标组数据 （2）执行切分split
 * （需要覆盖读取过程中元数据片迁移到目标组） 2、读取lob，检查操作结果
 */

public class ReadAndSplit13318 extends SdbTestBase {
    ...
    public void testLob() {
        ...
        // init
        SplitThread splitThrd = new SplitThread();
    ...
    private class SplitThread extends SdbThreadBase {
        ...
        public SplitThread() {
            ...
            srcGroupName = RandomWriteLobUtil.getSrcGroupName(db, csName, clName);
