...
public class Transaction18644 extends SdbTestBase {
    ...
    public void test() {
        ...
        // 事务2内置SQL查询
        DBCursor cursor = db2.exec("select * from " + csName + "." + hashCLName);
        List<BSONObject> actList = TransUtils.getReadActList(cursor);
        TransUtils.sortList(actList);
        TransUtils.sortList(expList);
        Assert.assertEquals(actList, expList);
        cursor = db2.exec("select count(a) as aCount from " + csName + "." + hashCLName);
        ...
        // 事务2内置SQL查询
        List<BSONObject> expRecords = TransUtils.getIncDatas(2000, 12000, 10);
        cursor = db2.exec("select * from " + csName + "." + hashCLName);
        actList = TransUtils.getReadActList(cursor);
        TransUtils.sortList(actList);
