...
public class LobOprUtils {
	...
	return createAndWriteLob(db.getCollectionSpace(csName).getCollection(clName), id, data);
    }
    ...
    public static void checkSplitResult(Sequoiadb sdb, String csName, String clName, ArrayList<String> splitGroupNames,
	...
	for (int i = 0; i < splitGroupNames.size(); i++) {
	    String nodeName = sdb.getReplicaGroup(splitGroupNames.get(i)).getMaster().getNodeName();
		...
		if (actErrorValue > expErrorValue) {
		    Assert.assertTrue(false,
			    "errorValue: " + actErrorValue + " subCont:" + subCount + "  explistnum:" + count);
		}
	    }
	}
