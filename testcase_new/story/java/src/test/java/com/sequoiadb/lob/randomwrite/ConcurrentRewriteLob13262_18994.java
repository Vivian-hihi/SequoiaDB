...
/**
 * @Description seqDB-13262:并发lock/lockAndSeek加锁写入lob ;seqDB-18994: 主子表并发lock/lockAndSeek加锁写入lob
 * @author wuyan
 * @Date 2019.08.27
 * @UpdateDate 2019.08.28
 * @version 1.10
 */
public class ConcurrentRewriteLob13262_18994 extends SdbTestBase {
    ...
    public void testLob(String clName) throws Exception {
        ...
        byte[] lockAndRewriteBuff = RandomWriteLobUtil.getRandomBytes(rewriteLobSize1);
        byte[] lockAndSeekRewriteBuff = RandomWriteLobUtil.getRandomBytes(rewriteLobSize2);

        ThreadExecutor thread = new ThreadExecutor();
        thread.addWorker(new LockAndReWriteLobThread(clName, lobOid, offset1, lockAndRewriteBuff));
        thread.addWorker(new LockAndSeekReWriteLobThread(clName, lobOid, offset2, lockAndSeekRewriteBuff));
        thread.run();
        ...
        try (DBLob lob = cl.openLob(lobOid)) {
            ...
            lob.read(actualBuff);
            byte[] expBuff1 = RandomWriteLobUtil.appendBuff(lobBuff, lockAndRewriteBuff, offset1);
            byte[] expBuff2 = RandomWriteLobUtil.appendBuff(expBuff1, lockAndSeekRewriteBuff, offset2);
    ...
    private class LockAndReWriteLobThread {
        ...
        public LockAndReWriteLobThread(String clName, ObjectId lobOid, int offset, byte[] rewriteLobBuff) {
            this.clName = clName;
            this.lobOid = lobOid;
            this.offset = offset;
        ...
        private void readLobAndCheckResult() {
            try (Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")) {
                DBCollection cl = sdb.getCollectionSpace(SdbTestBase.csName).getCollection(clName);
                byte[] actBuff = RandomWriteLobUtil.seekAndReadLob(cl, lobOid, rewriteLobBuff.length, offset);
                RandomWriteLobUtil.assertByteArrayEqual(actBuff, rewriteLobBuff);
            }
        }
    }
    ...
    private class LockAndSeekReWriteLobThread {
        ...
        public LockAndSeekReWriteLobThread(String clName, ObjectId lobOid, int offset, byte[] rewriteLobBuff) {
            this.clName = clName;
            this.lobOid = lobOid;
            this.offset = offset;
            this.rewriteLobBuff = rewriteLobBuff;
