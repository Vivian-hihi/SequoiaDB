 ...
 */
@Test(groups = "rc")
public class QueryAndUpdate17113 extends SdbTestBase {
     ...
     public void setUp() {
          ...
          cl.createIndex("textIndex17113", "{a:1}", false, false);
     ...
     public void test() {
          ...
          // 事务1索引扫描
          recordsCursor = cl1.query("{a:{$exists:1}}", null, null, "{'':'textIndex17113'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);

          // 事务2 select for update读记录走表扫描
          recordsCursor = cl2.query(null, null, null, "{'':null}", DBQuery.FLG_QUERY_FOR_UPDATE);
          DBCursor explainCursor = cl2.explain(null, null, null, (BSONObject) JSON.parse("{'':null}"), 0, 1,
                    DBQuery.FLG_QUERY_FOR_UPDATE, null);
          ...
          recordsCursor = cl2.query("{a:{$exists:1}}", null, null, "{'':'textIndex17113'}",
                    DBQuery.FLG_QUERY_FOR_UPDATE);
          explainCursor = cl2.explain((BSONObject) JSON.parse("{a:{$exists:1}}"), null, null,
                    (BSONObject) JSON.parse("{'':'textIndex17113'}"), 0, 1, DBQuery.FLG_QUERY_FOR_UPDATE, null);
          while (explainCursor.hasNext()) {
               String scanType = (String) explainCursor.getNext().get("ScanType");
          ...
          // 非事务索引扫描
          recordsCursor = cl.query("{a:{$exists:1}}", null, null, "{'':'textIndex17113'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 事务3索引扫描
          recordsCursor = cl3.query("{a:{$exists:1}}", null, null, "{'':'textIndex17113'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
     ...
     private class CL3Update extends SdbThreadBase {
          ...
          public void exec() throws Exception {
               cl3.update("{a:1}", "{$set:{a:4}}", "{'':'textIndex17113'}");
          }
     }
}
