...
/**
* FileName: TestAlterCS14995.java
* test content:Concurrent alter cs  different attribute,eg: pageSize/domain/LobPageSize
* testlink case:seqDB-14995
* @author wuyan
* @Date    2018.4.22
* @version 1.00
*/
public class TestAlterCS14995 extends SdbTestBase {
	...
	private class AlterPageSize extends SdbThreadBase{
		public void exec() throws BaseException {				
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	CollectionSpace dbcs = sdb.getCollectionSpace(csName);
		    	BSONObject options = (BSONObject)JSON.parse("{PageSize:"+ curPageSize + "}");
		    	dbcs.setAttributes(options);
		    }
		}
	}	
	...
	private class AlterLobPageSize extends SdbThreadBase{
		public void exec() throws BaseException {					
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	CollectionSpace dbcs = sdb.getCollectionSpace(csName);
		    	BSONObject options = (BSONObject)JSON.parse("{LobPageSize:"+ curLobPageSize + "}");
		    	dbcs.setAttributes(options);
		    }
		}
	}	
