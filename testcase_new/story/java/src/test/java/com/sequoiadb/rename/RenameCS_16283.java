...
public class RenameCS_16283 extends SdbTestBase{
	...
	public void test(){ 
		...
		if( renameCSThread.isSuccess() ){
			if( !insertDatasThread.isSuccess()){				
				BaseException e =(BaseException)(insertDatasThread.getExceptions().get(0));				
				if ( e.getErrorCode() != -23 && e.getErrorCode() != -34 ){
					Assert.fail("inseertDatas fail:"+insertDatasThread.getErrorMsg());			
				}
				checkInsertResult( newCSName, clName, insertSuccessNums);
			}
			if( !updateDatasThread.isSuccess()){
				BaseException e =(BaseException)(updateDatasThread.getExceptions().get(0));				
				if ( e.getErrorCode() != -23 && e.getErrorCode() != -34 ){
					Assert.fail("updateDatas fail:"+updateDatasThread.getErrorMsg());			
				}				
			...
			Assert.assertTrue(insertDatasThread.isSuccess(),insertDatasThread.getErrorMsg() );
			Assert.assertTrue(updateDatasThread.isSuccess(),updateDatasThread.getErrorMsg() );
	...
	private void checkInsertResult( String csName, String clName, long recordNums){					
...
//		}		 
	...
	private void checkUpdateResult( String csName, String clName, long recordNums){					
		DBCollection dbcl = sdb.getCollectionSpace(csName).getCollection(clName);
