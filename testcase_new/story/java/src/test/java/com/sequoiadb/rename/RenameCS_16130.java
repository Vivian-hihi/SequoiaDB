...
public class RenameCS_16130 extends SdbTestBase{
	...
	public void setUp(){
		...
		sdb.createDomain(domainName, new BasicBSONObject("Groups", rgNames));
		cs = sdb.createCollectionSpace(csName, new BasicBSONObject("Domain", domainName));
			...
			if(removeThread.isSuccess()){
				...
				while(cur.hasNext()){
					...
					if(csn.equals(csName)||csn.equals(newCSName)){
						Assert.fail("cs it's been remove, It shouldn't exist");
					}
				}
			}else{
				Domain domain = db.getDomain(domainName);
				...
				while(cur.hasNext()){
					...
					if(!csn.equals(newCSName)){
						Assert.fail("cs remove faild, rename success exp: "+ newCSName+" act: "+csn);
					}
				}
			}
		}
