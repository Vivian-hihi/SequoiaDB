...
public class Transaction17761D extends SdbTestBase {
    ...
    public void test(){
        ...
        recordCur = cl.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': null}");
        actDataList = TransUtils.getReadActList(recordCur);
        Assert.assertEquals(actDataList, expDataList);
        actDataList.clear();

        recordCur = cl.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': 'a'}");
        actDataList = TransUtils.getReadActList(recordCur);
        Assert.assertEquals(actDataList, expDataList);
        actDataList.clear();

        // 7 trans2 read
        recordCur = cl2.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': null}");
        actDataList = TransUtils.getReadActList(recordCur);
        Assert.assertEquals(actDataList, expDataList);
        actDataList.clear();

        recordCur = cl2.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': 'a'}");
        actDataList = TransUtils.getReadActList(recordCur);
        Assert.assertEquals(actDataList, expDataList);
        actDataList.clear();
        ...
        // 9 trans3 read
        recordCur = cl3.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': null}");
        actDataList = TransUtils.getReadActList(recordCur);
        Assert.assertEquals(actDataList, expDataList);
        actDataList.clear();

        recordCur = cl3.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': 'a'}");
        actDataList = TransUtils.getReadActList(recordCur);
        Assert.assertEquals(actDataList, expDataList);
        actDataList.clear();
    ...
    private class QueryThread extends SdbThreadBase {
        ...
        public void exec() throws BaseException {
            ...
            DBCursor cur = cl3.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': null}");
...
//            Assert.assertEquals(actQueryList, expQueryList);
            ...
            cur = cl3.query("{'a': {'$isnull': 0}}", null, "{'_id': 1}", "{'': 'a'}");
