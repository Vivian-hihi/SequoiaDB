 ...
 */
@Test(groups = "rc")
public class Transaction17103M extends SdbTestBase {
    private Sequoiadb sdb = null;
    private String clName = "cl_17103m";
    ...
    public void setUp() {
        ...
        sdb.getCollectionSpace(csName).createCollection("sub117103");
        sdb.getCollectionSpace(csName).createCollection("sub217103",
                (BSONObject) JSON.parse("{ShardingKey:{b:1}, ShardingType:'hash', AutoSplit:true}"));
        cl.attachCollection(csName + ".sub117103",
                (BSONObject) JSON.parse("{LowBound:{b:{'$minKey':1}}, UpBound:{b:500}}"));
        cl.attachCollection(csName + ".sub217103",
    ...
    public void test(String indexKey) {
        try {
            ...
            // 9 事务3记录读
            expList.addAll(updateR1s);
            cursor = cl3.query(null, null, "{a:1}", "{\"\":null}");
            actList = TransUtils.getReadActList(cursor);
            Assert.assertEquals(actList, expList);
            actList.clear();

            // 事务3索引读
            cursor = cl3.query(null, null, "{a:1}", "{\"\":\"a\"}");
            actList = TransUtils.getReadActList(cursor);
            Assert.assertEquals(actList, expList);
            actList.clear();
    ...
    private class DeleteThread extends SdbThreadBase {
        ...
        public void exec() throws BaseException {
            hint = "{\"\":\"a\"}";
            cl2.delete("{a: {$gte: " + (startId + updateValue) + ", $lt: " + (stopId + updateValue) + "}}", hint);
        }
    }
