...
/**
 * @FileName:seqDB-15958：同一个coord不指定自增字段并发插入
 * 预置条件:集合已存在，且已存在自增字段，自增字段上创建唯一索引
...
public class Insert15958 extends SdbTestBase{
    ...
    public void setUp() {
        ...
        scl = scs.createCollection(clName, (BSONObject)JSON.parse("{AutoIncrement:{Field:\"id\"}}"));
    ...
    public void test() {
		...
		AutoIncrementUtils.checkResult(scl, threadInsertNum*threadNum, currentValue);
	}
