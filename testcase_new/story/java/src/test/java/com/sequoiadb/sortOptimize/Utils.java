...
public class Utils {
    ...
    public static String getSortType(DBCollection cl, BSONObject matcher, BSONObject selector, BSONObject orderBy) {
        String sortType = "";
        DBCursor explainCursor = null;

        try {
            explainCursor = cl.explain(matcher, selector, orderBy, null, 0, -1, 0, (BSONObject)JSON.parse("{Expand:true}"));
            while(explainCursor.hasNext()) {
                BSONObject planPathObj = (BSONObject) explainCursor.getNext().get("PlanPath");
                BasicBSONList bsonLists = (BasicBSONList) planPathObj.get("ChildOperators");
                for(int i = 0; i < bsonLists.size(); i++) {
                    BSONObject obj = (BasicBSONObject) bsonLists.get(i);
                    BSONObject subobj1 = (BasicBSONObject) obj.get("PlanPath");
                    BSONObject subobj2 = (BasicBSONObject) subobj1.get("Estimate");
