...
/**
* FileName: TestAlterCS14996
* test content:Concurrent alter cs  and create cl
...
public class TestAlterCS14996 extends SdbTestBase {
	...
	public void testAlterCl(){	
		...
		if(altercs.isSuccess()){
			if(!createcl.isSuccess()){
				...
				BaseException e=(BaseException)(createcl.getExceptions().get(0));				
				if ( e.getErrorCode() != -6 && e.getErrorCode() != -32  ){
					Assert.fail("create fail:"+createcl.getErrorMsg()+ "  e:"+e.getErrorCode());
				}
			...
			BaseException e=(BaseException)(altercs.getExceptions().get(0));				
			if ( e.getErrorCode() != -32 && e.getErrorCode() != -275  ){
				Assert.fail("altercs fail:"+altercs.getErrorMsg()+ "  e:"+e.getErrorCode());
			}
			Assert.assertTrue(createcl.isSuccess(),createcl.getErrorMsg());
			checkCreateCLResult(true);
	...
	private class AlterCS extends SdbThreadBase{
		...
		public void exec() throws BaseException {					
		    try(Sequoiadb sdb = new Sequoiadb(SdbTestBase.coordUrl, "", "")){		    	
		    	...
		    	BSONObject options = (BSONObject)JSON.parse("{PageSize:"+ pageSize + "}");		    	
	...
	private void checkAlterCSResult(int expPageSize){
		...
		try(Sequoiadb dbca = nodeConnect.connect()){		    	
			...
			DBCollection dbcl = dbca.getCollectionSpace("SYSCAT").getCollection("SYSCOLLECTIONSPACES");
	...
	private void checkCreateCLResult(boolean isSuccessCreateCL){
		...
		if ( isSuccessCreateCL ){
			for(int i = 0; i < 10 ;i++ ){    		 		
	    		Assert.assertEquals(cs.isCollectionExist(clName + "." + i), true, clName + "." + i + " is not exist!");    		
	    	}
		}else{
			int count = 0;
			for(int i = 0; i < 10 ;i++ ){ 				
