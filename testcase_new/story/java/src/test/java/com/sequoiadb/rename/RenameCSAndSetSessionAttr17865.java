 ...
 * @content set seesionAttr is "{PreferedInstance:'s'}",priority slave node query,
 *          do the following:count/getIndex/listIndex/getLob/listLobs,the slave node has 
 *          no synchronous rename CS,the operator access the master node. *                
...
public class RenameCSAndSetSessionAttr17865 extends SdbTestBase {
    ...
    public void setUp() {
        ...
        if ( RenameUtil.getNodeNum(sdb, groupName) < 2 ) {
            throw new SkipException( "only one node in the group skip testcase" ) ;
        }        
    ...
    public void testInsertAndRename() {
        ...
        //the slave node has no synchronous rename CS, query of new csname must be access master node
    ...
    public void testQuery() {
    	...
    	ListIndexesThread listIndexes = new ListIndexesThread(listIndexSessionSdb);  
        ...
        Assert.assertTrue(getLobThread.isSuccess(),getLobThread.getErrorMsg());
        Assert.assertTrue(listLobThread.isSuccess(),listLobThread.getErrorMsg());    	
    }
    ...
    public class InsertThread extends SdbThreadBase {
        ...
        public void exec() throws BaseException {
            try ( Sequoiadb db = new Sequoiadb( SdbTestBase.coordUrl, "", "" ) ) {
                DBCollection dbcl = db.getCollectionSpace( oldCSName ).getCollection( clName ) ;
               RenameUtil.insertDatas( dbcl, endNo - beginNo, beginNo ) ;                
            }
        }
    }   
    ...
    public class CountThread extends SdbThreadBase {
        ...
        public void exec() throws BaseException {        	
            try {
            	DBCollection dbcl = db.getCollectionSpace( newCSName ).getCollection( clName ) ;
        ...
        public void exec() throws BaseException {        	
            try {            	
        		...
        		while ( listCursor.hasNext() ) {
        			...
        			BSONObject indexInfo = (BSONObject) object.get("IndexDef");
        		...
        		Assert.assertEquals(indexes, expIndexes, "only 3 indexes exist in the current cl!");
            }finally{
            	db.close();
            }
        }
    ...
    public class GetIndexThread extends SdbThreadBase {
        ...
        public void exec() throws BaseException {        	
            try {
            	DBCollection dbcl = db.getCollectionSpace( newCSName ).getCollection( clName ) ;
            	String name = "testa";
            	BSONObject indexInfo = (BSONObject) dbcl.getIndexInfo(name).get("IndexDef");
            	String indexName = (String) indexInfo.get("name");
