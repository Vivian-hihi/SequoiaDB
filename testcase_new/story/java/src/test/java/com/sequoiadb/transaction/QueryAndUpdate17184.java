...
public class QueryAndUpdate17184 extends SdbTestBase {
     private Sequoiadb sdb = null;
     private String clName = "cl17184tb";
     ...
     public void test() {
          ...
          // 事务1 select 读记录走索引扫描
          recordsCursor = cl1.query("{a:{$exists:1}}", null, null, "{'':'textIndex17184'}");
          explainCursor = cl1.explain((BSONObject) JSON.parse("{a:{$exists:1}}"), null, null,
                    (BSONObject) JSON.parse("{'':'textIndex17184'}"), 0, 1, 0, null);
          while (explainCursor.hasNext()) {
               String scanType = (String) explainCursor.getNext().get("ScanType");
          ...
          // 非事务索引扫描
          recordsCursor = cl.query("{a:{$exists:1}}", null, null, "{'':'textIndex17184'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 事务3读走索引扫描
          recordsCursor = cl3.query("{a:{$exists:1}}", null, null, "{'':'textIndex17184'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
     ...
     private class QueryThread extends SdbThreadBase {
          ...
          public QueryThread(String matcher, String hint) {
               super();
               this.matcher = matcher;
               this.hint = hint;
          }
          ...
          public void exec() throws Exception {
               ...
               DBCursor cursor = db.getCollectionSpace(csName).getCollection(clName).query(matcher, null, null, hint);
               List<BSONObject> records = TransUtils.getReadActList(cursor);
               try {
                    setExecResult(records);
               } catch (Exception e) {
