 ...
 */
@Test(groups = "ru")
public class QueryAndUpdate17239 extends SdbTestBase {
     ...
     public void test() {
          ...
          recordsCursor = cl1.query("{a:{$exists:1}}", null, null, "{'':'textIndex17239'}",
                    DBQuery.FLG_QUERY_FOR_UPDATE);
          explainCursor = cl1.explain((BSONObject) JSON.parse("{a:{$exists:1}}"), null, null,
                    (BSONObject) JSON.parse("{'':'textIndex17239'}"), 0, 1, DBQuery.FLG_QUERY_FOR_UPDATE, null);
          while (explainCursor.hasNext()) {
               String scanType = (String) explainCursor.getNext().get("ScanType");
          ...
          // 事务2 select读记录走索引扫描
          recordsCursor = cl2.query("{a:{$exists:1}}", null, null, "{'':'textIndex17239'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
          ...
          // 非事务索引扫描
          recordsCursor = cl.query("{a:{$exists:1}}", null, null, "{'':'textIndex17239'}");
          actList = TransUtils.getReadActList(recordsCursor);
          Assert.assertEquals(actList, expList);
