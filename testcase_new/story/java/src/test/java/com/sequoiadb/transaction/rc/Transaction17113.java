...
public class Transaction17113 extends SdbTestBase {
    ...
    public void test(String indexKey) {
        try {
            ...
            Collections.reverse(expList);
            DBCursor recordsCursor = cl2.query(null, null, null, "{'':null}", DBQuery.FLG_QUERY_FOR_UPDATE);
            ...
            Collections.reverse(expList);
            recordsCursor = cl2.query(null, null, "{a: -1}", "{'':null}", DBQuery.FLG_QUERY_FOR_UPDATE);
            actList = TransUtils.getReadActList(recordsCursor);
            Assert.assertEquals(actList, expList);
