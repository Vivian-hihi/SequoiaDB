...
public class TestRenameCS16134 extends SdbTestBase {
    ...
    public void test16134() {
        ...
        if (renameCS1Thread.isSuccess() && !renameCS2Thread.isSuccess()) {
            ...
            Assert.fail("renameCS1Thread and renameCS2Thread all failed :" + renameCS1Thread.getErrorMsg()
                    + renameCS2Thread.getErrorMsg());
        } else {
            Assert.fail("renameCS1Thread and renameCS2Thread all success :" + renameCS1Thread.getErrorMsg()
                    + renameCS2Thread.getErrorMsg());
        }
