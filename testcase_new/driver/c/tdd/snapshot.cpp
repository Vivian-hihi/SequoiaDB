...
TEST(sdb,sdbGetSnapshot_SDB_SANP_CONTEXTS)
{
   ...
   {
      rc = sdbConnect ( ARGS->hostName(), SERVER1, ARGS->user(), ARGS->passwd(), &cdb ) ;
      ASSERT_EQ( SDB_OK, rc ) << "host is: " << ARGS->hostName() << " , server is: "
                              << SERVER1 << " , user is: " << ARGS->user()
                              << " , passwd is: " << ARGS->passwd() ;
      rc = sdbConnect ( ARGS->hostName(), SERVER2, ARGS->user(), ARGS->passwd(), &ddb ) ;
      ASSERT_EQ( SDB_OK, rc ) ;
