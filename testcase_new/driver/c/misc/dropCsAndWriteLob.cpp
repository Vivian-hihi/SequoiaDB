...
INT32 createSplitCollection(sdbConnectionHandle* db,sdbCSHandle* cs,sdbCollectionHandle* cl,const char* CsName,const char* ClName)
{
	int rc = SDB_OK ; 
	// connect to sdb
	getConf() ;
	...
	if(rc == SDB_DMS_CS_EXIST)
	{
		...
		CHECK_RC(rc,"fail to drop exist collection space") ;
		rc = sdbCreateCollectionSpace(*db,CsName,SDB_PAGESIZE_4K,cs) ;
	}
	CHECK_RC(rc,"fail to create collection space") ;
	...
	bson_destroy(&option) ;
	CHECK_RC(rc,"fail to create collection") ;
	return rc ;
}
