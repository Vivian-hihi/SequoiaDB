...
class LobTest13463 extends PHPUnit_Framework_TestCase
{
   ...
   public function test_lockAndSeekOffsetVerify()
   {
      echo "\n---Begin to test lockAndSeek parameter[ offset ].\n";
      $len = -1; 
      ...
      echo "   Begin to lockAndSeek lob[ offset=-1 ].\n"; 
      $offset = -1;
      $err = $lobObj -> lockAndSeek( $offset, $len );
      $this -> assertEquals( -6, $err['errno'] );
      /*
      echo "   Begin to lockAndSeek lob[ offset='test' ].\n"; 
      $offset = "test";
      $err = $lobObj -> lockAndSeek( $offset, $len );
      $this -> assertEquals( -6, $err['errno'] );*/  //same: jira:3349
   }
   
   public function test_lockAndSeekLenVerify()
   {
      echo "\n---Begin to test lockAndSeek parameter[ len ].\n";
      $offset = 1; 
      ...
      echo "   Begin to lockAndSeek lob[ len=-2 ].\n"; 
      $len = -2;
      $err = $lobObj -> lockAndSeek( $offset, $len );
      $this -> assertEquals( -6, $err['errno'] );
      
      echo "   Begin to lockAndSeek lob[ len=0 ].\n"; 
      $len = 0;
      $err = $lobObj -> lockAndSeek( $offset, $len );
      $this -> assertEquals( -6, $err['errno'] );
      /*
      echo "   Begin to lockAndSeek lob[ len='test' ].\n"; 
      $len = "test";
      $err = $lobObj -> lockAndSeek( $offset, $len );
      $this -> assertEquals( -6, $err['errno'] );*/  //same: jira:3349
   }
