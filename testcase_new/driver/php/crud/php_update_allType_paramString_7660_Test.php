/****************************************************
@description:      update, cover all data type
...
@input:        1 test_createCL
               ...
               4 update, with condition cover all data type, $rule/$condition: string
                 $hint:null  ---test $hint when test index
               5 count, $condition: string
                 $hint:null  ---test $hint when test index
               6 dropCL
@output:     success
@modify list:
...
class DataOperator15 extends BaseOperator 
{  
   ...
   function updateRecs( $clDB, $recsArray )
   {
      $rule = json_encode( array( '$set' => array( 'b' => 'hello' ) ) );
      $hint = null;  //test $hint when test index
      $clDB -> update( $rule, null, $hint );
      for( $i = 0; $i < count( $recsArray ); $i++ ) 
      {
         ...
         $clDB -> update( $rule, $condition, $hint );
      }
   }
   
   function countRecs( $clDB )
   {
      $condition = json_encode( array( 'b' => 'hello' ) );
      $hint      = null;  //test $hint when test index
...
class TestData15 extends PHPUnit_Framework_TestCase
{
   ...
   public static function setUpBeforeClass()
   {
      ...
      self::$dbh = new DataOperator15();
      
      echo "\n---Begin to ready parameter.\n";
      self::$csName = self::$dbh -> COMMCSNAME;
      self::$clName = self::$dbh -> COMMCLNAME . '_7660_02';
   ...
   function test_update()
   {
      ...
      self::$dbh -> updateRecs( self::$clDB, self::$rawData );
   ...
   function test_count()
   {
      echo "\n   Begin to count records for update.\n";
   ...
   public static function tearDownAfterClass()
   {
      ...
      echo "\n---Test 7660_02 spend time: " . ( self::$endTime - self::$beginTime ) . " seconds.\n";
   }
