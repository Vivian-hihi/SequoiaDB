...
class Data12466(unittest.TestCase):
   ...
   def upsert_test(self,cl_list__expect,return_list_expect,upsert,**kwargs):
   ...
   def test(self):
      ...
      #condition+upsert
      condition={"a":{"$et":99}}
      self.upsert_test(upserted_list,upserted_list,upsert,condition=condition)

      #hint+upsert
      hint={"":"index"}
      l=[{"a":i+1,"b":i} for i in range(10)]
      self.upsert_test(l,l,upsert,hint=hint)

      #flags+upsert
      QUERY_FLG_FORCE_HINT=128
      try:  
         self.upsert_test(l,l,upsert,flagss=QUERY_FLG_FORCE_HINT,hint=hint)
      ...
      l.append({"a":"aaa"})
      self.upsert_test(l,l,upsert,condition=condition,setOnInsert=setOnInsert)
