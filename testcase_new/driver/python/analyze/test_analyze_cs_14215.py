# @decription: analyze with cs and mode 2
# @testlink:   seqDB-14215
...
class TestAnalyzeCS14215(testlib.SdbTestBase):
   def setUp(self):
      # skip standalone
      ...
      self.cs_name1 = 'testanalyzecs14215_1'
      self.cs_name2 = 'testanalyzecs14215_2'
      testlib.drop_cs(self.db, self.cs_name1, ignore_not_exist=True)
      testlib.drop_cs(self.db, self.cs_name2, ignore_not_exist=True)
      # create CSs
      self.cs1 = self.db.create_collection_space(self.cs_name1)
      self.cs2 = self.db.create_collection_space(self.cs_name2)
      
   def test_analyze_cs_14215(self):
      # get groups
      ...
      # create CLs
      cl_name11 = 'cll14215_11'
      cl_name12 = 'cll14215_12'
      cl_name21 = 'cll14215_21'
      cl_name22 = 'cll14215_22'
      
      self.cl1 = self.cs1.create_collection(cl_name11, {'ReplSize': 0})
      options = {'Group': src_groupname, 'ShardingKey' : {'a' : 1}, 'ShardingType': 'hash', 'ReplSize': 0}
      self.hashcl = self.cs1.create_collection(cl_name12, options)
      
      self.cl2 = self.cs2.create_collection(cl_name21, {'ReplSize': 0})
      options = {'Group': src_groupname, 'ShardingKey' : {'a' : 1}, 'ShardingType': 'range', 'ReplSize': 0}
      self.rangecl = self.cs2.create_collection(cl_name22, options)
   
      # insert datas
      insert_nums = 10000
      same_value = 0
      self.insert_diff_datas(self.cl1, insert_nums)
      self.insert_same_datas(self.cl1, insert_nums, same_value)
      self.insert_diff_datas(self.hashcl, insert_nums)
      self.insert_same_datas(self.hashcl, insert_nums, same_value)
      self.insert_diff_datas(self.cl2, insert_nums)
      self.insert_same_datas(self.cl2, insert_nums, same_value)
      self.insert_diff_datas(self.rangecl, insert_nums)
      self.insert_same_datas(self.rangecl, insert_nums, same_value)
      
      # create indexs
      self.cl1.create_index({'a':1}, 'a', False, False)
      self.hashcl.create_index({'a0':1}, 'a0', False, False)
      self.cl2.create_index({'a':1}, 'a', False, False)
      ...
      cursor1 = self.cl1.explain(condition = {'a' : 0}, options = {'Run' : True})
      ...
      cursor2 = self.cl2.explain(condition = {'a' : 0}, options = {'Run' : True})
      ...
      exp_explain2 = [{'ScanType': 'ixscan', 'IndexName': 'a', 'ReturnNum': insert_nums}]
      ...
      # analyze cs1
      self.db.analyze(options = {'CollectionSpace': self.cs_name1})
      
      # check explain after analyze cs1
      cursor1 = self.cl1.explain(condition = {'a' : 0}, options = {'Run' : True})
      cursor_hash1 = self.hashcl.explain(condition = {'a' : 0}, options = {'Run' : True})
      cursor_hash2 = self.hashcl.explain(condition = {'a0' : 0}, options = {'Run' : True})
      cursor2 = self.cl2.explain(condition = {'a' : 0}, options = {'Run' : True})
      ...
      print('------check explain success after analyze cs1------')
      
      # analyze cs2
      self.db.analyze(options = {'CollectionSpace': self.cs_name2, 'Mode': 2})
      
      # check explain after analyze cs2
      cursor1 = self.cl1.explain(condition = {'a' : 0}, options = {'Run' : True})
      cursor_hash1 = self.hashcl.explain(condition = {'a' : 0}, options = {'Run' : True})
      cursor_hash2 = self.hashcl.explain(condition = {'a0' : 0}, options = {'Run' : True})
      cursor2 = self.cl2.explain(condition = {'a' : 0}, options = {'Run' : True})
      ...
      print('------check explain success after analyze cs2------')      
