...
public class Index7172 extends SdbTestBase{
	...
	public void test(){
		...
		BSONObject autoIndexId = (BSONObject) JSON.parse("{AutoIndexId:false}");
		try{
		...
		try{
			...
			if(e.getErrorCode() != -279){
	 			Assert.assertTrue(false,"update data, errMsg:" + e.getMessage());
	 		}
		}
		
		//create id index
		try{
			BSONObject idIndexOption = (BSONObject) JSON.parse("{SortBufferSize:64}");
