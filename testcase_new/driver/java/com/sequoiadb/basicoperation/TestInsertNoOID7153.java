...
/**
* FileName: TestInsertNoOID7153.java
* test interface:
	...
	*/
	public void bulkInsert(){
		try{
			...
			Assert.assertEquals(true,list.toString().contains("_id"),"the _id is not exist");
			
			//check the interface:isOIDEnsured(),the return is true
			Assert.assertEquals(true,cl.isOIDEnsured(),"the isOIDEnsured is error"+cl.isOIDEnsured());
	        ...
	        Assert.assertEquals(idValue,actRecs.get("_id"),"actIdValue: "+actRecs.get("_id"));
	...
	*/
	public void bulkInsertNoId(){
		try{
			...
			//check if there is a _id, if _id not exist then error
			Assert.assertEquals(false,list.toString().contains("_id"),"the _id is exist");
			
			//check the interface:isOIDEnsured(),the return is true
			Assert.assertEquals(false,cl.isOIDEnsured(),"the isOIDEnsured is error"+cl.isOIDEnsured());
			...
			Assert.assertTrue(false,"bulkInsertNoId fail "+e.getErrorType());
		}		
	}
