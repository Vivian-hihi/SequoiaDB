...
/**
* FileName: TestSplit7113.java
* test interface:
* splitAsync(String sourceGroupName, String destGroupName,BSONObject splitCondition,BSONObject splitEndCondition)
* testlink cases:seqDB-7113
* testing strategy:    1.create rangecl
* 					   2.insert records
*                      3.split by range,rg:{no:1},{no:5}
...
public class TestSplit7113 extends SdbTestBase{
	...
	public void splitCL(){		
		try{
			...
			Assert.assertTrue(taskID != -1,"retrun incorrect taskID:"+taskID);
	...
	public void checkSplitResult(){			
		...
		Assert.assertEquals((long)listRecsNums.get(0),6,"srcGroup count:"+(long)listRecsNums.get(0));
		Assert.assertEquals((long)listRecsNums.get(1),4,"tarGroup count:"+(long)listRecsNums.get(1));		
	}
