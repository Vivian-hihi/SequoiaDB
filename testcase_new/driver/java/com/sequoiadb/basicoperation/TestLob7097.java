...
/**
* FileName: TestLob7097.java
* test interface:
*  createLob (ObjectId id); getCreateTime ()
*  getID()/removeLob(ObjectId lobID)/close ()--the same as testcase:testReadAndRemoveLobs7843
* getSize()/openLob (ObjectId id);--the same as testcase:testSeekLob7839(seqDB-7839)
* @author wuyan
    * @Date    2016.9.22
* @version 1.00
*/
	...
	*/
	public void putLob(String lobSb){
		...
		try{
			...
			oid = lob.getID();
			Assert.assertEquals(id,oid,"designated id must same as return oid");
		}catch(BaseException e){
		...
		try{
			...
			if(result > 60){
				Assert.assertTrue(false,"the create time:"+sdf.format(new Date(time))+"\n"
