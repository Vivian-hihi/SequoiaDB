...
/**
* FileName: TestUpsertShardingKey12629.java
* test interface:
* upsert (BSONObject matcher, BSONObject modifier, BSONObject hint,  int flag)
	...
	*/
	...
	public void upsertShardingKey(){
		try{
			...
			cl.upsert(matcher,modifier,hint, null, DBCollection.FLG_UPDATE_KEEP_SHARDINGKEY);			
			...
			cl.upsert(matcher2,modifier2,hint, setOnInsert, DBCollection.FLG_UPDATE_KEEP_SHARDINGKEY);
						
			//check upsertShardingKey results			
			String []expRecords = {"{a:[1,2,'test',[3,14]],no:0,b:0,c:'test0'}",		        
									"{a:2,no:[1,2,4],b:1,c:'test2'}",
