...
/**
* FileName: TestInsertToBson7152.java
* test interface:
...
public class TestInsertToBson7152 extends SdbTestBase{
	...
	public void insertDatas() throws ParseException{
		try{
			...
			Binary tmpBin = (Binary)obj.get("bindata");
			byte[] orig = tmpBin.getData();
			...
			rObj.put("_id",oid);
			Assert.assertEquals(1,cl.getCount(rObj),"the insert return oid is wrong");
	        ...
	        while(tmpCursor.hasNext()){
	            ...
	            /*Binary newval = (Binary)temp.get("bindata");
	            byte[] nval = newval.getData();
	            String out = new String(nval);
	...
	public void updateDatas(){
		try{
			...
			Assert.assertTrue(false,"update(DBQuery query) fail "+e.getMessage());
		}
	}
	...
	public void deleteDatas(){
		try{
			...
			Assert.assertTrue(false,"delete (BSONObject matcher)  fail "+e.getMessage());
		}
	}
