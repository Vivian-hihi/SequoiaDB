	...
	public void test(){
		...
		try{
			...
			while(csCursor.hasNext()){
				String actualCSName = (String) csCursor.getNext().get("Name");
			...
			while(clCursor.hasNext()){
				String actualCLName = (String) clCursor.getNext().get("Name");
		...
		try{
			for(int i = 0 ;i < nameNum; i++){
				for(int j = 0; j < numForOneName; j++){
					int age = 20;
					age = age + j;
					String insertData = "insert into "+ csName + "." + clName + "(name,age) "
							+ "values(" + "'"+ name + i + "'" + "," + age +")";
		...
		Assert.assertEquals(actualDataList, expectDataList);
			...
			Assert.fail("insert or select data failed, errMsg:" + e.getMessage());
		}
