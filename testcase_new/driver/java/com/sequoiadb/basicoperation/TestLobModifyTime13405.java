...
/**
* FileName: LockAndWriteLob13404.java
* test content:lock to write lob ,test the interface lock() and lockAndSeek()
* testlink case:seqDB-13404
* @author wuyan
    * @Date    2017.11.23
* @version 1.00
*/
public class TestLobModifyTime13405 extends SdbTestBase {	
	...
	private long putLob(){
		...
		try(DBLob lob = cl.createLob(oid) ){					
			...
			if( createTime > modifyTime){
				Assert.assertTrue(false, "the modify time must not be less than the create time!");
			}
		}		
		return modifyTime;
	}
	...
	private void getLastModifyTimeAndCheckResult(ObjectId oid, long lastModifyTime ){	
		...
		if( newModifyTime <= lastModifyTime){
			Assert.assertTrue(false, "the new modify time must not be less than the lastmodify time!");
		}
		...
		while ( listCursor.hasNext() ) {
            ...
            BSONTimestamp modifyTime = (BSONTimestamp) obj.get("ModificationTime");           
		...
		listCursor.close();
		Assert.assertEquals(modifyTimeFromDB, newModifyTime," java clent and db get modifytime inconsistency");
	}	
