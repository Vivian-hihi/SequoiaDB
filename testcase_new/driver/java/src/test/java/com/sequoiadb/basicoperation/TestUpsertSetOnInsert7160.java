...
/**
* FileName: TestUpsertSetOnInsert7160.java
* test interface:
* upsert (BSONObject matcher, BSONObject modifier, BSONObject hint, BSONObject setOnInsert)
* @author wuyan
    * @Date    2016.9.19
* @version 1.00
*/
public class TestUpsertSetOnInsert7160 extends SdbTestBase{
	...
	*test inteface:
	*upsert (BSONObject matcher, BSONObject modifier, BSONObject hint, BSONObject setOnInsert)
	*upsert (null, BSONObject modifier, null,null)
	*/
	@Test
	public void upsertData(){
		try{
			...
			//check the upsert result,match not to record then insert :{arr:[4],no:4}
			...
			Assert.assertTrue(false,"upsertSetOnInsert fail "+e.getMessage());
		}
	}	
