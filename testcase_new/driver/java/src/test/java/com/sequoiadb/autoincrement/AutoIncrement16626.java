...
public class AutoIncrement16626 extends SdbTestBase{
    ...
    public void TestMultiAutoIncrementOption(){
    	...
    	cl.createAutoIncrement(optionsList);
    	List<BSONObject> autoIncrementInfos = Commlib.GetAutoIncrementList(sdb, SdbTestBase.csName + "." + clName, 2);
    	...
    	for(int i = 0 ; i < 2 ; i++){
    		Assert.assertEquals(autoIncrementInfos.get(i).get("Generated").toString(), "strict");
        	
        	String sequenceName = autoIncrementInfos.get(i).get("SequenceName").toString();
        	BSONObject actOptions = Commlib.GetSequenceSnapshot(sdb, sequenceName);
        	Assert.assertEquals(actOptions.get("AcquireSize").toString(), "2000");
        	Assert.assertEquals(actOptions.get("CacheSize").toString(), "2000");
        	Assert.assertEquals(actOptions.get("Increment").toString(), "2");
        	Assert.assertEquals(actOptions.get("MaxValue").toString(), "5000");
        	Assert.assertEquals(actOptions.get("MinValue").toString(), "1");
        	Assert.assertEquals(actOptions.get("StartValue").toString(), "1");
    ...
    public void TestDropAutoIncrement(){
    	...
    	try{
    		...
    		Assert.fail("dropAutocrement with nonexistent field names should fail!");
    	}catch(BaseException e){
    	...
    	autoIncrementInfos = Commlib.GetAutoIncrementList(sdb, SdbTestBase.csName + "." + clName, 1);
    	Assert.assertEquals(autoIncrementInfos.size(), 0, "the autoincrement still exist!");
    	cs.dropCollection(clName);
    }
}
