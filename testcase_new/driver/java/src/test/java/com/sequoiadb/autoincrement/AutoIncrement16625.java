...
public class AutoIncrement16625 extends SdbTestBase{
    ...
    public void TestNoDefaultOption(){
    	...
    	autoIncrementInfo = Commlib.GetAutoIncrement(sdb, SdbTestBase.csName + "." + clName);
    	Assert.assertEquals(autoIncrementInfo.get("Field").toString(), "name");
    	Assert.assertEquals(autoIncrementInfo.get("Generated").toString(), "strict");
    	
    	String sequenceName = autoIncrementInfo.get("SequenceName").toString();
    	BSONObject actOptions = new BasicBSONObject();
    	actOptions = Commlib.GetSequenceSnapshot(sdb, sequenceName);
    	Assert.assertEquals(actOptions.get("AcquireSize").toString(), "1200");
    ...
    public void TestOptionField(){
    	...
    	BSONObject autoIncrementInfo = new BasicBSONObject();
    	autoIncrementInfo = Commlib.GetAutoIncrement(sdb, SdbTestBase.csName + "." + clName);
    	Assert.assertEquals(autoIncrementInfo.get("Field").toString(), "number");
    	Assert.assertEquals(autoIncrementInfo.get("Generated").toString(), "default");
    ...
    public void TestDropAutoIncrement(){
    	...
    	try{
    		...
    		Assert.fail("dropAutocrement with nonexistent field should fail!");
    	}catch(BaseException e){
    	...
    	BSONObject autoIncrementInfo = new BasicBSONObject();
    	autoIncrementInfo = Commlib.GetAutoIncrement(sdb, SdbTestBase.csName + "." + clName);
    	Assert.assertEquals(autoIncrementInfo.keySet().size(), 0, "the autoincrement still exist!");
    	cs.dropCollection(clName);
    }
