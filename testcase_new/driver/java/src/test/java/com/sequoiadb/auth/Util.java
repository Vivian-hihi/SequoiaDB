...
public class Util extends SdbTestBase{
    ...
    public static void createPasswdFile(String username, String password, String passwordFile)
            throws Exception {
        createPasswdFile(username,password,passwordFile,null);
    }

    public static void createPasswdFile(String username, String password, String passwordFile,String token)
            throws Exception {
        Ssh ssh = null;
        try {
            ssh = new Ssh(SdbTestBase.hostName, "root", SdbTestBase.rootPassword);
            String toolsPath = Util.getSdbInstallDir() + "/bin/";
            String passwdFilePath = toolsPath + passwordFile;
            String cmd = toolsPath + "sdbpasswd  -a \"" + username + "\" -p \"" + password + "\" -f \""
                    + passwdFilePath + "\"";
            if(token != null) {
                cmd += " -t \"" + token + "\"";
            }
    ...
    public static void downLoadFileToLocal(String localPath, String remotePath) throws Exception {
        Ssh ssh = null;
        try {
            ssh = new Ssh(SdbTestBase.hostName,"root", SdbTestBase.rootPassword);
    ...
    public static String getSdbInstallDir() throws Exception {
        Ssh ssh = new Ssh(SdbTestBase.hostName, "root", SdbTestBase.rootPassword);
        String dir = null;
        try {
    ...
    public static void removePasswdFile(String passwordFilePath) throws Exception {
        Ssh ssh = new Ssh(SdbTestBase.hostName, "root", SdbTestBase.rootPassword);
        try {
