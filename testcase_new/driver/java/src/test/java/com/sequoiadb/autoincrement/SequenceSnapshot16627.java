...
public class SequenceSnapshot16627 extends SdbTestBase{
    ...
    public void TestGetSnapshot(){
    	List<BSONObject> autoIncrementInfos = Commlib.GetAutoIncrementList(sdb, SdbTestBase.csName + "." + clName, 3);
    	...
    	for(int i=0 ; i<autoIncrementInfos.size() ; i++){
    		...
    		sequenceName.put("Name", autoIncrementInfos.get(i).get("SequenceName").toString());
    	...
    	DBCursor cur = sdb.getSnapshot(Sequoiadb.SDB_SNAP_SEQUENCES, matcher, selector, orderBy);
    	...
    	//getSnapshot(int snapType, BSONObject matcher, BSONObject selector, BSONObject orderBy, BSONObject hint, long skipRows, long returnRows)
    	cur = sdb.getSnapshot(Sequoiadb.SDB_SNAP_SEQUENCES, matcher, selector, orderBy, hint, skipRows, returnRows);
    	...
    	//getSnapshot(int snapType, String matcher, String selector, String orderBy)
    	cur = sdb.getSnapshot(Sequoiadb.SDB_SNAP_SEQUENCES, "{$or:"+sequenceNames.toString()+"}", "{StartValue:1}", "{StartValue:-1}");
    	expected = new BasicBSONList();
     	actual = new BasicBSONList();
     	expStartValue = new BasicBSONObject();
     	expStartValue.put("StartValue", 5);
    ...
    public void TestGetList(){
    	...
    	DBCursor cur = sdb.getList(Sequoiadb.SDB_LIST_SEQUENCES, query, selector, orderBy);
    	BasicBSONList expected = new BasicBSONList();
    	BasicBSONList actual = new BasicBSONList();
    	BSONObject expNameValue = new BasicBSONObject();
    	for(int i = autoIncrementInfos.size()-1 ; i > -1 ; i--){
    		expNameValue = new BasicBSONObject();
    		expNameValue.put("Name", autoIncrementInfos.get(i).get("SequenceName").toString());
