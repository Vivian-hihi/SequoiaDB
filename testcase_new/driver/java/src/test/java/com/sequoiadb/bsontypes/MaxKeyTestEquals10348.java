...
/**
* FileName: MaxKeyTestEquals10348.java* 
* test interface:
* equals (Object obj)
...
public class MaxKeyTestEquals10348 extends SdbTestBase{
	...
	public void testGetDateAndToString() {
		try{
			...
			Assert.assertEquals(maxkey.equals(maxkey),true,"check self fail");
			Assert.assertEquals(maxkey.equals(maxkey1),true,"check the same data fail");
			Assert.assertEquals(maxkey.equals(obj.get("maxkey")),true,"check the get object");
			Assert.assertEquals(maxkey.equals(null),false,"check null fail");
			String otherTypeValue = "MaxKey()";
			Assert.assertEquals(maxkey.equals(otherTypeValue),false,"check other type fail");			       
		}catch(BaseException  e){
