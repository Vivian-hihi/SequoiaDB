...
/**
* FileName: TestSnapshotAccessplan14511.java
* test interface:
* getSnapshot(int snapType, String matcher, String selector, String orderBy)
* the snapType is  Sequoiadb.SDB_SNAP_ACCESSPLANS	
* @author wuyan
    * @Date    2018.2.12
* @version 1.00
*/
public class TestSnapshotAccessplan14511 extends SdbTestBase{
    ...
    public void testSnapshotAccessplan() { 
    	...
    	BSONObject queryConf2 = (BSONObject) JSON.parse("{'$and':[{no:{$lt:400}},{no:{$gte:200}}]}");
    	...
    	DBCursor cursor = sdb.getSnapshot(Sequoiadb.SDB_SNAP_ACCESSPLANS, matcher, selector, orderBy);
        ...
        String []expAccessPlayInfo = {"{'MinTimeSpentQuery':{'ReturnNum':100}}",
    	        "{'MinTimeSpentQuery':{'ReturnNum':200}}",
    	        "{'MinTimeSpentQuery':{'ReturnNum':500}}"};
        for( int i = 0; i < expAccessPlayInfo.length; i++){
			BSONObject expRec =(BSONObject) JSON.parse(expAccessPlayInfo[i]);
			expectedList.add(expRec);
		}        
        Assert.assertEquals(actualList.toString(),expectedList.toString(),"actDatas: "+actualList.toString()+"\n"+"expRecs: "+expectedList.toString());
