...
/**  
     * FileName:        TestgetSnapshot15758   seqDB-15758
     * test interface:  getSnapshot(int snapType, BSONObject matcher, BSONObject selector, 
...
public class TestgetSnapshot15758 extends SdbTestBase{
    ...
    public void test() {
        ...
        DBCursor cursor = sdb.getSnapshot(Sequoiadb.SDB_SNAP_HEALTH, "{ \"IsPrimary\" : true , \"ServiceStatus\" : true }", "", "");
        int  length = 0;
        ...
        cursor = sdb.getSnapshot(Sequoiadb.SDB_SNAP_HEALTH, matcher, selector, orderBy, hint, length-1, 1);
        int  num = 0;
        while(cursor.hasNext()){
            num++;
