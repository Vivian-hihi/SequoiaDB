...
/**
* FileName: TestInsertToJson7151.java
* test interface:
...
public class TestInsertToJson7151 extends SdbTestBase{
	...
	public void insertDatas(){
		...
		obj.put("_id",oid);
		Assert.assertEquals(1,cl.getCount(obj),"the insert return oid is wrong");
	...
	public void checkClData(){
		...
		try{	
			...
			Assert.assertEquals(listExpdatas,list,"actDatas: "+list+"\n"+"expRecs: "+listExpdatas);
			cursor.close();
		}catch(BaseException e){			
			Assert.assertTrue(false,"check result is wrong, errMsg:%s\n"+e.getMessage());
		}finally{
			if (cursor != null){
				cursor.close();
			}
