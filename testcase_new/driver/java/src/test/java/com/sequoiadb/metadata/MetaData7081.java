	...
	public void test(){
		...
		for(i=0; i<clNameArr.length; i++ ){
			try{
				...
				//check cl name
				Assert.assertEquals(cs.getCollection(clNameArr[i]).getName(), clNameArr[i]);
				Assert.assertEquals(cs.getCollection(clNameArr[i]).getFullName(), commCSName + "." + clNameArr[i]);
				//check cs name
				Assert.assertEquals(cs.getCollection(clNameArr[i]).getCSName(), commCSName);
				
				//check cs
				Assert.assertEquals(cs.getCollection(clNameArr[i]).getCollectionSpace(), cs);
				
				//check sdb
				Assert.assertEquals(cs.getCollection(clNameArr[i]).getSequoiadb(), sdb);
				
			}catch(BaseException e){
				Assert.fail("create cl:" + clNameArr[i] + " failed, errMsg:" + e.getMessage());
			}finally{
				for(i = 0; i < clNameArr.length; i++){
					if(cs.isCollectionExist(clNameArr[i])){
						cs.dropCollection(clNameArr[i]);
