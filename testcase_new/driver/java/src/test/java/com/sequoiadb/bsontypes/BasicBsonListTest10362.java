...
/**
* FileName: BasicBsonListTest10362.java* 
* test interface:
* ObjectId ()
* TestLink: seqDB-10362:
* @author wuyan
    * @Date    2016.10.17
* @version 1.00
*/
public class BasicBsonListTest10362 extends SdbTestBase{
	...
	public void testBasicBsonList1() {
		try{
			...
			//test interface:put (int key, Object v),insert "no":[ "testlist" , "testlist1"] 
			list.put(0, "tlist0");
			list.put(1, "tlist1");
	        ...
	        //test the interface:get (String key),eg: "a" : [ { "no" : 123} , 2]
	       	BasicBSONList subResult = (BasicBSONList)actRecs.get("a");
	       	//the return is the second value:2
	       	Object rValue = subResult.get(1);	       
	       	Assert.assertEquals(rValue.toString(), "2","get() return value is wrong");
	       	
	       	//removeField (String key),remove the first key-value,then find by asList()
	       	subResult.removeField("0");
	       	
	    	Object asListValue = subResult.asList();	
	    	Assert.assertEquals(asListValue.toString(),"[2]","asListValue="+asListValue.toString());
	    	...
	    	Assert.assertEquals(rmap.toString(), "{1=1, 0=test}","actMap="+map.toString());
