...
public class RenameCS16544 extends SdbTestBase{	
	...
	@Test()
	public void test(){ 
		//test rename cs name is invalid parameter:include "$",".","","SYS";128bytes
		String[] newCSNamesA = { "$testcs16544","test.16544","","test16544csdddddaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
		...
		for( int i = 0; i < newCSNamesA.length; i++){
			try{
				...
				Assert.fail("newCSname is Invalid parameter, rename CS must be fail! newCSName is " + newNameA);
			}catch(BaseException e){			
				if( e.getErrorCode() != -6 ){
					Assert.fail("rename cs fail!newName:"+ newNameA + " " + e.getErrorCode()+e.getMessage());
				}
			}
		}
		
		//test rename cs name is valid parameter:special character/127bytes
		String[] newCSNamesB = { "test@#%&_asb16544","test16544asdddddaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
		...
		for( int i = 0; i < newCSNamesB.length; i++){
			...
			Assert.assertTrue(sdb.isCollectionSpaceExist(newCSName) , " the newCSName is " + newCSName);
			Assert.assertFalse(sdb.isCollectionSpaceExist(csName), " the oldCSName is " + csName);
			csName = newCSName;
		}		
	}
