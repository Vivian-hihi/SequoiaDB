...
/**
* FileName: TestWriteAndReadLob7098.java
* test interface:
	...
	*/
	public void readLob(ObjectId oid,String lobSb){
		...
		try{			
			...
			int readLen = 0; 
			do{
				//check return value is the total number of bytes read into the buffer
			...
			Assert.assertEquals(wMd5, rMd5,"the write an read file md5 different");
			
			//check the return value: -1 if the file has been reached
			Assert.assertEquals(rLob.read(buff,0, buff.length),-1,"the return must be 1");
			//check the return value:0 if len is Zero.
			String lobSb1 = getRandomString(0);
			byte[] buff1 = lobSb1.getBytes();
			Assert.assertEquals(rLob.read(buff1,0,0),0,"the return must be 0");		
		}catch(BaseException e){
