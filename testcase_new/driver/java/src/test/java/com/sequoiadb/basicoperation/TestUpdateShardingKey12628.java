...
/**
* FileName: TestUpdateShardingKey12628.java
* test interface:
* update (BSONObject matcher, BSONObject modifier, BSONObject hint,  int flag)
* update (String matcher, String modifier, String hint, int flag) 
...
public class TestUpdateShardingKey12628 extends SdbTestBase{
	...
	/**
	*test:
	*1.update ShardingKey by  update (String matcher, String modifier, String hint,int flag) 
	*2.update ShardingKey by  update(BSONObject matcher, BSONObject modifier, BSONObject hint,  int flag)
	*3.update ShardingKey by update( DBQuery query )
	*/
	@Test(enabled=false)
	public void updateShardingKey(){
		try{
			//update ShardingKey by  update (String matcher, String modifier, String hint,int flag) 
			...
			cl.update(matcher,modifier,hint, DBCollection.FLG_UPDATE_KEEP_SHARDINGKEY);		
			
			//update ShardingKey by  update(BSONObject matcher, BSONObject modifier, BSONObject hint,  int flag) 
			String matcher2  = "{ b : 1 }";
	...
	private void checkUpdateRecords(String []expRecords){
		try{			
	        ...
	        Assert.assertEquals(actualList.equals(expectedList),true,"check datas are unequal\n"
	        						...
	        						+"\nexpDatas: "+expectedList.toString());
		}catch(BaseException e){
			Assert.assertTrue(false,"check update result fail "+e.getMessage());	
		}		
	}	
