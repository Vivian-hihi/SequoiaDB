 ...
 */
public class Analyze14221 extends SdbTestBase {
    ...
    /**
     * 1.创建多个cs，并分别创建cl，在各cl上创建索引，插入数据，例如：插入数据包含a字段，值为0的2000条记录，任意数据2000条记录，其中a为索引字段
     * 2.在各cl中使用索引字段执行查询，检查扫描方式，例如：使用a=0执行查询
     * 3.执行统计，其中：不指定参数，执行步骤2中的查询，检查扫描方式，例如：使用a=0执行查询
     * <p>
     * 1.数据插入成功，索引创建成功 2.所有cl均使用索引执行查询 3.所有cl均使用表扫描执行查询
     */
    @Test
    public void test() {
