...
* @content   rename cs, the new cs name is exist;rename cl ,the old cl name is not exist.
	...
	public void test(){
		...
		try{
			...
			Assert.fail("newCSname is exist rename must be fail!");
				...
				if( e.getErrorCode() != -33 ){
					Assert.fail("rename cs fail!newName:"+ newCSName + " " + e.getErrorCode()+e.getMessage());
				}
		}		
		...
		try{
			...
			if( e.getErrorCode() != -23 ){
				Assert.fail("rename cl fail! " + e.getErrorCode()+e.getMessage());
			}
		}
	}
