...
/**
* FileName: TestSplit7112.java
* test interface:
* split(String sourceGroupName,String destGroupName, BSONObject splitCondition,BSONObject splitEndCondition)
* testlink cases:seqDB-7112
* testing strategy:    1.create rangecl,specify multiple shardingKey
* 					   2.insert records
*                      3.split by range,rg:{no:1,test:"test1"},{no:3,test:"test3"}
...
public class TestSplit7112 extends SdbTestBase {
	...
	public void checkSplitResult(){			
		...
		Assert.assertEquals((long)listRecsNums.get(0),8,"srcGroup count:"+(long)listRecsNums.get(0));
		Assert.assertEquals((long)listRecsNums.get(1),2,"tarGroup count:"+(long)listRecsNums.get(1));		
	}
