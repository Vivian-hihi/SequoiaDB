...
@input:        sdb:        Boolean
               hosts:      Array, one host    ---- 1g3d
                                  two hosts   ---- 2g3d
                                  three hosts ---- 3g3d
               eg: bin/sdb -f deploy.js -e 'var sdb=true; var hosts=["192.168.31.1", "192.168.31.2", "192.168.31.3"]'
...
{
   throw "Invalid param[sdb], should be Boolean" ;
}
...
function main()
{
   if ( DEPLOY_SEQUOIADB && HOSTS.length !== 0 )
   {
      deploySequoiadb() ;
   }
   else
   {
      println("Do not deploy sdb, because of sdb: " + DEPLOY_SEQUOIADB + ", hosts: " + HOSTS) ;
   }
}
...
function createCatalog( db, hostName, svcName, dbPath )
{
   ...
   println( "Create catalog: " + hostName + ":" + svcName ) ;
}
...
function createCoord( db, hostName, svcName, dbPath )
{
   ...
   println( "Create coord:   " + hostName + ":" + svcName ) ;
...
function createData( db, groupName, hostName, svcName, dbPath )
{
   ...
   println( "Create data:    " + hostName + ":" + svcName ) ;
