...
# FIXME: grep 4 mkdir
ret=$(echo ${testdir} | grep story)
if [ -n ${ret}"" ];then
   testRoots=($storyTestRoot)
else
   testRoots=($sdvTestRoot)
fi

# FIXME: lib dir used build valgrind's lib
libRoot="${curpath}/js/lib"
commlibstr="commlib.js"

# FIXME: use current sdb binstead /opt/sequoiadb/bin
sdbRoot="${curpath}/bin"
uuid=$$
uuname="s$$test"
...
function procJSFile()
{
   ...
   # FIXME: use basename shortFile name
   printOutFile=${reportDir}"/"`basename ${shortFile}`"_out.txt" 
...
{  
   local testRoot=$1
   # FIXME: use passDir's absulote dir
   if [ "$passDir" != "" ]; then
      if [ ${passDir:0:1} = "/" ] ; then
         testRoot="$passDir"
      else
         testRoot="$testRoot/$passDir"
      fi
   fi
...
{       
   if [ ! -d $reportDirRoot ] ; then
      # FIXME: do not remove reportDirRoot/*
      # rm -rf $reportDirRoot/*
      mkdir ${reportDirRoot}
   fi
}
