...
import com.sequoias3.commlibs3.Ssh;
...
public class S3NodeWrapper {
	...
	public S3NodeWrapper(String hostname, String port, String username, String password) throws Exception {
	...
	public boolean start() throws Exception {
		...
		try {
			...
			ssh.exec("source /etc/profile;" + this.dbInstallDir + "/tools/sequoias3/sequoias3.sh start "
					+ ">/tmp/s3start.log");
			if (ssh.getExitStatus() == 0) {
				this.status = NodeStatus.START_SUCCESS;
			} else {
				this.status = NodeStatus.START_FAILURE;
				throw new Exception("exec command : source /etc/profile;" + this.dbInstallDir
	...
	public boolean stop() throws Exception {
		...
		try {
			ssh = new Ssh(this.hostname, this.username, this.password);
			ssh.exec(this.dbInstallDir + "/tools/sequoias3/sequoias3.sh stop -a");
			if (ssh.getExitStatus() == 0) {
				this.status = NodeStatus.STOP_SUCCESS;
			} else {
				this.status = NodeStatus.STOP_FAILURE;
				throw new Exception("exec command : " + this.dbInstallDir + "/tools/sequoias3/sequoias3.sh stop -a "
	...
	public boolean isNodeActive() throws Exception {
		...
		try {
			ssh = new Ssh(this.hostname, this.username, this.password);
			ssh.exec(this.dbInstallDir + "/tools/sequoias3/sequoias3.sh status");
			String stdout;
			if (ssh.getExitStatus() == 0) {
				...
				throw new Exception("exec command : " + this.dbInstallDir + "/tools/sequoias3/sequoias3.sh status "
						+ "failed,stdout= " + ssh.getStdout() + ",stderr = " + ssh.getStderr());
			}
		} finally {
			if (ssh != null) {
	...
	public String toString() {
		return "[ s3hostname" + this.hostname + ",s3port = " + this.port + ",username = " + this.username
