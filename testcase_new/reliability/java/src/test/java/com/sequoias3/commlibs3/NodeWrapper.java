...
//            csqcur.close();
    ...
    public NodeCheckResult checkBusiness( boolean printRes ) {
        ...
        if ( svcPort >= S3TestBase.reservedPortBegin
                && svcPort <= S3TestBase.reservedPortEnd ) {
    ...
    public void backupDiaglog( String testCaseName )
        ...
        Ssh remote = new Ssh( hostName(), S3TestBase.remoteUser,
                S3TestBase.remotePwd ) ;
        remote.exec( String.format( "cp -r %s/diaglog %s/backup_%s", dbPath(),
                S3TestBase.workDir, testCaseName ) ) ;
