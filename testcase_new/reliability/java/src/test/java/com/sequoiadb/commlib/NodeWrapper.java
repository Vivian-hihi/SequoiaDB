...
public class NodeWrapper {
    ...
    public String toString() {
        return "NodeWrapper{" + "status=" + status + ", node=" + node + ", nodeInfo=" + nodeInfo + '}';
    }
...
//            DBCursor csqcur = coord.getSnapshot(Sequoiadb.SDB_SNAP_DATABASE, new BasicBSONObject(), null, null);
...
//            csqcur = coord.getSnapshot(Sequoiadb.SDB_SNAP_DATABASE, new BasicBSONObject(), null, null);
//            while(csqcur.hasNext()){
//                System.out.println(csqcur.getNext().toString());
//            }
//            csqcur.close();
    ...
    public boolean stop() throws ReliabilityException {
        try {
            Sequoiadb db = node.getReplicaGroup().getSequoiadb();
            System.out.println("stop() =====> " + hostName() + ":" + svcName() + "  " + db.hashCode());
            node.stop();
    ...
    public NodeCheckResult checkBusiness(boolean printRes) {
        ...
        try {
            ...
            if (obj.containsField("CurrentLSN")) {
                checkResult.LSNVer = ((BasicBSONObject) obj.get("CurrentLSN")).getInt("Version");
            }
