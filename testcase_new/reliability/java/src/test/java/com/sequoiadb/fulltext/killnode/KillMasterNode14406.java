...
public class KillMasterNode14406 extends SdbTestBase {
    ...
    public void test() throws Exception {
        try {
            this.cl.createIndex( fullIndexName, "{\"a\":\"text\"}", false,
                    false );
            this.insertData();
            Assert.assertTrue(
                    FullTextUtils.isIndexCreated( cl, fullIndexName, 500000 ) );
            GroupWrapper subCLGroup = groupMgr.getGroupByName( groupName );
            NodeWrapper subCLGroupMaster = subCLGroup.getMaster();

            // get masterNode
            Node preMasterNode = this.sdb
                    .getReplicaGroup( groupName ).getMaster();
            String preMasterHostName = subCLGroupMaster.hostName();
            String preMasterSvcName = subCLGroupMaster.svcName();
            System.out.println(
                    "Kill node:" + preMasterHostName + ":" + preMasterSvcName );

            // 建立并行任务
            FaultMakeTask faultTask = KillNode
                    .getFaultMakeTask( preMasterHostName, preMasterSvcName, 0 );
            ...
            // update
            this.cl.update( "{a:'a01'}", "{'$set':{a:'helloworld'}}",
                    "{'':null}" );
            Assert.assertTrue( FullTextUtils.isIndexCreated( cl, fullIndexName,
                    1000000 ) );
            ...
            Assert.assertTrue(
                    FullTextUtils.isIndexCreated( cl, fullIndexName, 999998 ) );

            // get old and new masterNode
            Sequoiadb preMasterNodeDB = new Sequoiadb( preMasterHostName,
                    Integer.valueOf( preMasterSvcName ), "", "" );
            Sequoiadb currentMasterNodeDB = this.sdb
                    .getReplicaGroup( groupName ).getMaster()
                    .connect();
            ...
            matcher.put( "a", "a22" );
            DBCursor cursor = preMasterNodeDB
