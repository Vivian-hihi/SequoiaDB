...
public class Vote14468 extends SdbTestBase {
    ...
    public void setUp() throws ReliabilityException{
        ...
        for(int i = 0; i < 5; i++){
            cl = cs.createCollection(clName+i ,(BSONObject) JSON.parse("{Group:'" + groupName + "'}"));
            cl.createIndex(indexName+i, "{a:'text',b:'text',c:'text',d:'text'}", false, false);
            String cappedName = FullTextDBUtils.getCappedName(cl, indexName+i);
            cappedNames.add(cappedName);
            String esIndexName = FullTextDBUtils.getESIndexName(cl, indexName+i);
    ...
    private class InsertTask extends OperateTask{
        ...
        public void exec() throws Exception {
            ...
            try{
                for(int i = 0; i< 5; i++){
                    cl = db.getCollectionSpace(csName).getCollection(clName+i);
                    FullTextDBUtils.insertData(cl, 10000);
                }
            }finally{
                db.close();
    ...
    private class ReelectTask extends OperateTask{
        ...
        public void exec() throws Exception {
            ...
            try{
                for(int i = 0; i < 5; i++){
                    db.getReplicaGroup(groupName).reelect((BSONObject)JSON.parse("{Seconds: 60}"));
                }
            }finally{
                db.close();
            }
