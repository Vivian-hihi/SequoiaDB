 ...
 * 预置条件：集合中已存在自增字段且CacheSize及AcquireSize均设置不为1
 * 操作步骤： 1.不指定自增字段插入记录，同时coord异常 
 *        2.待节点正常后，不指定自增字段继续插入记录 
...
public class Insert15974 extends SdbTestBase{
    ...
    public void test() {
		Sequoiadb db = null;
		try{
			FaultMakeTask faultTask = KillNode.getFaultMakeTask(SdbTestBase.coordUrl.split(":")[0], SdbTestBase.coordUrl.split(":")[1], 1);
	...
	public void createAutoIncrement(DBCollection cl, int autoIncrementNum){
		for(int i=0; i< autoIncrementNum ; i++){
			BSONObject obj = (BSONObject) JSON.parse("{Field:'id" + i + "',CacheSize:" + cacheSize + ",AcquireSize:" + acquireSize + "}");
	...
	public void checkResult(Sequoiadb db, int expectNum){
		...
		while(cursorS.hasNext()){
			 ...
			 ArrayList<BSONObject> autoIncrements = (ArrayList<BSONObject>) record.get("AutoIncrement");
			for(int i=0; i<autoIncrements.size(); i++){
				BSONObject autoIncrement = autoIncrements.get(i);
