...
/**
 * @Description seqDB-18528:主子表，转账的过程中正常所有数据节点主节点断网
 * @author yinzhen
 * @date 2019-6-19
 *
 */
@Test(groups = "rcauto")
public class Transaction18528 extends SdbTestBase {
    private Sequoiadb sdb;
    private DBCollection cl;
    private String clName = "cl18528";
    ...
    public void setUp() throws ReliabilityException {
        ...
        sdb.getCollectionSpace(csName).createCollection("sub118528");
        sdb.getCollectionSpace(csName).createCollection("sub218528", (BSONObject) JSON
                .parse("{'ShardingKey':{'account':1}, 'ShardingType':'hash', 'AutoSplit':true, 'ReplSize':-1}"));
        cl.attachCollection(csName + ".sub118528",
                (BSONObject) JSON.parse("{LowBound:{'account':{'$minKey':1}}, UpBound:{'account':3000}}"));
        cl.attachCollection(csName + ".sub218528",
            ...
            try {
                ...
                while (count++ < 600) {
                    ...
                    cl.update("{'account':" + accountB + "}", "{$inc:{'balance':" + transAmount + "}}",
                            "{'':'$shard'}");
                    db.commit();
                    System.out.println("account: " + accountA + " transfer to " + accountB + " $" + transAmount);
                    Thread.sleep(200);
                }
