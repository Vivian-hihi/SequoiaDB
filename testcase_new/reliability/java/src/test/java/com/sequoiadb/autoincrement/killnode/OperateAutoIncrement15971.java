...
/**
 * @FileName seqDB-15971: 新增/修改/删除自增字段时catalog备节点异常重启 
 * 预置条件：集合中已存在多个自增字段
 * 操作步骤：1.并发执行如下操作：a.新增自增字段 b.修改自增字段属性c.删除自增字段 d.插入记录，同时catalog备节点异常重启
 *        2.再次插入记录
 * 预期结果：1.自增字段操作成功，自增字段属性正确，记录插入成功
...
public class OperateAutoIncrement15971 extends SdbTestBase {
    ...
    public void test() {
		...
		try {
			...
			FaultMakeTask faultTask = KillNode.getFaultMakeTask(cataSlave.hostName(), cataSlave.svcName(), 10);
