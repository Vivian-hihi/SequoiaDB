...
public class RestartSlaveNode14407 extends SdbTestBase {
    ...
    public void test() {
        ...
        FullTextUtils.checkFullSyncToES( esClient, sdb, SdbTestBase.csName,
                this.clName, this.fullIndexName, 500000 );

        // get slaveNode and masterNode
        Node preSlaveNode = this.sdb.getReplicaGroup( this.groupNames.get( 0 ) )
                .getSlave();
        Node preMasterNode = this.sdb
                .getReplicaGroup( this.groupNames.get( 0 ) ).getMaster();
        ...
        Sequoiadb preSlaveNodeDB = new Sequoiadb( preSlaveHostName,
                preSlaveSvcName, "", "" );
        ...
        this.insertData();
        FullTextUtils.checkFullSyncToES( esClient, sdb, SdbTestBase.csName,
                this.clName, this.fullIndexName, 1000000 );
        ...
        FullTextUtils.checkFullSyncToES( esClient, sdb, SdbTestBase.csName,
                this.clName, this.fullIndexName, 999998 );

                
        // query previous slaveNode 
        BSONObject matcher = new BasicBSONObject();
        matcher.put( "a", "a22" );
        DBCursor cursor = preSlaveNodeDB
                .getCollectionSpace( SdbTestBase.csName )
                .getCollection( this.clName )
                .query( matcher, null, null, null );
        int count = 0;
        while ( cursor.hasNext() ) {
            BSONObject obj = (BSONObject)cursor.getNext();
            count++;
        }
        cursor.close();
        ...
        cursor = preMasterNodeDB.getCollectionSpace( SdbTestBase.csName )
                .getCollection( this.clName )
                .query( matcher, null, null, null );
        count = 0;
        while ( cursor.hasNext() ) {
