...
/**
 * @FileName seqDB-15972: 新增/修改/删除自增字段时catalog整组异常重启
 * 预置条件：集合中已存在多个自增字段
...
public class OperateAutoIncrement15972 extends SdbTestBase {
    ...
    public void test() {
		...
		try {
			...
			FaultMakeTask faultTask1 = KillNode.getFaultMakeTask(cataAllUrls.get(0).split(":")[0], cataAllUrls.get(0).split(":")[1], 10);
			FaultMakeTask faultTask2 = KillNode.getFaultMakeTask(cataAllUrls.get(1).split(":")[0], cataAllUrls.get(1).split(":")[1], 10);
			FaultMakeTask faultTask3 = KillNode.getFaultMakeTask(cataAllUrls.get(2).split(":")[0], cataAllUrls.get(2).split(":")[1], 10);
	...
	private class CreateAutoIncrementTask extends OperateTask {
        ...
        public void exec() throws Exception {
            ...
            try {
                ...
                for(int i=autoIncrementNum; i< autoIncrementNum*2; i++){
        			BSONObject obj = (BSONObject) JSON.parse("{Field:'id" + i + "',CacheSize:1,AcquireSize:1}");
        ...
        public void exec() throws Exception {
            ...
            try {
                ...
                for(int i=0; i< autoIncrementNum/2; i++){
        			BSONObject obj = (BSONObject) JSON.parse("{AutoIncrement:{Field:'id" + i + "',CacheSize:10,AcquireSize:10}}");
	...
	private class DropAutoIncrementTask extends OperateTask {
        ...
        public void exec() throws Exception {
            ...
            try {
                ...
                CollectionSpace cs = db.getCollectionSpace(csName);
                DBCollection cl = cs.getCollection(clName);
                for(int i=autoIncrementNum/2; i< autoIncrementNum; i++){
	...
	public void checkResult(Sequoiadb db, int expectNum){
		...
		//获取自增字段
		DBCursor cursorS = db.getSnapshot(8, (BSONObject) JSON.parse("{Name:'" + csName + "." + clName + "'}"), null, null);
		ArrayList<String> arrList = new ArrayList<String>();
		while(cursorS.hasNext()){
			ArrayList<BSONObject> record = (ArrayList<BSONObject>) cursorS.getNext().get("AutoIncrement");
		...
		//比较记录是否包含自增字段
		DBCursor cursorR = cl.query("{'mustCheckAutoIncrement':{$exists:1}}", null, null, null);
		while(cursorR.hasNext()){
			BSONObject record = cursorR.getNext();
			for(int i=0; i<arrList.size(); i++){
				boolean hasAutoIncrementField = record.containsField(arrList.get(i));
