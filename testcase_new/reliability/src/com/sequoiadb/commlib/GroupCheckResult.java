 ...
 * File Name:GroupCheckResult.java
 * 类的详细描述
 *
 *  @author 类创建者姓名
 * Date:2017-2-27下午12:54:17
 *  @version 1.00
 */
...
public class GroupCheckResult {
    ...
    public String toString() {
        String ret = String.format( "{GroupName:%s, " + "\nGroupID:%d,"
                + "\nPrimaryNode:%d," + "\nConnCheck:%s,"
                + "\nPrimaryCheck:%s," + "\nLSNCheck:%s,"
                + "\nServiceCheck:%s," + "\nDiskCheck:%s,"
                + "\nDeployCheck:%s\n", groupName, groupID, primaryNode,
                Boolean.toString( connCheck ),
                Boolean.toString( primaryCheck ), Boolean.toString( LSNCheck ),
                Boolean.toString( serviceCheck ),
                Boolean.toString( diskCheck ), Boolean.toString( deployCheck ) ) ;
        ret += "[" ;
