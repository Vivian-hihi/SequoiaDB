...
public class NetSplit2586 extends SdbTestBase {
    ...
    public void test() {
        ...
        try {
            ...
            // 得到一个非断网主机的coordurl
            connectUrl = Utils.getDiffHostWithSvc(srcSlaHost, groupMgr.getAllHosts());

            // 建立并行任务
            FaultMakeTask faultTask = BrokenNetwork.getFaultMakeTask(srcSlaHost, 3, 15, 25);
            ...
            //5000 + 5000 + 1000 - bound :插入线程插入的数据+setUp插入的数据+所有线程结束后插入的数据-切分至源组的数据 = 目标组数据量
