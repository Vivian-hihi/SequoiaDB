...
/**
 * @descreption seqDB-25308:创建SDB映射表，timestamp类型转换
 * @author YiPan
 * @date 2022/1/29
 * @updateUser
 * @updateDate
 ...
 * @version 1.0
 */
public class CreateTable25308 extends FlinkTestBase {
    private Sequoiadb sdb;
    private StreamTableEnvironment tableEnv;
    private DBCollection cl;
    private final String csName = "cs_25308";
    private final String clName = "cl_25308";
    private final String tableNameBase = "tb_25308_";
    private final String filed_Zero = "test_zero";
    private final String filed_Now = "test_now";
    private final long data_Zero = 0;
    private final long data_Now = System.currentTimeMillis();
    ...
    public void toSuccessType() throws Exception {
        DataType[] dataTypes = { DataTypes.BIGINT(), DataTypes.DOUBLE() };
        for ( int i = 0; i < dataTypes.length; i++ ) {
            String tableName = ConversionUtils.initTableName( tableNameBase,
                    dataTypes[ i ] );
            createTable( tableName, dataTypes[ i ], filed_Zero, filed_Now );
    ...
    private void toCHAR() throws Exception {
        ...
        for ( int i = 0; i < dataTypes.length; i++ ) {
            ...
            Row row = ConversionUtils.queryOne( tableEnv, tableName );
            Assert.assertEquals( row.getField( filed_Zero ),
                    new Timestamp( data_Zero ).toString() );
            Assert.assertEquals( row.getField( filed_Now ),
                    new Timestamp( data_Now ).toString() );
        }
    }
    ...
    private void insertData() {
        ...
        bson.put( filed_Zero, new Timestamp( data_Zero ) );
        bson.put( filed_Now, new Timestamp( data_Now ) );
        cl.insert( bson );
        System.out
                .println( "CreateTable25308 insert timestamp is " + data_Now );
    }
