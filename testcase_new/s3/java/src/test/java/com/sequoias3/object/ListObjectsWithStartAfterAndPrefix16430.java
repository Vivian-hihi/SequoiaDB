...
public class ListObjectsWithStartAfterAndPrefix16430 extends S3TestBase {
	...
	private void listObjectsAndCheckResult(List<String> keyList, int startAfterNo, int maxKeys) throws IOException {
		...
		ListObjectsV2Request request = new ListObjectsV2Request().withBucketName(bucketName).withEncodingType("url")
				.withStartAfter(expKeyList.get(startAfterNo)).withPrefix(prefix).withMaxKeys(maxKeys);
