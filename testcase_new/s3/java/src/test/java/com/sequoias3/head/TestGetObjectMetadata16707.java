...
public class TestGetObjectMetadata16707 extends S3TestBase {
    ...
    private void testGetObjectMetadata() throws Exception {
        ...
        ObjectMetadata metadata = s3Client
                .getObjectMetadata(new GetObjectMetadataRequest(bucketName, keyName, versionidV3));
        Date currDate = metadata.getLastModified();

        // 指定ifNoneMatch/ifMatch/ifModifiedSince/ifNoneModifiedSince条件查询对象
        HttpHead request = new HttpHead(
                S3TestBase.s3ClientUrl + "/" + bucketName + "/" + keyName + "?versionId=" + versionidV2);
        ...
        request.setHeader("If-Unmodified-Since", HeadUtils.getModifiedGMTDate(currDate, 1));
