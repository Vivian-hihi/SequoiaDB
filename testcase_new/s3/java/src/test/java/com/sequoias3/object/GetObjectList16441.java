...
public class GetObjectList16441 extends S3TestBase {
	...
	private void setUp() throws Exception {
		...
		for(int i = 0 ; i < commPrefixesNums ; i++){
			...
			s3Client.putObject(bucketName, currentKeyName, "object_file16441");
			expCommPrefixesList.add(currentKeyName.substring(0,currentKeyName.lastIndexOf(delimiter)+1));
		}
	...
	private void checkContentsResult(List<S3ObjectSummary> objectSummaries){
		...
		for(int i = 0 ; i < objectSummaries.size() ; i++){
			Assert.assertEquals(objectSummaries.get(i).getKey(), expContentKeyList.get(i));
			Assert.assertEquals(objectSummaries.get(i).getOwner().getDisplayName(), userName);
		}
	}
}
