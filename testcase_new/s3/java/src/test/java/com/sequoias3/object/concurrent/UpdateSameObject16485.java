...
public class UpdateSameObject16485 extends S3TestBase {
    ...
    private void checkUpdateObjectResult(AmazonS3 s3Client) throws IOException {
        ...
        Assert.assertEquals(listObjectsV2Result.getObjectSummaries().get(0).getBucketName(), bucketName,
                "bucketName is wrong!");
        Assert.assertEquals(listObjectsV2Result.getObjectSummaries().get(0).getKey(), keyName, "keyName is wrong!");
        Assert.assertEquals(listObjectsV2Result.getObjectSummaries().get(0).getETag(),
                TestTools.getMD5(newContent.getBytes()), "content is wrong!");
