...
public class SetObjectAcl19463 extends S3TestBase {
    ...
    private void testSetObjectAcl() throws Exception {
        ...
        try {
            s3Client.setObjectAcl(non_existBucket, keyName, CannedAccessControlList.PublicReadWrite);
        ...
        try {
            s3Client.setObjectAcl(bucketName, non_existObject, "0", CannedAccessControlList.PublicReadWrite);
            Assert.fail("expect failed but found success.");
        } catch (AmazonS3Exception e) {
        ...
        // 使用标准acl配置对象acl，其中指定对象不存在（指定versionId）
        try {
            s3Client.setObjectAcl(bucketName, non_existObject, CannedAccessControlList.PublicReadWrite);
            Assert.fail("expect failed but found success.");
        } catch (AmazonS3Exception e) {
