...
public class UploadPart18679 extends S3TestBase {
    ...
    private void test(long partSize1, long partSize2, long partSize3, long partSize4, 
            long partSize5, long partSize6) throws Exception {
        // init file
        long fileSize = partSize1 + partSize2 + partSize3 + partSize4 + partSize5 + partSize6;
        ...
        long[] partSizes = {partSize1, partSize2, partSize3, partSize4, partSize5, partSize6};
        List<PartETag> partETags = this.partUpload(uploadId, partSizes, filePath);
    ...
    private void tearDown() {
        try {
            if (actSuccessTests.getAndIncrement() == generateFileSize().length) {
    ...
    private List<PartETag> partUpload(String uploadId, long[] partSizes, String filePath){
        File file = new File(filePath);
        int fileOffset = 0;
        List<PartETag> partEtags = new ArrayList<>();
        for (int i = 0; i < partSizes.length; i++) {
            UploadPartRequest partRequest = new UploadPartRequest().withFile(file)
                    .withFileOffset(fileOffset).withPartNumber(i+1).withPartSize(partSizes[i])
