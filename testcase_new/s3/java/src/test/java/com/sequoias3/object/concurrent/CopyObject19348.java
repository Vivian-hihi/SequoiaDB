...
public class CopyObject19348 extends S3TestBase {
    ...
    public void testCopyObject() throws Exception {
        ...
        threadExec.addWorker(new CopyObject(userMetaA, contentDispositionA));
        threadExec.addWorker(new CopyObject(userMetaB, contentDispositionB));
        threadExec.run();

        checkObjectMetaData(userMetaA, userMetaB, contentDispositionA, contentDispositionB);
    ...
    private class CopyObject {
        ...
        private CopyObject(Map<String, String> userMeta, String contentDisposition) {
    ...
    private void checkObjectMetaData(Map<String, String> expMetaA, Map<String, String> expMetaB,
            String contentDispositionA, String contentDispositionB) {
        ...
        if (actMeta.size() == expMetaA.size()) {
            for (Map.Entry<String, String> entry : expMetaA.entrySet()) {
                ...
                Assert.assertEquals(actMeta.get(key), expMetaA.get(key),
                        "actMetaA = " + actMeta.toString() + ",expMetaA = " + expMetaA.toString());
            }
            Assert.assertEquals(result.getContentDisposition(), contentDispositionA);
        } else {
            Assert.assertEquals(actMeta.size(), expMetaB.size(),
                    "expMetaB is : " + expMetaB.toString() + "actMetaB is : " + actMeta.toString());
            for (Map.Entry<String, String> entry : expMetaB.entrySet()) {
                Object key = entry.getKey();
                Assert.assertEquals(actMeta.get(key), expMetaB.get(key),
                        "actMetaB = " + actMeta.toString() + ",expMeta = " + expMetaB.toString());
            }
            Assert.assertEquals(result.getContentDisposition(), contentDispositionB);
        }
