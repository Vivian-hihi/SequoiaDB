...
public class CopyObject19349 extends S3TestBase {
    ...
    private void checkObjectMetaData(Map<String, String> expMeta, String contentDisposition) {
        ...
        Assert.assertEquals(actMeta.size(), expMeta.size(),
                "expMetaB is : " + expMeta.toString() + "actMetaB is : " + actMeta.toString());
        for (Map.Entry<String, String> entry : expMeta.entrySet()) {
            Object key = entry.getKey();
            Assert.assertEquals(actMeta.get(key), expMeta.get(key),
                    "actMetaB = " + actMeta.toString() + ",expMeta = " + expMeta.toString());
        }
        Assert.assertEquals(result.getContentDisposition(), contentDisposition);
