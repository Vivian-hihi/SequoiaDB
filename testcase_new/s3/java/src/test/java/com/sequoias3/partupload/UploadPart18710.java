...
public class UploadPart18710 extends S3TestBase {
	...
	private void testUpload() throws Exception {
		...
		// 分10个分段上传
		List<PartETag> partEtags = PartUploadUtils.partUpload(s3Client, bucketName, keyName, uploadId1, oldFile,
				10 * 1024 * 1024);
		PartUploadUtils.completeMultipartUpload(s3Client, bucketName, keyName, uploadId1, partEtags);

		// 再次指定相同key初始化分段上传对象
		String uploadId2 = PartUploadUtils.initPartUpload(s3Client, bucketName, keyName);
		// 分20个分段上传
		partEtags = PartUploadUtils.partUpload(s3Client, bucketName, keyName, uploadId2, newFile);

		// 查询分段列表
		checkPartList(20, uploadId2);

		PartUploadUtils.completeMultipartUpload(s3Client, bucketName, keyName, uploadId2, partEtags);
		checkUploadResult();
		runSuccess = true;
	}
	...
	private void checkPartList(int partNum, String uploadId) {
		...
		for (int i = 0; i < parts.size(); i++) {
			...
			Assert.assertEquals(partSize, PartUploadUtils.partLimitMinSize, "partnumber = " + partNumber);
		}
	}

	private void checkUploadResult() throws Exception {
		String actMd5 = ObjectUtils.getMd5OfObject(s3Client, localPath, bucketName, keyName, "1");
		String expMd5 = TestTools.getMD5(newfilePath);
		Assert.assertEquals(actMd5, expMd5, "version id = 1");

		actMd5 = ObjectUtils.getMd5OfObject(s3Client, localPath, bucketName, keyName, "0");
