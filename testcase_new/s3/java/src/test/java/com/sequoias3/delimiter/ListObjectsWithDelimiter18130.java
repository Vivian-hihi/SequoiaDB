...
public class ListObjectsWithDelimiter18130 extends S3TestBase {
	...
	private void setUp() throws Exception {
		...
		for (int i = 0; i < objectNames.length; i++) {
			s3Client.putObject(bucketName, objectNames[i], "object_file18130");
		}
	}
	...
	public void testGetObjectList() throws Exception {
		...
		int listReturnTimes = (int) Math.ceil((double) expCommprefixes.size() / maxkeys);
		...
		do {
			...
			if (times == listReturnTimes && lastReturnNum != 0) {
				Assert.assertEquals(result.getKeyCount(), lastReturnNum, "return num is wrong in last round.");
			} else {
				Assert.assertEquals(result.getKeyCount(), maxkeys, "return num is wrong");
			}
		...
		Assert.assertEquals(contentsResult.size(), 0, "contentsResult : " + contentsResult.toString());
		runSuccess = true;
	}
