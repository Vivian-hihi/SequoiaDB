...
public class InitiateMultipartUploadRequest18806 extends S3TestBase {
	...
	public void testIllegalKeyName() throws Exception {
		...
		initRequest = new InitiateMultipartUploadRequest(bucketName, ObjectUtils.getRandomString(901));
		try {
			s3Client.initiateMultipartUpload(initRequest);
		...
		// test b : 桶名为null
		initRequest = new InitiateMultipartUploadRequest(null, "/dir/test18806");
		try {
			s3Client.initiateMultipartUpload(initRequest);
