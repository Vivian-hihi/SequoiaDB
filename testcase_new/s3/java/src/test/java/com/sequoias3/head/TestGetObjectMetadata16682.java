...
public class TestGetObjectMetadata16682 extends S3TestBase {
    ...
    private void testGetObjectMetadata() throws Exception {
        ...
        ObjectMetadata metadata = s3Client
                .getObjectMetadata(new GetObjectMetadataRequest(bucketName, keyName, result1.getVersionId()));
        checkResult(metadata, result1, (long) contentV1.length(), datev1_1, datev1_2);

        metadata = s3Client
                .getObjectMetadata(new GetObjectMetadataRequest(bucketName, keyName, result2.getVersionId()));
    ...
    private void checkResult(ObjectMetadata metadata, PutObjectResult result, long expsize, Date date1, Date date2) {
        ...
        if (actDate.getTime() < (date1.getTime() / 1000) * 1000
            ...
            Assert.fail("lastmodified is wrong!  actDate is : " + HeadUtils.getGMTDate(actDate) + ", date1 is :"
                    + HeadUtils.getGMTDate(date1) + ", date2 is : " + HeadUtils.getGMTDate(date2));
        }
    }
}
