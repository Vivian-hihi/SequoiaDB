...
/**
 * test content:  withMetadata接口参数校验
 * testlink-case: seqDB-16479
 * @author wangkexin
 * @Date 2019.01.07
 * @version 1.00
 */
public class TestWithMetadata16479 extends S3TestBase {
	...
	public void testLegalMetadata(Map<String,String> expMeta) throws Exception {
		...
		Assert.assertEquals(actMeta.size(), expMeta.size(), "expMeta is : " + printMap(expMeta) + "actMeta is : " + printMap(actMeta));
		for(Map.Entry<String, String> entry : expMeta.entrySet()){
			String expValue = entry.getValue() == null?"":entry.getValue();
			String actValue = actMeta.get(entry.getKey()) == null?"":actMeta.get(entry.getKey());
