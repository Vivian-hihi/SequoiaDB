...
/**
 * test content:  PutRegion接口参数校验 
 * testlink-case: seqDB-17352
 * @author wangkexin
 * @Date 2019.01.28
 * @version 1.00
 */
public class TestPutRegion17352 extends S3TestBase{
	...
	private void checkShardingTypeMode(String regionName, String dataCSShardingType, String dataCLShardingType) throws Exception{
		Assert.assertTrue(RegionUtils.headRegion(regionName));
    	GetRegionResult result = RegionUtils.getRegion(regionName);
    	Region region = result.getRegion();
    	Assert.assertEquals(region.getName(), regionName.toLowerCase());
