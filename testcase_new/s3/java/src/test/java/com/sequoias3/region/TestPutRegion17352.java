...
public class TestPutRegion17352 extends S3TestBase {
    ...
    public Object[][] generateRegionName() {
        return new Object[][] {
                ...
                new Object[] { ObjectUtils.getRandomString(20), "shardingTypeMode", "year", "quarter" },
                // test c : 包含字母 数字字符[0-9a-zA-Z]
                new Object[] { "01abcdefgABCDEFG", "shardingTypeMode", "quarter", "month" },
                new Object[] { "234hijklmnHIJKLMN", "shardingTypeMode", "month", "year" },
    ...
    public void legalRegionName(String regionName, String mode, String dataCSShardingType, String dataCLShardingType)
            throws Exception {
        ...
        if (mode.equals("shardingTypeMode")) {
            Region region = new Region();
            region.withName(regionName).withDataCSShardingType(dataCSShardingType)
                    .withDataCLShardingType(dataCLShardingType).withDataDomain(dataDomain).withMetaDomain(metaDomain);
            RegionUtils.putRegion(region);
    ...
    public void illegalParameterName() throws Exception {
        ...
        try {
            Region region = new Region();
            region.withName("region17352").withMetaLocation("").withMetaHisLocation(metaCSName + "." + metaClNames[1])
                    .withDataLocation(dataCSName + "." + dataClName[0]);
            RegionUtils.putRegion(region);
        ...
        try {
            Region region = new Region();
            region.withName("region17352").withMetaLocation(metaCSName + "." + metaClNames[0]).withMetaHisLocation("")
                    .withDataLocation(dataCSName + "." + dataClName[0]);
            RegionUtils.putRegion(region);
        ...
        try {
            Region region = new Region();
            region.withName("region17352").withMetaLocation(metaCSName + "." + metaClNames[0])
        ...
        try {
            Region region = new Region();
            region.withName("region17352").withDataCSShardingType("day").withDataCLShardingType("month")
                    .withDataDomain(dataDomain).withMetaDomain(metaDomain);
            RegionUtils.putRegion(region);
            Assert.fail("put region with illegal dataCSShardingType should fail!");
        } catch (AmazonS3Exception e) {
        ...
        try {
            Region region = new Region();
            region.withName("region17352").withDataCSShardingType("year").withDataCLShardingType("day")
                    .withDataDomain(dataDomain).withMetaDomain(metaDomain);
            RegionUtils.putRegion(region);
            Assert.fail("put region with illegal dataCLShardingType should fail!");
        } catch (AmazonS3Exception e) {
            Assert.assertEquals(e.getErrorCode(), "InvalidShardingType");
        }
        runSuccess = true;
    ...
    private void checkShardingTypeMode(String regionName, String dataCSShardingType, String dataCLShardingType)
            throws Exception {
        Assert.assertTrue(RegionUtils.headRegion(regionName));
        GetRegionResult result = RegionUtils.getRegion(regionName);
        Region region = result.getRegion();
