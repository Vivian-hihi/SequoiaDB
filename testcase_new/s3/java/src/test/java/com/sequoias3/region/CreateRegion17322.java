...
public class CreateRegion17322 extends S3TestBase {
    ...
    private void checkGetRegionResult(GetRegionResult result, Region expRegion, List<String> expBucketNames)
            throws Exception {
        ...
        Assert.assertEquals(actBuckets.size(), expBucketNames.size(),
                "actBuckets = " + actBuckets.toString() + ",expBucketNames = " + expBucketNames.toString());
        for (int i = 0; i < actBuckets.size(); i++) {
            if (!expBucketNames.contains(actBuckets.get(i).getName())) {
                throw new Exception("exp bucketName not in act BuckNames" + ",actBucket = " + actBuckets.toString()
