...
public class DeleteRegion17324 extends S3TestBase {
    ...
    private void testStatic() throws Exception {
        ...
        region.withDataLocation(csName + "." + clNames[0]).withMetaLocation(csName + "." + clNames[1])
                .withMetaHisLocation(csName + "." + clNames[2]).withName(regionName1);
    ...
    private void testDynamic() throws Exception {
        // create region
        Region region = new Region();
        region.withDataCSShardingType("year").withDataCLShardingType("year").withName(regionName2);
        RegionUtils.putRegion(region);
        ...
        String csName1 = RegionUtils.getDataCSName(regionName2, "year", new Date());
