...
public class GetObjectVersionList16392 extends S3TestBase {
    ...
    private void checkVersionsResult(List<S3VersionSummary> verList) {
        ...
        for (int i = 0; i < verList.size(); i++) {
            ...
            Assert.assertEquals(verList.get(i).getVersionId(), expVersionId[i],
                    "version id is wrong! the key is : " + verList.get(i).getKey());
            Assert.assertEquals(verList.get(i).getETag(), expVersionsKeyEtag.get(i),
                    "etag is wrong! the key is : " + verList.get(i).getKey());
        }
    }
}
