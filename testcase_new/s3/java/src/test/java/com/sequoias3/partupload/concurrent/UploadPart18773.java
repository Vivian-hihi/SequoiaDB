...
public class UploadPart18773 extends S3TestBase {
	...
	// TODO:2、用例实现和文本用例有差别，需要开启版本控制，这样才能实现同一对象不同uploadId并发，由于不同对象版本互相不冲突，上传多个版本对象不会被覆盖
	@Test
	private void test() throws Exception {
		...
		for (int i = 0; i < firstUploadPartTimes; i++) {
			...
			int partNumber = i + 1;
			this.uploadFirstPart(uploadIds.get(0), fileOffset, partNumber, partSize);
		}

		// upload multi part again, and complete upload
		ThreadExecutor threadExec = new ThreadExecutor();
		for (int i = firstUploadPartTimes; i < uploadPartTimes - firstUploadPartTimes; i++) {
			int fileOffset = partSize * i;
			int partNumber = i + 1;
			threadExec.addWorker(new ThreadUploadOtherParts(uploadIds.get(i + 1), fileOffset, partNumber, partSize));
		}
		threadExec
				.addWorker(new ThreadCompleteUploadPart(uploadIds.get(0), partETags.subList(0, firstUploadPartTimes)));
		threadExec.run();
	...
	private class ThreadUploadOtherParts {
		...
		public ThreadUploadOtherParts(String uploadId, int fileOffset, int partNumber, int partSize) {
		...
		private void uploadPart() {
			...
			try {
				...
				UploadPartRequest partRequest = new UploadPartRequest().withFile(file).withFileOffset(fileOffset)
						.withPartNumber(partNumber).withPartSize(partSize).withBucketName(bucketName).withKey(key)
	...
	private class ThreadCompleteUploadPart {
		...
		public ThreadCompleteUploadPart(String uploadId, List<PartETag> eTags) {
			this.uploadId = uploadId;
			this.eTags = eTags;
		}
	...
	private void uploadFirstPart(String uploadId, int fileOffset, int partNumber, int partSize) {
		UploadPartRequest partRequest = new UploadPartRequest().withFile(file).withFileOffset(fileOffset)
				.withPartNumber(partNumber).withPartSize(partSize).withBucketName(bucketName).withKey(key)
				.withUploadId(uploadId);
