...
public class CreateObject16343 extends S3TestBase {
    ...
    private void checkObjectResult(S3Object object, String versionid, Date expDate, String expContent)
            throws Exception {
        // check create time
        ObjectMetadata metadata = object.getObjectMetadata();
        Date actCreateDate = metadata.getLastModified();
        if (actCreateDate.after(expDate)) {
            Assert.fail("object create time is different! versionid is " + metadata.getVersionId()
                    + ",the actCreateDate is : " + actCreateDate.toString() + ",the expDate is : "
                    + expDate.toString());
        }
