...
/**
 * test content: head查询桶接口参数校验 
 * testlink-case: seqDB-16668 
 * @author wangkexin
 * @Date 2018.12.07
 * @version 1.00
 */
...
public class TestHeadBucket16668  extends S3TestBase{
	...
	private void testDoesObjectExist() throws Exception {
		...
		try{
			...
			Assert.fail("test b headBucket:bucket name is illegal should fail");
		...
		try{
			...
			Assert.fail("test b doesBucketExist:bucket name is illegal should fail");
		}catch(IllegalArgumentException e){
			Assert.assertTrue(e.getMessage().equals("The bucketName parameter must be specified."));
		}
