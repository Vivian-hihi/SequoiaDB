...
public class ListObjectsWithDelimiter18120 extends S3TestBase {
    ...
    public Object[][] generatePageSize() throws UnsupportedEncodingException {
        return new Object[][]{
                ...
                new Object[]{URLEncoder.encode("/test1/AZ/", encodingType), 0},
                // test b:delimiter type is special chararcter
                new Object[]{URLEncoder.encode("/test*_.(d!-t'')", encodingType), 1},
                // test c:delimiter type is &@:,$=+?;ASCII
                new Object[]{URLEncoder.encode("/test&@:,$=+? t_1", encodingType), 2},
                // http://jira:8080/browse/SEQUOIADBMAINSTREAM-4392
                new Object[]{URLEncoder.encode("\040te\065s", encodingType), 3},
                new Object[]{URLEncoder.encode("/\035te\41a\57", encodingType), 4},
                // test d: delimiter type is 、^`><{}[]#%"~|
                new Object[]{URLEncoder.encode("test、^`><{}[]#%\"~|_1", encodingType), 5}
        };
    }

    @Test(dataProvider = "listWithDelimiterProvider")
    public void testListObjects(String delimiter, int position) throws Exception {
        updateDelimiterSuccessAgain(bucketName, delimiter, S3TestBase.s3AccessKeyId, "url");
        DelimiterConfiguration currDelimiter = DelimiterUtils.getDelimiterWithEncodingType(bucketName,
                S3TestBase.s3AccessKeyId, "url");
        Assert.assertEquals(URLDecoder.decode(currDelimiter.getDelimiter(), encodingType),
    ...
    private void listObjectsAndCheckResult(String delimiter, int position) throws IOException {
        ...
        Assert.assertEquals(commonPrefixes.get(0), URLDecoder.decode(delimiter, "utf-8"));
    ...
    public static void updateDelimiterSuccessAgain(String bucketName, String delimiter, String accessKeyId,
        ...
        while (tryTimes-- > 0) {
            try {
                DelimiterUtils.putBucketDelimiterWithEncodingType(bucketName, delimiter, accessKeyId, encodingType);
                return;
