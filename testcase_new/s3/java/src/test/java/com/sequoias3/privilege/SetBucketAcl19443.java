...
public class SetBucketAcl19443 extends S3TestBase {
    ...
    private void testSetBucketAcl() throws Exception {
        ...
        PrivilegeUtils.checkSetBucketAclResult(ownerS3Client, bucketName, expGrant1);
        getObjectByOtherUser();

        // 使用x-amz-grant-*方式配置桶acl
        for (Permission permission : Permission.values()) {
            Grant expGrant2 = new Grant(new CanonicalGrantee(ownerId), permission);
            PrivilegeUtils.setBucketAclByHeader(s3AccessKeyId, bucketName, expGrant2);
            PrivilegeUtils.checkSetBucketAclResult(ownerS3Client, bucketName, expGrant2);
            getObjectByOtherUser();
        }
        ...
        for (Permission permission : Permission.values()) {
            ...
            PrivilegeUtils.setBucketAclByBody(ownerS3Client, bucketName, expGrant3);
            PrivilegeUtils.checkSetBucketAclResult(ownerS3Client, bucketName, expGrant3);
            getObjectByOtherUser();
        }
