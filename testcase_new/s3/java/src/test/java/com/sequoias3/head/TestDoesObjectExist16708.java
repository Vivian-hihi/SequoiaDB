...
/**
 * test content: head查询对象接口参数校验
 * testlink-case: seqDB-16708 
 * @author wangkexin
 * @Date 2018.12.07
 * @version 1.00
 */
...
public class TestDoesObjectExist16708  extends S3TestBase{
	...
	private void testDoesObjectExist() throws Exception {
		...
		try{
			...
			Assert.fail("test c doesObjectExist:bucket name is illegal should fail");
		}catch(IllegalArgumentException e){
			Assert.assertTrue(e.getMessage().equals("The bucket name parameter must be specified when requesting an object's metadata"));
		}
		
		try{
			Assert.assertFalse(s3Client.doesObjectExist(bucketName, null));
			Assert.fail("test c doesObjectExist:key name is illegal should fail");
		}catch(IllegalArgumentException e){
			Assert.assertTrue(e.getMessage().equals("The key parameter must be specified when requesting an object's metadata"));
		}
		...
		try{
			...
			Assert.fail("test c getObjectMetadata:bucket name is illegal should fail");
		}catch(IllegalArgumentException e){
			Assert.assertTrue(e.getMessage().equals("The bucket name parameter must be specified when requesting an object's metadata"));
		}
		...
		try{
			...
			Assert.fail("test c getObjectMetadata:bucket name is \"\" should fail");
