...
/**
 * test content: 指定versionID查询对象不存在 
 * testlink-case: seqDB-16688 
 * @author wangkexin
 * @Date 2018.12.10
 * @version 1.00
 */
...
public class TestGetObjectMetadata16688  extends S3TestBase{
	...
	private void testGetObjectMetadata() throws Exception {
		...
		try{
			s3Client.getObjectMetadata(new GetObjectMetadataRequest(bucketName, keyName, notMatchVersionid));
		...
		try{
			s3Client.getObjectMetadata(new GetObjectMetadataRequest(bucketName, notMatchKeyName, versionid));
		...
		try{
			s3Client.getObjectMetadata(new GetObjectMetadataRequest(bucketName, notMatchKeyName, notMatchVersionid));
