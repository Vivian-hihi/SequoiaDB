...
def insert_boundary_max( cl ):   
   ...
   condition = {"a":OrderedDict([("$type",1),("$et",18)])}
   selector = { "a": "" }
   rec = cl.query(condition=condition, selector=selector).next()
   ...
   if( dumps(rec) != '{"a": 9223372036854775807}' ):
      print( 'dumps(rec), expect: {"a": 9223372036854775807}, actual: %s' % ( dumps(rec) ) )
...
def insert_boundary_min( cl ):
   ...
   if( dumps(rec) != '{"a": -9223372036854775808}' ):
      print( 'dumps(rec), expect: {"a": -9223372036854775808}, actual: %s' % ( dumps(rec) ) )
      raise  Exception( 'CHECK_JSON_ERROR' ) 
...
def insert_int32( cl ):    
   ...
   condition = {"a":OrderedDict([("$type",1),("$et",16)])}
   cnt = cl.get_count(condition=condition)
...
def query_js_max_safe_integet( cl ):       
   ...
   rec = cl.query( selector = { "a": "" } ).next()
         
   if( dumps(rec) != '{"a": -9007199254740992}' ):
      print( 'dumps(rec), expect: {"a": -9007199254740992}, actual: %s' % ( dumps(rec) ) )
      raise  Exception( 'CHECK_JSON_ERROR' )
   ...
   if( dumps(rec) != '{"a": -9007199254740991}' ):
      print( 'dumps(rec), expect: {"a": -9007199254740991}, actual: %s' % ( dumps(rec) ) )
      raise  Exception( 'CHECK_JSON_ERROR' )   
