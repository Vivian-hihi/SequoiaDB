...
public class Transaction5998 extends SdbTestBase {
    ...
    private void test() throws Exception { 
        ...
        long cnt1 = cl.getCount(new BasicBSONObject("a", new BasicBSONObject("$lt", 10)));
        Assert.assertEquals(REPEAT_BEGIN_NUM, cnt1); 
        
        long cnt2 = cl.getCount(new BasicBSONObject("a", new BasicBSONObject("$gte", DOCS_NUM)));
        Assert.assertEquals(REPEAT_BEGIN_NUM, cnt2); 
        
        ArrayList<BSONObject> andArr = new ArrayList<>();
        andArr.add(new BasicBSONObject("a", new BasicBSONObject("$gte", REPEAT_BEGIN_NUM)));
        andArr.add(new BasicBSONObject("a", new BasicBSONObject("$lt", DOCS_NUM)));
    ...
    private class trans2 {
        ...
        private void update() {
            ArrayList<BSONObject> insertor = new ArrayList<>();
            for (int i = REPEAT_BEGIN_NUM; i < DOCS_NUM + REPEAT_BEGIN_NUM; i++) {
                insertor.add(new BasicBSONObject("a",i));
            }
            cl.insert(insertor);
        }
