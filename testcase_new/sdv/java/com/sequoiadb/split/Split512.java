...
public class Split512 extends SdbTestBase {
	...
	public void setUp() {
		try {
			...
			ArrayList<String> tmp = MySdbTools.getGroupName(commSdb, csName, clName); // 获取切分组名
	...
	public void insertAndCheck(Sequoiadb sdb, Sequoiadb destDataNode, Sequoiadb srcdataNode) {
		...
		try {
			...
			if (!MySdbTools.isCollectionContainThisJSON(destGroupCL, bobj.toString())) {
			...
			if (!MySdbTools.isCollectionContainThisJSON(srcGroupCL, bobj2.toString())) {
				Assert.fail("check query data not pass");
			}
		} catch (BaseException e) {
			Assert.fail(e.getMessage());
