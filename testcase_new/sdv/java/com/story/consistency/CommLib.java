...
public class CommLib {
				...
				for(int i = 0; i < cataInfo.size(); ++i){
					...
					do{
						...
						for(int j = 0; j < nodeAddrs.size(); j++){
							...
							DBCursor cur = dataDB.getCollectionSpace(tmpCSName).getCollection(tmpCLName).
										getIndexes();
							//get the data for each node
							ArrayList<BSONObject> oneNodeData = new ArrayList<BSONObject>();
							while(cur.hasNext()){
							...
							if(j > 0){
								if ( !allNodeData.get(j).equals( allNodeData.get(j-1) ) ){
									/*
									System.out.println( String.valueOf(allNodeData.get(j)) );
									System.out.println( allNodeData.get(j) );
									System.out.println( allNodeData.get(j-1) );
									System.out.println();
									*/
									checkSucc = true;
									break;
	...
	public void createNode(Sequoiadb sdb, 
		...
		try{
			...
			do{
				...
				try{
					...
					if(e.getErrorCode() == -157){ 
						//-157:Invalid node configuration(Port is occupied)
						svnName = svnName + 10;
					}
				}
			...
			if(e.getErrorCode() != -153 && e.getErrorCode() != -156){  
				//-153:Group already exist; -156: Failed to start the node
				Assert.fail(e.getMessage());
			}
		}
	}
