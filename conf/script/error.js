...
var SDB_ERROR_RESERVED_1              = -72   ; // Reserved;
...
var SDB_CM_OP_NODE_FAILED             = -160  ; // Failed to operate on node;
...
var SDB_RTN_CMD_NO_SERVICE_AUTH       = -166  ; // The command can't be run in the service plane;
...
var SDB_CATA_RM_NODE_FORBIDDEN        = -204  ; // Unable to remove the last node or primary in a group;
var SDB_CATA_FAILED_TO_CLEANUP        = -205  ; // Unable to clean up catalog, manual cleanup may be required;
var SDB_CATA_RM_CATA_FORBIDDEN        = -206  ; // Unable to remove primary catalog or catalog group for non-empty database;
var SDB_ERROR_RESERVED_2              = -207  ; // Reserved;
...
var SDB_DMS_CS_NOT_EMPTY              = -275  ; // There are some collections in the collection space;
var SDB_CAT_LOCALHOST_CONFLICT        = -276  ; // 'localhost' and '127.0.0.1' cannot be used mixed with other hostname and IP address;
var SDB_CAT_NOT_LOCALCONN             = -277  ; // If use 'localhost' and '127.0.0.1' for hostname, coord and catalog must in the same host ;
var SDB_CAT_IS_NOT_DATAGROUP          = -278  ; // The special group is not data group;
var SDB_RTN_AUTOINDEXID_IS_FALSE      = -279  ; // can not update/delete records when $id index does not exist;
var SDB_CLS_CAN_NOT_STEP_UP           = -280  ; // can not step up when primary node exists or LSN is not the biggest;
var SDB_CAT_IMAGE_ADDR_CONFLICT       = -281  ; // Image address is conflict with the self cluster;
var SDB_CAT_GROUP_HASNOT_IMAGE        = -282  ; // The data group does not have image group;
var SDB_CAT_GROUP_HAS_IMAGE           = -283  ; // The data group has image group;
var SDB_CAT_IMAGE_IS_ENABLED          = -284  ; // The image is in enabled status;
var SDB_CAT_IMAGE_NOT_CONFIG          = -285  ; // The cluster's image does not configured;
var SDB_CAT_DUAL_WRITABLE             = -286  ; // This cluster and image cluster is both writable;
var SDB_CAT_CLUSTER_IS_READONLY       = -287  ; // This cluster is readonly;
var SDB_RTN_QUERYMODIFY_SORT_NO_IDX   = -288  ; // Sorting of 'query and modify' must use index;
var SDB_RTN_QUERYMODIFY_MULTI_NODES   = -289  ; // 'query and modify' can't use skip and limit in multiple nodes or sub-collections;
var SDB_DIR_NOT_EMPTY                 = -290  ; // Given path is not empty;
var SDB_IXM_EXIST_COVERD_ONE          = -291  ; // Exist one index which can cover this scene;
var SDB_CAT_IMAGE_IS_CONFIGURED       = -292  ; // The cluster's image has already configured;
var SDB_RTN_CMD_IN_LOCAL_MODE         = -293  ; // The command is in local mode;
var SDB_SPT_NOT_SPECIAL_JSON          = -294  ; // The object is not a special object in sdb shell;
var SDB_AUTH_USER_ALREADY_EXIST       = -295  ; // The specified user already exist;
var SDB_DMS_EMPTY_COLLECTION          = -296  ; // The collection is empty;
...
var SDB_DS_NOT_ENABLE                 = -308  ; // data source is not enabled yet;
var SDB_DS_NO_REACHABLE_COORD         = -309  ; // No reachable coord notes;
...
var SDB_DS_NOT_INIT                   = -314  ; // Data source has not been initialized;
var SDB_OPERATION_INCOMPATIBLE        = -315  ; // Operation is incompatible with the object;
var SDB_CAT_CLUSTER_IS_DEACTIVED      = -316  ; // This cluster is deactived;
