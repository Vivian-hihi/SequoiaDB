...
var FILE_NAME_INSTALL_ZOOKEEPER = "installZooKeeper.js" ;
...
function _init()
{
   ...
   catch ( e )
   {
      ...
      PD_LOG( arguments, PDERROR, FILE_NAME_INSTALL_ZOOKEEPER,
              sprintf( errMsg + ", rc: ?, detail: ?", GETLASTERROR(), GETLASTERRMSG() ) ) ;
      exception_handle( SDB_SYS, errMsg ) ;
   }
   setTaskLogFileName( task_id ) ;
   
   PD_LOG2( task_id, arguments, PDEVENT, FILE_NAME_INSTALL_ZOOKEEPER,
...
function _checkPath( ssh, path, type )
{
      ...
      if ( SYS_LINUX == SYS_TYPE )
      {
         ...
         if ( SDB_OK == ret ) // in case the path exists
         {
            ...
            if ( SDB_OK == ret )
            {
               ...
               PD_LOG2( task_id, arguments, PDERROR, FILE_NAME_INSTALL_ZOOKEEPER, errMsg ) ;
   ...
   catch( e )
   {
      SYSEXPHANDLE( e ) ;
      rc = GETLASTERROR() ;
      errMsg = sprintf( "Error happened when check ? path ", type ) ;
...
function _stopZNode( ssh, installPath )
{
   ...
   if ( 0 != ret )
   {
      ...
      PD_LOG2( task_id, arguments, PDWARNING, FILE_NAME_INSTALL_ZOOKEEPER,
   ...
   try
   {
      ...
      if ( SYS_LINUX == SYS_TYPE )
      {
         ...
         if ( SDB_OK != ret )
         {
            ...
            PD_LOG2( task_id, arguments, PDWARNING, FILE_NAME_INSTALL_ZOOKEEPER, errMsg ) ;
         }
      }
      else
      {
...
function main()
{
   ...
   try
   {
      ...
      _startZNode( ssh, installPath, dataPath ) ;
      // 9. disconnect
