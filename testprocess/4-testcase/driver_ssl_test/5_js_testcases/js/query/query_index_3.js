...
{
   println( "failed to find records after create unique index, rc= " + e ) ;
   throw e ;
}
...
if( count != 10 )
{
   println("return wrong number of records after create unique index");
...
while ( looptime < 1  )
{
   ...
   {
      println( "failed to find record after drop the unique index, rc= " + e ) ;
      throw e ;
   }
...
if( count != 10 )
{
   println("return wrong number of records after drop unique index");
   println(varCL.find({$or:[{no:{$gt:250,$lt:500}},{no:{$gt:500,$lt:900}}],name:{$regex:"foo*"},no:{$mod:[5,3]},$and:[{no:{$gt:800}},{no:{$lt:850}}],name:{$nin:["foo898","foo893","foo888"]}}));
   throw -1 ;
}
