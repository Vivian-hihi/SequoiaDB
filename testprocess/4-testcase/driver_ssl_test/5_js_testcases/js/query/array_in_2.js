/* *****************************************************************************
@discretion: array: $in with index
@modify list:
...
//create index
try{
	varCL.createIndex(CSPREFIX_IDX,{"b":1});
}catch(e)
{
	if(e!=-247){
...
println("********************************************");
try{
	var sel1 = varCL.find({b:{$in:[1,2,3]}}).hint({"":CSPREFIX_IDX});
}catch(e){
...
println("********************************************");
try{
	var sel2 = varCL.find({b:{$in:[1]}}).hint({"":CSPREFIX_IDX});
}catch(e){
...
println("********************************************");
try{
	var sel3 = varCL.find({b:{$in:[4,5,6,7,8]}}).hint({"":CSPREFIX_IDX});
}catch(e){
...
println("********************************************");
try{
	var sel4 = varCL.find({b:{$in:[]}}).hint({"":CSPREFIX_IDX});
}catch(e){
