/* *****************************************************************************
@discretion: array: match records are not only array-type
@modify list:
...
println("********************************************");
try{
	var sel1 = varCL.find({a:[1,2]});
}catch(e){
	println("find({a:[1,2]}) failed! rc=" + e);
	throw e;
}
//verify data
try{
...
println("********************************************");
try{
	var sel2 = varCL.find({a:[1,2]}).hint({"":CSPREFIX_IDX});
}catch(e){
	println("find({a:[1,2]}) failed! rc=" + e);
	throw e;
}
...
println("********************************************");
try{
	var sel3 = varCL.find({a:[1]}).hint({"":CSPREFIX_IDX});
}catch(e){
