/* *****************************************************************************
@discretion: jira-510: $type $elemMatch $regex
...
println("********************************************");
try{
	var sel1 = varCL.find({$or:[{a:1},{a:{$type:2}}]}).sort({b:1}).hint({"":CSPREFIX_IDX});
}catch(e){
	println("find({$or:[{a:1},{a:{$type:2}}]}) failed! rc=" + e);
	throw e;
}
//verify data
try{
...
println("find({$or:[{a:1},{a:{$type:2}}]}) succ!");

//find({a:{$type:3,$elemMatch:{name:'tom',phone:123}}})
//result:
//		{a:{name:'tom',phone:123},b:3}
println("********************************************");
try{
	var sel2 = varCL.find({a:{$type:3,$elemMatch:{name:'tom',phone:123}}}).hint({"":CSPREFIX_IDX});
}catch(e){
	println("find({a:{$type:3,$elemMatch:{name:'tom',phone:123}}}) failed! rc=" + e);
	throw e;
}
//verify data
try{
...
println("find({a:{$type:3,$elemMatch:{name:'tom',phone:123}}}) succ!");
...
println("********************************************");
try{
	var sel3 = varCL.find({$or:[{a:{$gt:0}},{a:{$regex:'abcdefgh',$options:'i'}}]}).sort({b:1}).hint({"":CSPREFIX_IDX});
}catch(e){
