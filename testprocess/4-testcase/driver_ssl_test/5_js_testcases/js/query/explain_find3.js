/******************************************************************************
@Description : 1. hash collection's query explain
@Modify list :
...
function main()
{
	...
	//create hashCL
	try{
		var varCL = varCS.createCL(CSPREFIX_CL,{ShardingKey:{a:1,b:-1},ShardingType:'hash',Partition:4096,Compressed:true});
	...
	try{
		varCL.find().explain();
		varCL.find({a:{$et:-9900},b:{$et:100}},{b:1,c:'abcd'}).explain();
		varCL.find({a:{$lt:-5000},b:{$in:[3000,4000,5000,6000,7000]}}).sort({b:1}).explain();
		varCL.find().sort({a:-1}).limit(100).skip(1000).explain();
	...
	//create index
	try{
		varCL.createIndex(CSPREFIX_IDX,{c:1},false,false);
	}catch(e)
	{
	...
	try{
		if(groups_num>1){
			...
			for(var i=0;i<partId;i++){
				...
				lowPar = i*stepPar;
				highPar = (i+1)*stepPar;
				varCL.split(sourceGroup, grouplist[tarGroupIndex],{Partition:lowPar},{Partition:highPar});
