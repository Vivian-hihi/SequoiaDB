/* *****************************************************************************
@discretion: array: $all with index
@modify list:
...
println("********************************************");
try{
	var sel1 = varCL.find({a:{$all:[1,2,3]}}).sort({b:1}).hint({"":CSPREFIX_IDX});
}catch(e){
	println("find({a:{$all:[1,2,3]}}) failed! rc=" + e);
	throw e;
}
//verify data
try{
	...
	if(rec2.toObj()['b']!=3){
		...
		println("expect:" + "{a:[1,3,2],b:3,c:'array'}");
		println("return:" + rec2);
		throw 'result1-error';
	}
...
println("********************************************");
try{
	var sel2 = varCL.find({a:{$all:[3,1]}}).sort({b:1}).hint({"":CSPREFIX_IDX});
}catch(e){
	println("find({a:{$all:[3,1]}}) failed! rc=" + e);
	throw e;
}
//verify data
try{
...
println("********************************************");
try{
	var sel3 = varCL.find({a:{$all:[1,2,3,4]}}).hint({"":CSPREFIX_IDX});
}catch(e){
	println("find({a:{$all:[1,2,3,4]}}) failed! rc=" + e);
	throw e;
}
//verify data
try{
...
println("find({a:{$all:[1,2,3,4]}}) succ!");
