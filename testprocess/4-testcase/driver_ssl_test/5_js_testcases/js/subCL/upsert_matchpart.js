...
function test_range_attach_hash_upsert_basic()// NOT Error, test mainCL'ShardingType is range and subCL's ShardingType is hash , insert's result
{
	...
	try
	{
		...
		println( "subCL2" );
		mainCL.attachCL( CS_Name+".month1", { LowBound:{b:0},UpBound:{a:50} } ) ;
		println( "attach subCL1" ) ;
		mainCL.attachCL( CS_Name+".month2", { LowBound:{a:50},UpBound:{a:{$maxKey:1}} } ) ;
		//mainCL.attachCL( "cs_subCL_test.month2", { LowBound:{a:50},UpBound:{a:100} } ) ;
