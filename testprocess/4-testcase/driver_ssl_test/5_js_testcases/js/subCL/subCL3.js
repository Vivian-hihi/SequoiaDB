/* *****************************************************************************
@discretion: attach hashCL and insert
@modify list:
   2014-07-30 pusheng Ding  Init
***************************************************************************** */

function test_range_attach_hash_insert_large_same() // 
{
	...
	try
	{
		...
		println( "mainCL" );
		var subCL1 = cs.createCL( subCl_Name + "1", { ShardingKey:{ a:1 }, ShardingType: "hash", ReplSize:0, Compressed:false, IsMainCL:false } ) ;
		println( "subCL1" );
...
try
{
   ...
   test_range_attach_hash_insert_large_same();
}
catch( e )
{
   if( "ModeStandAlone" == e )
