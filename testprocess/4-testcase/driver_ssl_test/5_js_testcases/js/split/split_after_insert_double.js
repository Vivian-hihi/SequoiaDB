...
function getPG(){
var db = new SecureSdb(COORDHOSTNAME,coordPort);

var catadb = new SecureSdb(COORDHOSTNAME,cataPort);
...
//createCL , split in id
var varCL = varCS.createCL(CSPREFIX_CL,{ShardingKey:{id:1} , ReplSize:claSize.ReplSize(),Compressed:true});
//createCL , split in id
var varCL2 = varCS.createCL(CSPREFIX_CL+"2",{ShardingKey:{id:1} , ReplSize:claSize.ReplSize(),Compressed:true});
                ...
                if( PGname != arrGroupName[i][0] ){
                        ...
                        if( _PGname != arrGroupName[i][0] ){
                                ...
                                try{
                                        ...
                                        varCL2.split( PGname,arrGroupName[i][0],{id:500*t} );
                                        while( catadb.SYSCAT.SYSTASKS.find().count() != 0 );
...
for(var i = 0 ; i != SLgroupID.length ; ++i){
        ...
        try{
                ...
                var len2 = eval( "gdb."+CSPREFIX_CS+"."+CSPREFIX_CL+"2"+".find().size()" );
                println(len);
                println(len2);
        }catch(e){
             throw e;
