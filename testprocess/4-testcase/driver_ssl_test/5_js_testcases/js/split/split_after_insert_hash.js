...
_Partition=8192;
//createCL , split in a
var varCL = varCS.createCL(CSPREFIX_CL,{ShardingKey:{id:1} ,ShardingType:"hash",Partition:8192, ReplSize:claSize.ReplSize(),Compressed:true});
...
for( var i = 0 ; i != 500*2 ; ++i ){
        ...
        for(var i=0; i !=arrGroupName.length ;++i){
                ...
                if( PGname != arrGroupName[i][0] ){
                        ...
                        try{
                           varCL.split( PGname,arrGroupName[i][0],{Partition:4096} );
                ...
                if( PGname != arrGroupName[i][0] ){
                        ...
                        if( _PGname != arrGroupName[i][0] ){
                                ...
                                try{
                                        ...
                                        varCL.split( PGname,arrGroupName[i][0],{Partition:Math.round(_Partition/arrGroupName.length)*t} );
                                        while( catadb.SYSCAT.SYSTASKS.find().count() != 0 );
