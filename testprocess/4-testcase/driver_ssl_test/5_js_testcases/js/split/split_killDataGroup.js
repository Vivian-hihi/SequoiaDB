		...
		if( _PGname != arrGroupName[i][0] )
		{
			...
			try
			{
				...
				else
				{
					...
					try
					{
						db.getRG(_PGname).stop();
					}
					catch(e)
					{
						println("can't stop "+_PGname+" : "+e);
					}
				}
			}
			catch(e)
...
println(catadb.SYSCAT.SYSCOLLECTIONS.find());
			try
			{
				println(arrGroupName[0][0]+"is starting now");
				db.getRG(_PGname).start();
			}
			catch(e)
			{
				println("can't startRG:"+e);
			}		
			try
			{
