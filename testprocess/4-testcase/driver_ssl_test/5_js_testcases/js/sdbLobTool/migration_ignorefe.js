...
function testExportImportParaIgnore( db, lobNum, exportCmd, exportFile,
                               importCmd, newCLNAME, expSplit, impSplit, migCmd )
{
   if( undefined == expSplit ){ expSplit = null ; }
   if( undefined == impSplit ){ impSplit = null ; }
   try {
      // put lob data
      var fileName = CSPREFIX + "exportDefault.txt" ;
      var lobNum = 100 ;
      ...
      var fileName = CSPREFIX + "_mig_ignorefe_newCL.txt" ;
...
function main( db )
{
   ...
   // put lob data
   var lobNum = 100 ;
   var filename = CSPREFIX + "_mig_ignorefe.file" ;
...
*******************************************************************************/
   ...
   var migCmd = InstallPath + "/bin/sdblobtool --hostname " + COORDHOSTNAME +
                ...
                " --dstcollection " + impFullCL + " --ignorefe true" ;
...
*******************************************************************************/
   ...
   var migCmd = InstallPath + "/bin/sdblobtool --hostname " + COORDHOSTNAME +
                ...
                "--collection " + expFullCL + " --file " + exportFile +
                " --dsthost " + COORDHOSTNAME + " --dstservice " + COORDSVCNAME +
                " --dstcollection " + impFullCL + " --ignorefe false" ;
