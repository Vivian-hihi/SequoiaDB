...
try
{
	var rc = cl.aggregate({$match:{no:{$in:[1000,1003,1007,1002,1006,1004,1009]}}},{$project:{no:1,info:1}},{$skip:2},{$limit:6});
}
catch(e)
{
...
var no_array =  new Array(1003,1004,1006,1007,1009);
for(var k=0;k<noarray.length;k++)
{
	if(noarray[k]!=no_array[k]){
		println("return wrong record information,noarray["+k+"]="+noarray[k]);
		throw -1 ;
		}
}
...
if( i != 5 )
{
	...
	println(cl.aggregate({$match:{no:{$in:[1000,1003,1007,1002,1006,1004,1009]}}},{$project:{no:1,info:1}},{$skip:2},{$limit:6}));
	throw -1 ;
}
