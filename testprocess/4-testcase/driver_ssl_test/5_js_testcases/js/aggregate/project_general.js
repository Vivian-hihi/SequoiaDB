...
try
{
	var rc=cl.aggregate({$project:{no:1,major:0,dep:1,"info.name":1,"infoS":"$info.sex","INFO":"$info"}});
//	println("the result is "+rc) ;
}
catch(e)
{
...
while(rc.next())
{
	...
	for(var fieldname in obj)
	{
        ...
        if(!((fieldarray.length == 5) && (fieldarray[0]=="no"&&fieldarray[1]=="dep"&&fieldarray[2]=="info.name" && fieldarray[3]=="infoS"&&fieldarray[4]=="INFO")))
	{
...
if(i != 12)
{
	...
	//println(cl.aggregate({$project:{no:1,major:0,dep:1,"info.name":1,"infoS":"$info.sex","INFO":"$info"}}));	
	throw -1 ;	
}
try
{
