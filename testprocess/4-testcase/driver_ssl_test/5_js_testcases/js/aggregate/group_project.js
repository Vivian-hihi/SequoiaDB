...
try
{
	var rc = cl.aggregate({$group:{_id:"$major",major:{$first:"$major"},score:{$first:"$score"}}},{$sort:{major:-1}},{$project:{major:0,score:1}},{$limit:3});
...
while(rc.next()){
	...
	if(!((fieldarray.length == 2) && (fieldarray[0]=="score" && fieldarray[1]=="major")))
	{
...
if( i != 3 )
{
   ...
   println(cl.aggregate({$group:{_id:"$major",major:{$first:"$major"},score:{$first:"$score"}}},{$sort:{major:-1}},{$project:{major:0,score:1}},{$limit:3}));
   throw -1 ;
}
