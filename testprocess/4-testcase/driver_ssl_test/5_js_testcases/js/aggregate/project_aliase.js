...
try
{
	var rc = cl.aggregate({$project:{no:1,Major:"$major",dep:0,name:"$info.name","info.sex":0}});
}
catch(e)
{
...
while(rc.next())
{
	...
	if(!((fieldarray.length == 3) && (fieldarray[0]=="no"&&fieldarray[1]=="Major"&&fieldarray[2]=="name")))
	{
	...
	//println(rc.current().toObj()["name"]);
	if(rc.current().toObj()["name"]==null)
	{
		println("the information of record is wrong ");
		throw -1;
	}
}
...
if( i != 17 )
{
	...
	cl.aggregate({$match:{$or:[{$and:[{no:{$gte:1000,$lte:1004}},{major:"计算机软件与理论"}]},{major:"电学"}]}});
	throw -1 ;
}
