...
try
{
   var rc =  cl.aggregate({$group:{_id:{dep:"$dep",major:"$major"}}});
}
catch(e)
{
   println("failed to execut aggregate function when _id more one,e="+e);
   throw e ;
}
...
{
   i++;
   majorarray.push(rc.current().toObj()["major"]);
}

var major_array = new Array("光学","物理学","电学","计算机工程","计算机科学与技术","计算机软件与理论");
for(var k =0 ;k<majorarray.length;k++){

   if(majorarray[k]!=major_array[k]){
      println("return the wrong record infomation majorarray["+k+"]="+majorarray[k]);
      throw -1 ;
      }
   }
...
if( i != 6 )
{
   ...
   println(cl.aggregate({$group:{_id:{dep:"$dep",major:"$major"}}}));
   throw -1 ;
}
try
{
