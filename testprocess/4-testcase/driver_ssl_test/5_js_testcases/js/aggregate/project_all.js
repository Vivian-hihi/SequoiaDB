...
try
{
   var rc = cl.aggregate({$project:{no:0,major:0,dep:0,"info.name":0,"info.sex":0}});
}
catch(e)
{
...
while(rc.next())
{
   ...
   if(!((fieldarray.length == 5) && (fieldarray[0]=="_id"&&fieldarray[1]=="no"&&fieldarray[2]=="major"&&fieldarray[3]=="dep"&&fieldarray[4]=="info")))
   {
...
if( i != 12 )
{
   ...
   println(cl.aggregate({$project:{no:0,major:0,dep:0,"info.name":0,"info.sex":0}}));
   throw -1 ;
}
