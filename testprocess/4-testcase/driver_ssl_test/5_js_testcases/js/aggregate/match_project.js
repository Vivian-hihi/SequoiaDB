...
try
{
	var rc = cl.aggregate({$match:{$and:[{no:{$gt:1001,$lt:1012}},{dep:"计算机学院"}]}},{$project:{no:1,"info.name":1,major:1,dep:1}});
}
catch(e)
{
...
while(rc.next())
{
	...
	name.push(obj["info.name"]);
	for(var fieldname in obj)
	{
		fieldarray.push(fieldname);
	}
	if(!((fieldarray.length == 4) && (fieldarray[0]=="no"&&fieldarray[1]=="info.name"&&fieldarray[2]=="major"&&fieldarray[3]=="dep")))
	{
...
if(!(name[0]=="Holiday"&&name[1]=="Sam"&&name[2]== "Coll"&&name[3]=="Jim"))
{
	println("return wrong information about the field info.name");
	cl.aggregate({$project:{no:1,"info.name":1,major:1,dep:1}},{$match:{$and:[{no:{$gt:1001,$lt:1012}},{dep:"计算机学院"}]}});
	throw -1 ;
}
if( i != 4 )
{
