...
try
{
	var rc = cl.aggregate({$match:{no:{$lt:1011}}},{$project:{no:1,info:1}},{$match:{"info.sex":"男"}},{$limit:4});
}
catch(e)
{
...
if( i != 4 )
{
	...
	println(cl.aggregate({$match:{no:{$lt:1011}}},{$project:{no:1,info:1}},{$match:{"info.sex":"男"}},{limit:4}));
	throw -1 ;
}
