...
try
{
	var rc = cl.aggregate({$project:{no:1,"info.name":1,major:1,dep:1}},{$match:{$and:[{no:{$gt:1001,$lt:1012}},{dep:"计算机学院"}]}},{$project:{no:1,major:1,"info.name":1}},{$group:{_id:"$major"}},{$sort:{no:-1}});

}
catch(e)
{
	println("failed to execut aggregate function limit when the value is -3,e="+e);
	throw e ;	
}
var i =0 ;
var Major = new Array();
while(rc.next())
{
	...
	if(!((fieldarray.length == 3) &&(fieldarray[0]=="no"&&fieldarray[2]=="info.name"&&fieldarray[1]=="major")))
	{
...
if(!(Major[0]=="计算机工程"&&Major[1]=="计算机软件与理论"))
{
	println("the information is wrong ...");
	throw -1 ;
}
...
if( i != 2 )
{
	...
	println(cl.aggregate({$project:{no:1,"info.name":1,major:1,dep:1}},{$match:{$and:[{no:{$gt:1001,$lt:1012}},{dep:"计算机学院"}]}},{$project:{no:1,major:1,"info.name":1}},{$group:{_id:"$major"}},{$sort:{no:-1}}));
	throw -1 ;
}
