...
try
{
   var rc =  cl.aggregate({$group:{_id:"$dep"}});
}
catch(e)
{
   println("failed to execut aggregate function when have one _id,e="+e);
   throw e ;
}
...
{
   i++;
   deparray.push(rc.current().toObj()["dep"]);
}

var dep_array = new Array("物电学院","计算机学院");
for(var k =0 ;k<deparray.length;k++){

	if(deparray[k]!=dep_array[k]){
		println("return the wrong record infomation deparray["+k+"]="+deparray[k]);
		throw -1 ;
		}
	}
...
if( i != 2 )
{
	...
	println(cl.aggregate({$group:{_id:{dep:"$dep"}}}));
	throw -1 ;
}
