...
try
{
	var rc = cl.aggregate({$group:{_id:"$major",maxscore:{$max:"$score"},minscore:{$min:"$score"},avg_score:{$avg:"$score"},sum_score:{$sum:"$score"}}});
...
while(rc.next()){
	...
	if(!((fieldarray.length == 4) && (fieldarray[0]=="maxscore"&&fieldarray[1]=="minscore"&&fieldarray[2]=="avg_score"&&fieldarray[3]=="sum_score")))
	{
...
if( i != 6 )
{
	...
	println(cl.aggregate({$group:{_id:"$major",maxscore:{$max:"$score"},minscore:{$min:"$score"},avg_score:{$avg:"$score"},sum_score:{$sum:"$score"}}}));
	throw -1 ;
}
