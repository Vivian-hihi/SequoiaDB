...
try
{
	var rc = cl.aggregate({$group:{_id:"$dep",addtoset_major:{$addtoset:"$major"},push_no:{$push:"$no"}}},{$skip:1},{$limit:1});
//	println(rc) ;
}
catch(e)
{
...
while(rc.next()){
	...
	//验证字段名是否只有addtoset_major和push_no字段
	var fieldarray = new Array();
	for(var fieldname in obj)
	{
		fieldarray.push(fieldname);
	}	
	if(!((fieldarray.length == 2) && (fieldarray[0]=="addtoset_major"&&fieldarray[1]=="push_no")))
	{
	...
	//验证输出字段addtoset的值是否正确
	var major=obj["addtoset_major"];
	if(!(major[0]=="计算机科学与技术"&&major[1]=="计算机工程"&&major[2]=="计算机软件与理论"))
	{
			println("the information of addtoset_major field is wrong");
			throw -1 ;
	}
	//验证输出字段push_no的值是否正确
	var no = obj["push_no"];
	var num=1000;
//	println(no.length) ;
	for(var Num=0;Num<no.length;Num++)
	{
		if(no[Num]<1000 || no[Num]>1005)
		{
			println("the information of push_no field is wrong");
			throw -1 ;
		}
	}
}
