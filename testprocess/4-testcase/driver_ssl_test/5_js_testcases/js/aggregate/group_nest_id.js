...
try
{
	var rc =  cl.aggregate({$group:{_id:"$info.sex"}});

}
catch(e)
{
	println("failed to execut aggregate function when _id is a nest filed,e="+e);
	throw e ;	
}
...
{
	i++;
	sexarray.push(rc.current().toObj()["info"]["sex"]);
}

var sex_array = new Array("女","男");
for(var k =0 ;k<sexarray.length;k++){

	if(sexarray[k]!=sex_array[k]){
		println("return the wrong record infomation sexarray["+k+"]="+sexarray[k]);
		throw -1 ;
		}
	}
...
if( i != 2 )
{
	...
	println(cl.aggregate({$group:{_id:"$info.sex"}}));
	throw -1 ;
}
