...
try
{
	var rc = cl.aggregate({$match:{$and:[{interest:{$exists:1}},{info:{$elemMatch:{sex:"女"}}},{"info.name":{$regex:'k.*',$options:'i'}},{dep:{$ne:"计算机学院"}}]}});
}
catch(e)
{
...
if( i != 1 )
{
	...
	println(cl.aggregate({$match:{$and:[{interest:{$exists:1}},{info:{$elemMatch:{sex:"女"}}},{"info.name":{$regex:'k.*',$options:'i'}},{dep:{$ne:"计算机学院"}}]}}));
	throw -1 ;
}
