...
try
{
	var rc = cl.aggregate({$match:{$or:[{no:{$gte:1000,$lte:1006}},{major:"光学"}]}},{$project:{no:1,"info":1}},{$sort:{no:-1}});
}
catch(e)
{
...
var no_array =  new Array(1010,1009,1006,1004,1003,1002,1001,1000);
...
if( i != 8 )
{
	...
	println(cl.aggregate({$match:{$or:[{no:{$gte:1000,$lte:1006}},{major:"光学"}]}},{$project:{no:1,"info":1}},{$sort:{no:-1}}));
	throw -1 ;
}
