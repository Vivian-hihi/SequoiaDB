...
{
   println("failed to execut aggregate function when _id is null,e="+e);
   throw e ;
}
...
for(var k =0 ;k<noarray.length;k++)
{
   if(noarray[k]!=no_num)
   {
      println("return the wrong record infomation noarray["+k+"]="+noarray[k]);
      throw -1 ;
   }
   no_num++ ;
}
...
if( i != 17 )
{
   ...
   println(cl.aggregate({$group:{_id:null}}));
   throw -1 ;
}

//the value of _id is a field name that does not exist
try
{
   var rc =  cl.aggregate({$group:{_id:"$field"}});

}
catch(e)
{
	println("failed to execut aggregate function when _id is a field name that does not exist,e="+e);
	throw e ;	
}
...
{
   println("return wrong numbers of records");
   println(cl.aggregate({$group:{_id:"$field"}}));
   throw -1 ;
}

if(noarray[0]!=1000)
{
   println("return the wrong record infomation noarray[0]="+noarray[0]);
   throw -1 ;
}
