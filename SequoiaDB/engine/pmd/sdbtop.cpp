...
public: // operation
   ...
   INT32 getActualPosition( Position &actualPosition, Position &referPosition, const string zoomMode, const string occupyMode ) ;
   ...
   INT32 SNPRINTF_TOP( CHAR* pBuffer, INT32 &printfLength, const CHAR* PSrc ) ;
   INT32 MVPRINTW_TOP( string &expression, INT32 expressionLength, string alignment, INT32 start_row, INT32 start_col ) ;
...
Event::Event()
{
   ...
   root.input.colourOfTheMax.foreGroundColor = 0 ;
   root.input.colourOfTheMax.backGroundColor = 7 ;
   root.input.colourOfTheMin.foreGroundColor = 0 ;
   root.input.colourOfTheMin.backGroundColor = 7 ;
...
INT32 Event::getActualPosition( Position &actualPosition, Position &referPosition, const string zoomMode, const string occupyMode )
{
   INT32 rc = SDB_OK;
   INT32 row, col ;
   FLOAT32 SCALE_ROW = 0.0;
...
INT32 Event::SNPRINTF_TOP( CHAR* pBuffer, INT32 &printfLength, const CHAR* pSrc )
{
   INT32 rc = SDB_OK;
   INT32 i = 0;
...
INT32 Event::MVPRINTW_TOP( string &expression, INT32 expressionLength, string alignment, INT32 start_row, INT32 start_col )
{
   INT32 rc = SDB_OK ;
   INT32 col = 0 ;
...
INT32 Event::getResultFromBSONobj( const BSONObj &bsonobj, const string& sourceField, const string& displayMode, string& result, BOOLEAN canSwitch, const string& baseField, const FiledWarningValue& waringValue, INT32& colourPairNumber )
{
   INT32 rc = SDB_OK ;
   INT32 pos_last = 0 ;
   ...
   try
   {
      ...
      else
      {
         ...
         else if( AVERAGE== displayMode )
         {
            if( element.isNumber() )
            {
               FLOAT64 average = ( element.Number() - last_element.Number() ) /
...
#define MIDDLE_RIGHT "MIDDLE_RIGHT"
#define LOWER_RIGHT "LOWER_RIGHT"*/
INT32 Event::fixedOutputLocation( INT32 start_row, INT32 start_col, INT32 &fixed_row, INT32 &fixed_col, INT32 referRowLength,INT32 referColLength, string displayType, string autoSetType )
{
   INT32 rc = SDB_OK;
   fixed_row = start_row;
   fixed_col = start_col;
...
INT32 Event::getFieldStructNameAndColour( const FieldStruct& fieldStruct, const string &displayMode, string &fieldName, Colours &fieldColour )
{
   INT32 rc = SDB_OK;
