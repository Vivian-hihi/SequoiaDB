                      ...
                      BSONObj &res )
{
   ...
   {
      PD_LOG( PDERROR, "invalid func type: d",
              ele.type() ) ;
      rc = SDB_INVALIDARG ;
   ...
   {
      std::string s ;
      if ( SDB_OK != JSVal2String( _scope->context(),
                                   val, s ) )
      {
         rc = SDB_SYS ;
      ...
      if ( JSObjIsSdbObj( _scope->context(), obj ) )
      {
         if ( JSObjIsCursor( _scope->context(), obj ) )
         {
            ...
            if ( SDB_OK != rc )
            {
                 res = BSON( FMP_ERR_MSG <<
                      "failed to convert jsobj to cursor" <<
                       FMP_RES_CODE << rc ) ;
                goto error ;
            }
         ...
         else if ( JSObjIsQuery( _scope->context(), obj ) )
         {
            ...
            if ( !JSVAL_IS_OBJECT(val) )
            {
               rc = SDB_SYS ;
               res = BSON( FMP_ERR_MSG <<
                           "find()._exec() did not return a obj" <<
                           FMP_RES_CODE << rc ) ;
               goto error ;
            }
         ...
         else
         {
            rc = SDB_INVALIDARG ;
            res = BSON( FMP_ERR_MSG <<
                       "this sdb obj can not used to be return" <<
                       FMP_RES_CODE << rc ) ;
            goto error ;
         }
      }
