/*******************************************************************************
   ...
   Source File Name = sdbcm.cpp

   Descriptive Name = sdbcm Main
...
INT32 cmMain ( INT32 argc, CHAR **argv ) ;
...
PD_TRACE_DECLARE_FUNCTION ( SDB_SDBCM_INITENV, "initEnv" )
INT32 initEnv( UINT16 &port )
{
   SDB_ASSERT( port, "Invalid input" ) ;
   INT32 rc = SDB_OK ;
   PD_TRACE_ENTRY ( SDB_SDBCM_INITENV );
   const CHAR *svcname = NULL;
   CHAR conf[OSS_MAX_PATHSIZE + 1] = { 0 } ;
   ...
   // buid diaglog path
   if ( ( ossStrlen ( conf ) + sizeof ( SDBCM_LOG_PATH ) + 1 ) >
                    OSS_MAX_PATHSIZE )
   {
   ...
   ossStrncpy ( _pdDiagLogPath, conf, ossStrlen(conf) ) ;
   ...
   PD_TRACE_EXITRC ( SDB_SDBCM_INITENV, rc );
   return rc ;
error:
   goto done ;   
}
...
PD_TRACE_DECLARE_FUNCTION ( SDB_CMMAIN, "cmMain" )
INT32 cmMain ( INT32 argc, CHAR **argv )
{
   INT32 rc = SDB_OK ;
   PD_TRACE_ENTRY ( SDB_CMMAIN );
...
TCHAR g_service_name[] = L"sdbcm"; // service name
SERVICE_STATUS g_service_status;
SERVICE_STATUS_HANDLE g_status_handle;
HANDLE g_wait_event = NULL;

PD_TRACE_DECLARE_FUNCTION ( SDB_SDBCM_RUN, "run" )
INT32 run()
{
    INT32 rc = SDB_OK ;
    PD_TRACE_ENTRY ( SDB_SDBCM_RUN );
    ...
    {
      DWORD g_error = GetLastError();
      ossPrintf ( "Failed to start service, GetLastError() = %d"OSS_NEWLINE, g_error );
      rc = SDB_SYS ;
    }
    PD_TRACE_EXITRC ( SDB_SDBCM_RUN, rc );
    return rc ;
}

PD_TRACE_DECLARE_FUNCTION ( SDB_SDBCM_SVCMAIN, "ServiceMain" )
VOID WINAPI ServiceMain(DWORD argc, LPTSTR *argv)
{
   PD_TRACE_ENTRY ( SDB_SDBCM_SVCMAIN );
    ...
    {
       ossPrintf ( "Failed to Register service"OSS_NEWLINE );
       goto done;
    }
   ...
   try
   {
      boost::thread start ( cmMain, argc, (CHAR**)argv ) ;
      start.detach () ;
   }
   catch ( boost::exception& )
   {
   ...
   if (! ReportStatusToSCMgr(SERVICE_RUNNING))
    ...
    DWORD dwWait = WaitForSingleObject(g_wait_event, INFINITE);
   ...
   PD_TRACE_EXIT ( SDB_SDBCM_SVCMAIN );
   return ;
}
 
PD_TRACE_DECLARE_FUNCTION ( SDB_SDBCM_SVCCTRLHANDLER, "ServiceCtrlHandler" )
VOID WINAPI ServiceCtrlHandler( DWORD control )
{
   PD_TRACE_ENTRY ( SDB_SDBCM_SVCCTRLHANDLER );
    ...
    PD_TRACE_EXIT ( SDB_SDBCM_SVCCTRLHANDLER );
    return;
}
 
PD_TRACE_DECLARE_FUNCTION ( SDB_SDBCM_RPTSTA2SCM, "ReportStatusToSCMgr" )
BOOL ReportStatusToSCMgr( DWORD dwCurrentState )
{
    BOOL fResult;
    PD_TRACE_ENTRY ( SDB_SDBCM_RPTSTA2SCM );
    ...
    PD_TRACE_EXIT ( SDB_SDBCM_RPTSTA2SCM );
    return fResult != 0;
}

PD_TRACE_DECLARE_FUNCTION ( SDB_SDBCM_STPSVC, "StopService" )
VOID StopService(LPTSTR lpszMsg)
{
    PD_TRACE_ENTRY ( SDB_SDBCM_STPSVC );
    TCHAR chMsg[256];
    HANDLE hEventSource;
    ...
    SetEvent(g_wait_event);
    PD_TRACE_EXIT ( SDB_SDBCM_STPSVC );
}
