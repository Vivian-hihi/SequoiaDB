...
namespace engine
{
   ...
   // PD_TRACE_DECLARE_FUNCTION ( SDB_PMDOMMGRENTRYPT, "pmdOMManagerEntryPoint" )
   INT32 pmdOMManagerEntryPoint ( pmdEDUCB *cb, void *pData )
   {
      INT32 rc = SDB_OK ;
      PD_TRACE_ENTRY ( SDB_PMDOMMGRENTRYPT );

      omManager *pOMMgr = ( omManager* )pData ;
      ...
      while ( !cb->isDisconnected() )
      {
         if ( cb->waitEvent( eventData, OSS_ONE_SEC ) )
         {
            ...
            {
               //restore handle
               pOMMgr->dispatchMsg( (NET_HANDLE)eventData._userData,
                                    (MsgHeader*)(eventData._Data) ) ;
            }
            else
            {
               pOMMgr->dispatchEvent ( &eventData ) ;
            }
