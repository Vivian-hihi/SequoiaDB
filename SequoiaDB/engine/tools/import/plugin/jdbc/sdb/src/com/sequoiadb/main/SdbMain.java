...
//	    command = "--connect jdbc:db2://192.168.30.223:50110/sequoia --username db2inst1 --password liuck_2015 --table sedb10 --sumrow 10 --spilt-by column1 --threads 1";
//		String command = command;
       ...
       CommandLine cl = null;
       Logger logger = Logger.getLogger(SdbMain.class);
     ...
     //--sumrow
      ...
      //--threads
       if(null !=cl && cl.hasOption("threads")) {
          ...
          {
        	  logger.error("could not found --sumRow option");
        	  return null;
          }
          if(map.get("threads") == null){
        	  logger.error("could not found --threads option");
        	  return null;
          }
          if(map.get("table") == null){
        	  logger.error("could not found --table option");
        	  return null;
          }
          ...
          for(int i=1;i<=threads;i++){
       	   ...
       	   {   
       		   ...
       		   sql = "select * from (select s.*,rownumber() over() as rn from (select * from "+table+") as s ) as s1 where s1.rn between "+start+" and "+sumRow;   
       	       if(map.get("dbType").toString() == "oracle")
       	       sql = "select * from (select rownum,* from "+table+" where rownum <="+sumRow+") where rownum >"+start;	   
       	   }else{
       		   if(map.get("dbType").toString() == "db2")
       	       sql = "select * from (select s.*,rownumber() over() as rn from (select * from "+table+") as s ) as s1 where s1.rn between "+start+" and "+end;
       	       if(map.get("dbType").toString() == "oracle")
       	    	sql = "select * from (select rownum,* from "+table+" where rownum <="+sumRow+") where rownum >"+start;   
       	   }
       	   start = i*avg+1;
