/*******************************************************************************
   ...
   Source File Name = pmdSeAdptTcpListener.cpp

   Descriptive Name = Tcp listener for search engine adapter.
...
namespace seadapter
{
   ...
   {
      INT32 rc = SDB_OK ;
      PD_TRACE_ENTRY ( SDB_PMDSETCPLSTNENTPNT ) ;
      ...
      while ( !cb->isDisconnected() && !pListerner->isClosed() )
      {
         ...
         *((SOCKET *) &pData) = s ;
         ...
         rc = eduMgr->startEDU ( EDU_TYPE_SE_AGENT, pData, &agentEDU ) ;
         if ( rc )
         {
      ...
      PD_TRACE_EXITRC ( SDB_PMDSETCPLSTNENTPNT, rc );
      return rc;
