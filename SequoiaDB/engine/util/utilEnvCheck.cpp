...
namespace engine
{
   #define ENV_ERROR_VAL -1
   #define ENV_CHECK_VM_VAL_TYPE INT64

   const static FLOAT32 vmMinFreeKbytesPercentage = 8 ;
   ...
   {//return TRUE means system is running on openVZ, suggest DO NOT run sequoiadb on openVZ 
         ...
         ( SDB_OK != ossAccess( "/proc/bc" ) ) )
      {
         PD_LOG( PDWARNING, "Your system is running on openVZ. "
                 "Even though it will still work, but SequoiaDB suggest not to do that. " ) ;
         isOpenVZ = TRUE ;
      }
   ...
   /*
   1 ) /proc/pid/...
   2 ) /proc/self/... "self" means this running process selves pid, support by linux
   */
      ...
      if( f.is_open() )
      {
         ...
         if( f.fail() )
         {
            PD_LOG( PDWARNING, "Failed to read from"
                    " /proc/self/numa_maps. " ) ;
            isNumaInUse = TRUE ;
         ...
         {
            PD_LOG( PDWARNING, "Can not parse numa_maps line : \"%s\". ", line.c_str() ) ;
            isNumaInUse = TRUE ;
            goto closedone ;
         }
   ...
   ENV_CHECK_VM_VAL_TYPE utilCheckVmParamVal( const CHAR *pathPtr, 
      const ENV_CHECK_VM_VAL_TYPE &idealValue )
   {
      BOOLEAN isVmParamOK = TRUE ;
      ENV_CHECK_VM_VAL_TYPE val ;
      if( SDB_OK == ossAccess( pathPtr ) )
      {
         ...
         if( val != idealValue )
         {
            PD_LOG( PDWARNING, "%s is %d, SequoiaDB suggest setting it to %d. "
                    "Edit /etc/sysctl.conf to set it. ", pathPtr, val, idealValue ) ;
            isVmParamOK = FALSE ;
            f.close();
            goto done ;
         }
      ...
      {
         PD_LOG( PDWARNING, "utilCheckVmParamVal() : %s not exists or can not read. ", 
                 pathPtr ) ;
         isVmParamOK = FALSE ;
         goto done ;
      }
   ...
   static ENV_CHECK_VM_VAL_TYPE utilGetSysMemTotalVolKB()
   {
      ENV_CHECK_VM_VAL_TYPE memTotalVolKB = 0 ;
      std::fstream f( "/proc/meminfo", std::ios_base::in ) ;
      string::size_type beginLoc = 0, endLoc = 0 ;
      const CHAR space = ' ' ;
      string volStr ;
      string line ;
      if( ! f.is_open() )
   ...
   {//return TRUE means all of the linux kernel vm parameters are best for SequoiaDB ; 
      ...
      vector<ENV_CHECK_VM_VAL_TYPE> vmIdealValVec( 8 ) ;
      ...
      vmIdealValVec[5] = ( utilGetSysMemTotalVolKB() / 100 ) * vmMinFreeKbytesPercentage ;
      ...
      {
         if( vmIdealValVec[i] != utilCheckVmParamVal( vmPathVec[i], vmIdealValVec[i] ) )
         {
            goodStat = FALSE ;
         }
      }
   ...
   static BOOLEAN isIdealValue( const string &result, const string &idealValue, const string &seperator1, const string &seperator2, 
      const string &path )
   {
      BOOLEAN ret = TRUE ;
      string opMode ;
      ...
      if( string::npos == beginLoc || string::npos == endLoc || beginLoc >= endLoc )
      {
      ...
      if( idealValue != opMode )
      {
         PD_LOG( PDWARNING, "Your system environment value in %s is %s, "
                  "but SequoiaDb suggest setting it to %s. ", path.c_str(), opMode.c_str(), idealValue.c_str() ) ;
   ...
   static BOOLEAN thpCheck1( const string &path )
   {
      BOOLEAN ret = TRUE ;
      ...
      if( !isIdealValue( line, idealValue, seperator1, seperator2, path ) )
      {
         goto error ;
      }
   ...
   BOOLEAN utilCheckThpStatus() //thp : transparent_hugepage
   {
      ...
      if( !thpCheck1( thpEnabledPath ) )
      {
         goto error ;
      }
   
      if( !thpCheck1( thpDefragPath ) )
      {
         goto error ;
      }
