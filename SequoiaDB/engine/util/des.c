...
#define UTIL_ENCRYPTION_MODE 1
#define UTIL_DECRYPTION_MODE 0
...
INT32 initial_message_permutation[] = {
...
INT32 key_shift_sizes[] = {
   -1, 1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1
} ;

INT32 sub_key_permutation[] = {
   14,   17,   11,   24,    1,    5,
    3,   28,   15,    6,   21,   10,
   23,   19,   12,    4,   26,    8,
   16,    7,   27,   20,   13,    2,
...
INT32 right_sub_message_permutation[] = {
   16,  7, 20, 21,
   29, 12, 28, 17,
    1, 15, 23, 26,
    5, 18, 31, 10,
...
INT32 final_message_permutation[] =  {
...
void des_generate_key( BYTE* key )
{
	INT32 i = 0 ;
...
void des_generate_sub_keys( BYTE* main_key, key_set* key_sets )
{
	INT32 i = 0 ;
   INT32 j = 0 ;
	INT32 shift_size = 0 ;
	BYTE shift_byte  = 0 ;
   BYTE first_shift_bits  = 0 ;
   BYTE second_shift_bits = 0 ;
   BYTE third_shift_bits  = 0 ;
   BYTE fourth_shift_bits = 0 ;
...
void des_process_message( BYTE* message_piece, BYTE* processed_piece,
                          key_set* key_sets, INT32 mode )
{
	INT32 i = 0 ;
   INT32 k = 0 ;
   INT32 key_index  = 0 ;
   ...
   BYTE pre_end_permutation[8] ;
	...
	ossMemset( initial_permutation, 0, 8 ) ;
	ossMemset( processed_piece, 0, 8 ) ;
                  ...
                  BYTE *pCiphertext )
{
   ...
   INT32 i = 0 ;
   INT32 padding   = expressLen % 8 ;
   key_set *pKeySet = NULL ;

   pKeySet = (key_set *)SDB_OSS_MALLOC( 17 * sizeof( key_set ) ) ;
   if ( NULL == pKeySet )
   {
      rc = SDB_OOM ;
      goto error ;
   ...
   des_generate_sub_keys( pDesKey, pKeySet ) ;

   for( i = 0; i < expressLen; i += 8 )
   {
      BYTE *pCurrent = pExpress + i ;
      INT32 len = expressLen - i ;
      ...
      if( len < 8 )
      {
         ...
         ossMemset( tmp, (BYTE)len, 8 ) ;
         ossMemcpy( tmp, pCurrent, len ) ;

         des_process_message( tmp, pCiphertext + i, pKeySet,
                              UTIL_ENCRYPTION_MODE ) ;
      }
      else
      {
         des_process_message( pCurrent, pCiphertext + i, pKeySet,
                              UTIL_ENCRYPTION_MODE ) ;
      }

      if( i + 8 >= expressLen && padding == 0 )
      {
         BYTE tmp[8] ;

         ossMemset( tmp, (BYTE)8, 8 ) ;
