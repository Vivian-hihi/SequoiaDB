...
namespace engine
{
   ...
   */
   ...
   {
      friend class _dpsTransScopedConf ;
   ...
   /*
      _dpsTransScopedConf define
    */
   class _dpsTransScopedConf : protected _dpsTransConfItem
   {
   public:
      _dpsTransScopedConf( dpsTransConfItem *pItem )
      : _dpsTransConfItem(),
        _mask( 0 ),
        _pItem( pItem )
      {
      ...
      void setTransIsolation( INT32 isolation )
      {
         if ( NULL != _pItem )
         {
            _pItem->_transIsolation = isolation ;
            OSS_BIT_SET( _mask, TRANS_CONF_MASK_ISOLATION ) ;
         }
      }
      ...
      void setTransTimeout( UINT32 timeout )
      {
         if ( NULL != _pItem )
         {
            _pItem->_transTimeout = timeout ;
            OSS_BIT_SET( _mask, TRANS_CONF_MASK_TIMEOUT ) ;
         }
      }
      ...
      void setTransWaitLock( BOOLEAN waitLock )
      {
         if ( NULL != _pItem )
         {
            _pItem->_transWaitLock = waitLock ;
            OSS_BIT_SET( _mask, TRANS_CONF_MASK_WAITLOCK ) ;
         }
      }
      ...
      void setUseRollbackSemgent( BOOLEAN use )
      {
         if ( NULL != _pItem )
         {
            _pItem->_useRollbackSegment = use ;
            OSS_BIT_SET( _mask, TRANS_CONF_MASK_USERBS ) ;
         }
      }
      ...
      void setTransAutoCommit( BOOLEAN autoCommit )
      {
         if ( NULL != _pItem )
         {
            _pItem->_transAutoCommit = autoCommit ;
            OSS_BIT_SET( _mask, TRANS_CONF_MASK_AUTOCOMMIT ) ;
         }
      }
      ...
      void setTransAutoRollback( BOOLEAN autoRollback )
      {
         if ( NULL != _pItem )
         {
            _pItem->_transAutoRollback = autoRollback ;
            OSS_BIT_SET( _mask, TRANS_CONF_MASK_AUTOROLLBACK ) ;
         }
      }
      ...
      void setTransRCCount( BOOLEAN rcCount )
      {
         if ( NULL != _pItem )
         {
            _pItem->_transRCCount = rcCount ;
            _mask |= TRANS_CONF_MASK_RCCOUNT ;
         }
      }

      void restore()
      {
         if ( NULL != _pItem && 0 != _mask )
         {
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_ISOLATION ) )
            {
               _pItem->_transIsolation = _transIsolation ;
            }
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_TIMEOUT ) )
            {
               _pItem->_transTimeout = _transTimeout ;
            }
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_USERBS ) )
            {
               _pItem->_useRollbackSegment = _useRollbackSegment ;
            }
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_AUTOCOMMIT ) )
            {
               _pItem->_transAutoCommit = _transAutoCommit ;
            }
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_AUTOROLLBACK ) )
            {
               _pItem->_transAutoRollback = _transAutoRollback ;
            }
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_WAITLOCK ) )
            {
               _pItem->_transWaitLock = _transWaitLock ;
            }
            if ( OSS_BIT_TEST( _mask, TRANS_CONF_MASK_RCCOUNT ) )
            {
               _pItem->_transRCCount = _transRCCount ;
            }

            _pItem->_transConfMask = _transConfMask ;
            _pItem->_transConfVer = _transConfVer ;
         }
      }

   protected:
      UINT32               _mask ;
      dpsTransConfItem *   _pItem ;
   } ;

   typedef class _dpsTransScopedConf dpsTransScopedConf ;
