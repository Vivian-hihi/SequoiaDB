/*******************************************************************************
   ...
   Source File Name = clsObjBase.hpp
...
#ifndef CLS_OBJ_BASE_HPP_
#define CLS_OBJ_BASE_HPP_
...
#define ON_EVENT_RANGE(typeBegin, typeEnd, func ) \
   { typeBegin, typeEnd, sig_event, #func, NULL, (EVENT_FUNC)(&thisClass::func) },
#define ON_EVENT(type, func) ON_EVENT_RANGE(type, type, func)

   class _clsObjBase : public SDBObject
   {
      public:
      ...
      protected:
         ...
         void  _onTimer(UINT64 timerID, UINT32 interval, UINT64 occurTime)
         {
            ...
            {
               PD_LOG( PDINFO,  "[%s]Timer(ID:%u,interval:%u) lantcy %u seconds",
                  name(), timerID, interval, (UINT32)(ts.time - occurTime));
            }
   ...
   typedef INT32 (_clsObjBase::*MSG_FUNC)( NET_HANDLE handle, MsgHeader* msg );
   typedef INT32 (_clsObjBase::*EVENT_FUNC)( pmdEDUEvent *event );
   ...
   inline  const _msgMap* _clsObjBase::getThisMsgMap()
   {
      static const _msgMapEntry msgEntries[] = {
         {0, 0, sig_end, "", NULL, NULL } };
      static const _msgMap msgMap = {&_clsObjBase::_getBaseMsgMap, &msgEntries[0]};
      return &msgMap;
   }

};

#endif //CLS_OBJ_BASE_HPP_
