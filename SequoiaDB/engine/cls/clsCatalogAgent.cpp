...
#define CLS_CA_SHARDINGTYPE_HASH 2
   ...
   */
   ...
   INT32 _clsCatalogSet::updateCatSet( const BSONObj & catSet, UINT32 groupID )
   {
      ...
      // get autoIncrement, it is optional.
      if ( catSet.hasField( CAT_AUTOINCREMENT ) )
      {
   ...
   INT32 _clsCatalogSet::_updateAutoIncMap( const BSONObj &autoIncArr )
   {
      ...
      while ( arrIt.more() )
      {
         ...
         fieldName = obj.getField( CAT_AUTOINC_FIELD ).String() ;
         seqName = obj.getField( CAT_AUTOINC_SEQ ).String() ;
         genStr = obj.getField( CAT_AUTOINC_GENERATED ).String() ;

         if ( CAT_GENERATED_ALWAYS == genStr )
         {
            genType = AUTOINC_GEN_ALWAYS ;
         }
         else if ( CAT_GENERATED_STRICT == genStr )
         {
            genType = AUTOINC_GEN_STRICT ;
         }
         else if ( CAT_GENERATED_DEFAULT == genStr )
         {
         ...
         {
            rc = SDB_INVALIDARG ;
            PD_LOG( PDERROR, "Unknown generated type[%s]", genStr.c_str() ) ;
            goto error ;
         }

         pNewItem = SDB_OSS_NEW coordAutoIncItem() ;
         PD_CHECK( pNewItem, SDB_OOM, error, PDERROR,
                   "Failed to malloc new autoIncrement item, rc: %d", rc ) ;

         rc = pNewItem->init( fieldName.c_str(), seqName.c_str(), genType ) ;
                                            ...
                                            coordAutoIncItem &to )
   {
      ...
      */
      ...
      INT32                rc = SDB_OK ;
      AUTOINC_ITEM_MAP*    pToMap ;
      AUTOINC_ITEM_MAP*    pFromMap ;
      AUTOINC_ITEM_MAP_IT  toIt ;
      AUTOINC_ITEM_MAP_IT  fromIt ;
      coordAutoIncItem*    pOldItem ;
      coordAutoIncItem*    pNewItem ;
      ...
      if ( from.hasSubField() && to.hasSubField() )
      {
         ...
         for ( fromIt = pFromMap->begin() ; fromIt != pFromMap->end() ; ++fromIt )
         {
            ...
            if ( toIt == pToMap->end() )
            {
               ...
               PD_RC_CHECK( rc, PDERROR, "Failed to init new autoIncrement item, rc: %d", rc ) ;
