...
namespace engine
{
                            ...
                            BOOLEAN isWaitAll, BOOLEAN clearReplyIfFailed )
   {
      ...
      while ( requestIdMap.size() > 0 )
      {
         ...
         PD_CHECK( !(cb->isInterrupted ()) || cb->isDisconnected(),
                                               ...
                                               CoordGroupInfoPtr &groupInfo )
   {
      pmdKRCB *pKrcb          = pmdGetKRCB() ;
      CoordCB *pCoordcb       = pKrcb->getCoordCB() ;
      return pCoordcb->getGroupInfo( groupName, groupInfo ) ;
   }
                                                ...
                                                CoordGroupInfoPtr &groupInfo )
   {
      ...
      do
      {
         ...
         while ( TRUE )
         {
            ...
            if ( cb->isInterrupted() && !( cb->isDisconnected() ) )
            {
         ...
         while ( !replyQue.empty() )
         {
            ...
            if ( SDB_OK == rc )
            {
               ...
               if ( SDB_OK == rc )
               {
                  ...
                  {
                     PD_LOG ( PDERROR,
                              "Failed to get cata-group-info"
                              "update route failed(rc=%d)",
                              rc );
                  }
               }
            }
