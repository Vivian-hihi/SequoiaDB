...
namespace engine
{
                           ...
                           BSONObj **ppErrorObj )
   {
      ...
      while ( TRUE )
      {
         ...
         {
            rc = insertToNormalCL( cataInfo, pReceiveBuffer, pInsertor, count,
                                 pRouteAgent, cb, groupObjsMap, hasSendSomeData );
         }//end of else if( !cataInfo->isMainCL() )
         else
         {
            //TODO:insert to mainCL
            rc = insertToMainCL( cataInfo, pReceiveBuffer, pInsertor, count,
                                 pRouteAgent, cb, groupSubCLMap );
         }
         if ( SDB_OK != rc )
         {
                                    ...
                                    GroupSubCLMap &groupSubCLMap )
   {
      ...
      try
      {
         ...
         PD_CHECK( SDB_OK == rc, SDB_CLS_COORD_NODE_CAT_VER_OLD, error,
                  ...
                  rc );
         rc = rtnCoordGetCataInfo( cb, subCLName.c_str(), FALSE, subClCataInfo );
         PD_CHECK( SDB_OK == rc, SDB_CLS_COORD_NODE_CAT_VER_OLD, error,
                  PDWARNING, "failed to get catalog of sub-collection(%s)",
                  subCLName.c_str() );
         rc = subClCataInfo->getGroupByRecord( insertObj, groupID );
         PD_CHECK( SDB_OK == rc, SDB_CLS_COORD_NODE_CAT_VER_OLD, error,
                  PDWARNING, "couldn't find the match catalog of sub-collection(%s)",
                  subCLName.c_str() );
...
/*   PD_TRACE_DECLARE_FUNCTION ( SDB_RTNCOINS_INSTOGROUPS, "rtnCoordInsert::insertToGroups" )
                           ...
                           REQUESTID_MAP &sendNodes )
   {
      ...
      while( iterMap != groupMsgMap.end() )
      {
         ...
         rc = rtnCoordSendRequestToNodeGroups( (MsgHeader *)pSrcMsg,
                                             groupLst, TRUE, pRouteAgent, cb,
                                             iterMap->second.dataList,
                                             sendNodes );
         PD_RC_CHECK( rc, PDERROR,
                     "failed to send the insert request to group(goupID=%u)",
                     iterMap->first );
         ++iterMap;
      }
   done:
                                       ...
                                       pmdEDUCB *cb )
   {
      ...
      while ( !replyQue.empty() )
      {
         ...
         {
            PD_LOG ( PDERROR,
                     "failed to execute on data node"
                                          ...
                                          BOOLEAN &hasSendSomeData )
   {
      ...
      PD_RC_CHECK( rc, PDERROR,
               "failed to shard the data by group(rc=%d)", rc );
      ...
      else
      {
         ...
         rc = insertToGroups( groupMsgMap,
                           (MsgOpInsert *)pReceiveBuffer,
                                       ...
                                       GroupSubCLMap &groupSubCLMap )
   {
      ...
      if ( groupSubCLMap.size() == 0 )
      {
         // it is the first time,
         // parse the data from input-message(pReceiveBuffer)
         rc = shardDataByGroup( cataInfo, count, pInsertor, cb, groupSubCLMap );
      }
      else
      {
         // it is not the first time,
      ...
      PD_RC_CHECK( rc, PDERROR,
                  "failed to shard the data(rc=%d)", rc );
      ...
      while( iterGroup != groupObjsMap.end() )
      {
         ...
         for( ; i < iterGroup->second.size(); i++ )
         {
            try
            {
               ...
               SINT32 usedLen = ossRoundUpToMultipleX( boInsertor.objsize(), 4 );
   ...
   {
      INT32 rc = SDB_OK;
      GroupSubCLMap::const_iterator iterGroup
                                 = groupSubCLMap.begin();
      while ( iterGroup != groupSubCLMap.end() )
      {
         UINT32 groupID = iterGroup->first;
         ...
         while ( iterCL != iterGroup->second.end() )
         {
            ...
            try
            {
               ...
               SINT32 clInfoUsedLen
                        = ossRoundUpToMultipleX( subClInfoLst[objPos].objsize(), 4 );
