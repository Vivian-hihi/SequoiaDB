   ...
   */
                                    ...
                                    INT32 backDataSize )
   {
      ...
      else
      {
         ...
         rc = _checkIndexValue( getSelector().getPattern(),
                                orderby, prefixNum, objValue ) ;
         if ( rc )
         {
            goto error ;
         }
   ...
   INT32 _rtnContextBase::_checkIndexValueAndSel( const BSONObj &selPattern,
                                                  const BSONObj &indexValue )
   {
      INT32 rc = SDB_OK ;
      BOOLEAN match = TRUE ;
      BOOLEAN onlyHasExcludeEle = FALSE ;

      if ( selPattern.isEmpty() || indexValue.isEmpty() )
      {
         goto done ;
      }
      ...
      try
      {
         // eg: selPattern = {'_id': {'$include': 0}}
         //     indexValue = {'a': 1, 'b': 1}
         // the indexValue is valid
         if ( 1 == selPattern.nFields() &&
              rtnCheckExcludeSelector( selPattern.firstElement() ) )
         {
            onlyHasExcludeEle = TRUE ;
         }

         BSONObjIterator itrIndexValue( indexValue ) ;
         while ( itrIndexValue.more() )
         {
            BSONElement eleIndexValue = itrIndexValue.next() ;
            const CHAR *fieldName = eleIndexValue.fieldName() ;
            BSONElement eleSel = selPattern.getField( fieldName ) ;

            if ( EOO == eleSel.type() )
            {
               if ( onlyHasExcludeEle )
               {
                  continue ;
               }
               ...
               while ( TRUE )
               {
                  ...
                  *c = '.' ;
                  if ( EOO != eleSel.type() )
                  {
                     if ( rtnCheckExcludeSelector( eleSel ) )
                     {
                        match = FALSE ;
                     }
                     break ;
            ...
            else if ( rtnCheckExcludeSelector( eleSel ) )
            {
               match = FALSE ;
               break ;
            }
         ...
         {
            rc = SDB_INVALIDARG ;
            PD_LOG_MSG( PDERROR, "The select pattern must contain all "
                        "fields of %s, rc: %d",
                        FIELD_NAME_INDEXVALUE, rc ) ;
            goto error ;
         }
      }
      catch ( std::exception &e )
      {
         rc = ossException2RC( &e ) ;
         PD_LOG( PDERROR, "An exception occurred when checking IndexValue "
                 "and sel: %s, rc: %d", e.what(), rc ) ;
         goto error ;
      }
   ...
   INT32 _rtnContextBase::_checkIndexValueAndOrderby( const BSONObj &orderby,
                                                      INT32 prefixNum,
                                                      const BSONObj &indexValue )
   {
      INT32 rc = SDB_OK ;
      ...
      try
      {
         BSONObjIterator itrOrderby( orderby ) ;
         BSONObjIterator itrIndexValue( indexValue ) ;

         while( itrOrderby.more() && count <= prefixNum )
         {
            BSONElement eleOrderby = itrOrderby.next() ;
            BSONElement eleIndexValue = itrIndexValue.next() ;

            if ( EOO == eleIndexValue.type() ||
                 0 != ossStrcmp( eleOrderby.fieldName(),
                                 eleIndexValue.fieldName() ) )
            {
               rc = SDB_INVALIDARG ;
               PD_LOG_MSG( PDERROR, "The IndexValue must contain prefix "
                           "fields of orderby, rc: %d", rc ) ;
               goto error ;
            }
   ...
   INT32 _rtnContextBase::_checkIndexValue( const BSONObj &selPattern,
                                            const BSONObj &orderby,
                                            INT32 prefixNum,
                                            const BSONObj &indexValue )
   {
      ...
      rc = _checkIndexValueAndOrderby( orderby, prefixNum, indexValue ) ;
      if ( rc )
      {
         goto error ;
      }

      rc = _checkIndexValueAndSel( selPattern, indexValue ) ;
      if ( rc )
      {
         goto error ;
      }
