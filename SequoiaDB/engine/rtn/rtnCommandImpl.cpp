...
namespace engine
{
                              ...
                              SINT64 &contextID )
   {
      ...
      if ( !orderBy.isEmpty() )
      {
         rc = rtnSort( (rtnContext**)&context, orderBy, cb, numToSkip,
                                         ...
                                         BOOLEAN   sysCall )
   {
      ...
      if ( NULL != cb )
      {
         ...
         while ( it != contextList.end() )
         {
            ...
            // if context doesn't exist or there's not data context
            if ( !ctx || RTN_CONTEXT_DATA != ctx->getType() )
            {
               continue ;
            }
            ...
            if ( ossStrncmp ( ((rtnContextData*)ctx)->getSU()->CSName(),
