...
#define DISK_CMD  "wmic VOLUME get Capacity,DriveLetter,Caption,"\
                                                              ...
                                                              BOOLEAN showDetail )
   {
      ...
      */
      ...
      for( UINT32 index = 0; index < procVec.size(); index++ )
      {
         ...
         {
            rc = SDB_SYS ;
            PD_LOG_MSG( PDERROR, "Fail to build retObj, rc: %d", rc ) ;
            goto error ;
         }
      }
   done:
   ...
   */
   ...
   INT32 _remoteSystemGetSystemConfigs::_extractSystemInfo( const CHAR *buf,
                                                         BSONObjBuilder &builder )
   {
      INT32 rc = SDB_OK ;
      vector<string> splited ;
      ...
      */
      ...
      {
         vector<string> columns ;
         boost::algorithm::split( columns, *itrSplit, boost::is_any_of(" = ") ) ;
   ...
   */
                                                ...
                                                string matchStr )
   {
      ...
      {
         rc = SDB_SYS ;
         PD_LOG( PDERROR, "Failed to split result, rc: %d" ) ;
         goto error ;
      }
      for ( vector<string>::iterator itr = splited.begin();
            itr != splited.end();  )
