...
SINT32 ossEDUExceptionFilter( LPEXCEPTION_POINTERS lpEP )
{
   ...
   CHAR *dumpDir = NULL ;
   ossGetTrapExceptionPath ( &dumpDir ) ;
   if ( !dumpDir )
      goto done ;
   {
      oss_edu_data * pEduData = _ossEduData.get() ;
...
static CHAR ossTrapExceptionPath [ OSS_MAX_PATHSIZE+1 ] ;
static BOOLEAN ossAmIInEngine = FALSE ;
