...
//      INT32 rc = SDB_OK ;
//      BSONObj splitInfo( pQuery ) ;
//      UINT64 taskID = CLS_INVALID_TASKID ;
//      SINT64 contextID = -1 ;
//      catCtxSplitCL *pSplitCtx = NULL ;
//
//      PD_TRACE_ENTRY ( SDB_CATALOGMGR_CMDSPLIT ) ;
//
//      switch ( opCode )
...
//      {
//         // Get the task ID
//         rc = rtnGetNumberLongElement( splitInfo, CAT_TASKID_NAME, (INT64 &)taskID ) ;
//         PD_RC_CHECK( rc, PDERROR,
//                      "Failed to execute splitCL [%d]: "
//                      "failed to get the field [%s] from query",
//                      opCode, CAT_TASKID_NAME ) ;
...
//                 opCode, taskID ) ;
//
//         // Find the context bound to the task
//         // If failed, might be switched to Secondary node, cancel the task
//         rc = _pCatCB->getSplitContextByTask( taskID, contextID ) ;
//         PD_RC_CHECK( rc, PDERROR,
//                      "Failed to execute splitCL [%d]: "
//                      "Failed to get context ID bound to task [%llu]",
//                      opCode, taskID ) ;
//
//         PD_LOG( PDDEBUG,
//                 "Split step [%d]: Got context ID [%lld] from task ID [%llu]",
//                 opCode, contextID, taskID ) ;
//
//         rc = catFindContext( contextID, (catContext **)&pSplitCtx, _pEduCB ) ;
//         PD_RC_CHECK( rc, PDERROR,
//                      "Failed to execute splitCL [%d]: "
//                      "failed to get context [%lld] from task [%llu]",
//                      opCode, contextID, taskID ) ;
//
//         PD_LOG( PDDEBUG,
//                 "Split step [%d]: got context [%lld] for split task [%llu]",
//                 opCode, contextID, taskID ) ;
//
//         // Process the step in context
//         rc = pSplitCtx->getMore( opCode, splitInfo, ctxBuf, _pEduCB ) ;
//         if ( SDB_DMS_EOC == rc )
//         {
//            rc = SDB_OK ;
//            catDeleteContext( pSplitCtx->contextID(), _pEduCB ) ;
//            pSplitCtx = NULL ;
//         }
//         PD_RC_CHECK( rc, PDERROR,
//                      "Failed to execute split task [%llu]: "
//                      "step [%d] failed, rc: %d",
//                      taskID, opCode, rc ) ;
//         break ;
//      }
...
//         UINT32 groupID = CAT_INVALID_GROUPID ;
//
//         rc = catSplitCancel( splitInfo, _pEduCB, groupID, taskID,
//                              _majoritySize() ) ;
//         PD_RC_CHECK( rc, PDERROR,
//                      "Failed to cancel split task, opCode: %d, rc: %d",
//                      opCode, rc ) ;
//
//         // reply construct
...
//      }
//
//      PD_LOG( PDDEBUG, "Finished split step [%d]", opCode ) ;
//
//   done :
//      PD_TRACE_EXITRC ( SDB_CATALOGMGR_CMDSPLIT, rc ) ;
//      return rc ;
