...
function updateProgress()
{
	...
	for( var i = 0; i < progressLen; ++i )
	{
		percent = parseInt( progressInfo['Progress'][i]['UninstalledCount'] * 100 / progressInfo['Progress'][i]['TotalCount'] ) ;
		sumPercent += percent ;
		...
		if( progressInfo['Progress'][i]['UninstalledCount'] == progressInfo['Progress'][i]['TotalCount'] )
		{
			++success ;
			if( isFinish[i] == false )
			{
	...
	if( progressInfo['Status'] == 'uninstall' )
	{
...
{
	if( $.cookie( 'SdbUser' ) == undefined || $.cookie( 'SdbPasswd' ) == undefined || $.cookie( 'SdbTaskID' ) == undefined || $.cookie( 'SdbComeback' ) == undefined || $.cookie( 'SdbBusinessName' ) == undefined )
	{
		gotoPage( 'index.html' ) ;
		return;
	}
