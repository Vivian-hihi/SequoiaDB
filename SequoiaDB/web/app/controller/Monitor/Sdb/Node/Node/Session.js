(function(){
   ...
   sacApp.controllerProvider.register( 'Monitor.SdbNode.Session.Ctrl', function( $scope, $compile, $timeout, $location, SdbRest, SdbFunction ){
      ...
      var getSessionList = function(){
         var sql ;
         if( nodeRole == 1 ) //coord节点
         {
            sql = 'SELECT * FROM $SNAPSHOT_SESSION where GLOBAL=false' ;
            if( $scope.SessionType == 'current' )
            {
               sql = 'SELECT * FROM $SNAPSHOT_SESSION_CUR where GLOBAL=false' ;
            }
         }
         else //其他节点
         {
         ...
         SdbRest.Exec( sql, {
            'success': function( roleInfo ){
               if( roleInfo.length > 0 )
               {
                  ...
                  if( nodeRole == 1 )
                     $scope.SessionTable['callback']['SetFilter']( 'Classify', '' ) ; //如果是coord节点，默认过滤改成全部
                  getSessionList() ;
               }
            },
            'failed': function( errorInfo ){
