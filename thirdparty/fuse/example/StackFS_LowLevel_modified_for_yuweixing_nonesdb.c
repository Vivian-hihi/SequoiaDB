...
//gcc -Wall StackFS_LowLevel.c `pkg-config fuse --cflags --libs` -o stackFS_ll_back -g
// ./stackFS_ll_back -r rootdir/ stackdir1 --tracing
FILE *logfile;
		...
		const char *name, mode_t mode, struct fuse_file_info *fi)
{
	...
	StackFS_trace("Create called on %s and parent ino : %llu flags:%u mode:%u",
					name, lo_inode(req, parent)->ino, fi->flags, mode);
					...
					struct fuse_file_info *fi)
{
	...
	StackFS_trace("Open called on name : %s and fuse inode : %llu kernel inode : %llu fd : %d flags:%u",
	lo_name(req, ino), get_higher_fuse_inode_no(req, ino), get_lower_fuse_inode_no(req, ino), fd,fi->flags);
