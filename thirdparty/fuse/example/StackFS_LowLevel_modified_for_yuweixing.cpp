...
//gcc -Wall StackFS_LowLevel.c `pkg-config fuse --cflags --libs` -o sdbback -g
// ./sdbback -r rootdir/ stackdir1 --tracing
//g++ -Wall ./CPP/common.cpp StackFS_LowLevel_modified_for_yuweixing.cpp `pkg-config fuse --cflags --libs` -o StackFS_LowLevel_modified_for_yuweixing_cpp -I ../include/  -I '/home/users/yuweixing/driver/C&CPP/include' -I ./CPP -L '/home/users/yuweixing/driver/C&CPP/lib/' -lsdbcpp -lrt -O0 -g
FILE *logfile;
...
{
    public:
        void sdbOpenDir(fuse_req_t req, fuse_ino_t ino,
...
class sdbLobFuse sdblobfuse;

extern "C"  char *lo_name(fuse_req_t req, fuse_ino_t ino);
extern "C"  struct lo_inode *lo_inode(fuse_req_t req, fuse_ino_t ino);
extern "C" void StackFS_trace(const char *format, ...);

void sdbLobFuse::sdbOpenDir(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info * fi)
{
    DIR *dp;
    struct lo_dirptr *d;
...
void doSdbOpenDir(sdbLobFuse *lf, fuse_req_t req, fuse_ino_t ino, struct fuse_file_info * fi)
{
    if(NULL != lf)
        lf->sdbOpenDir(req, ino, fi);
}
...
int print_banner(void)
{
	...
	if (time == 0)
		return -1;
	len = sprintf(banner, "Time : %lld Pid : %d Tid : %lu ",
							(long long int)time, pid, tid);
	(void) len;
...
void sdb_hash_table_destroy(struct node_table *t)
{
	free(t->array);
}
...
static void sdbLookup(fuse_req_t req, fuse_ino_t parent,
						const char *name)
{
...
static void sdbGetattr(fuse_req_t req, fuse_ino_t ino,
					struct fuse_file_info *fi)
{
	int res;
	struct stat buf;
...
static void sdbSetattr(fuse_req_t req, fuse_ino_t ino,
...
static void sdbCreate(fuse_req_t req, fuse_ino_t parent,
...
static void sdbmkdir(fuse_req_t req, fuse_ino_t parent,
...
static void sdbOpen(fuse_req_t req, fuse_ino_t ino,
					struct fuse_file_info *fi)
{
	int fd;
...
static void sdbOpenDir(fuse_req_t req, fuse_ino_t ino,
					struct fuse_file_info *fi)
{
#if 0
	DIR *dp;
	...
	fuse_reply_open(req, fi);
    #endif
    doSdbOpenDir(&sdblobfuse, req, ino, fi);
}

static void sdbRead(fuse_req_t req, fuse_ino_t ino, size_t size,
...
static void sdbReaddir(fuse_req_t req, fuse_ino_t ino, size_t size,
...
static void sdbRelease(fuse_req_t req, fuse_ino_t ino,
					struct fuse_file_info *fi)
{
	(void) ino;
...
static void sdbReleaseDir(fuse_req_t req, fuse_ino_t ino,
						struct fuse_file_info *fi)
{
	struct lo_dirptr *d;
	(void) ino;
...
static void sdbWrite(fuse_req_t req, fuse_ino_t ino, const char *buf,
...
#if	USE_SPLICE
static void sdbWriteBuf(fuse_req_t req, fuse_ino_t ino,
...
static void sdbUnlink(fuse_req_t req, fuse_ino_t parent,
						const char *name)
{
	int res;
	char *fullPath = NULL;
...
static void sdbRmdir(fuse_req_t req, fuse_ino_t parent,
						const char *name)
{
	int res;
	char *fullPath = NULL;
...
static void sdbForgetInode(fuse_req_t req, struct lo_inode *inode,
						uint64_t nlookup)
{
	int res;
...
static void sdbForget(fuse_req_t req, fuse_ino_t ino, uint64_t nlookup)
{
	...
	sdbForgetInode(req, inode, nlookup);
	//generate_end_time(req);
	//populate_time(req);
...
static void sdbForgetMulti(fuse_req_t req, size_t count,
...
static void sdbFlush(fuse_req_t req, fuse_ino_t ino,
					struct fuse_file_info *fi)
{
	int err;
...
static void sdbStatfs(fuse_req_t req, fuse_ino_t ino)
{
	int res;
	struct statvfs buf;
...
static void sdbFsync(fuse_req_t req, fuse_ino_t ino, int datasync,
			struct fuse_file_info *fi)
{
	int res;
...
#if  TESTING_XATTR
static void sdbGetxattr(fuse_req_t req, fuse_ino_t ino,
...
static struct fuse_lowlevel_ops hello_ll_oper = {
	...
	.fsync		=	sdbFsync,
#if	TESTING_XATTR
	.getxattr	=	sdbGetxattr,
#endif
	.forget		=	sdbForget,
	.forget_multi	=	sdbForgetMulti,
	.create		=	sdbCreate,
	.open		=	sdbOpen,
	.read		=	sdbRead,
	.write		=	sdbWrite,
#if	USE_SPLICE
	.write_buf	=	sdbWriteBuf,
#endif
	.release	=	sdbRelease,
	.unlink		=	sdbUnlink,
	.mkdir		=	sdbmkdir,
...
static struct fuse_lowlevel_ops hello_ll_oper=
{
	...
	sdbOpenDir,//opendir
	sdbReaddir,//readdir
	sdbReleaseDir,//releesaedir
	0,//fsyncdir
	sdbStatfs,//statfs
	0,//setxattr
#if	TESTING_XATTR
	sdbGetxattr,//getxattr
#else
  0,
#endif
	0,//listxattr
...
{
	struct stackFS_info *s_info = (stackFS_info*)data;
...
int init_env(sdbDataSource &ds)
{
    int count = 0;
    const CHAR *pHostName              = "localhost" ;
    const CHAR *pServiceName         = "18800" ;
    const CHAR *pUsr                         = "" ;
    const CHAR *pPasswd                   = "" ;
   ...
   ds.addCoord( "192.168.20.45:18800" ) ;
...
int main(int argc, char **argv)
{
    ...
    v.push_back( "192.168.20.45:18800" );
    ...
    sdb_hash_table_destroy(&lo->hash_table);
