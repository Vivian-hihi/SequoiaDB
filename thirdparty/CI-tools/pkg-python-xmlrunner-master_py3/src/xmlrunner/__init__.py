...
class _XMLTestResult(_TextTestResult):
    ...
    def _get_info_by_testcase(self):
        ...
        for tests in (self.successes, self.failures, self.errors):
            for test_info in tests:
                ...
                if not tests_by_testcase.__contains__(testcase_name):
    ...
    def _report_testsuite(suite_name, tests, xml_document):
        ...
        failures =list(filter(lambda e: e.outcome==_TestInfo.FAILURE, tests))
        testsuite.setAttribute('failures', str(len(failures)))
        
        errors = list(filter(lambda e: e.outcome==_TestInfo.ERROR, tests))
