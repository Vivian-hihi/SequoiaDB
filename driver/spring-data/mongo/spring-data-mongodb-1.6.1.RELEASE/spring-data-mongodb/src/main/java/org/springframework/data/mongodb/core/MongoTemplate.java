...
import org.springframework.data.mongodb.assist.*;
...
public class MongoTemplate implements MongoOperations, ApplicationContextAware {
	...
	protected void executeQuery(Query query, String collectionName, DocumentCallbackHandler dch, CursorPreparer preparer) {
		...
		DBObject hintObject = query.getHintObject();
	...
	private <T> DBObject toDbObject(T objectToSave, MongoWriter<T> writer) {
		...
		if (!(objectToSave instanceof String)) {
			...
			try {
				return Helper.BasicBSONObjectToBasicDBObject((BasicBSONObject) JSON.parse((String) objectToSave));
	...
	private <T> void doSaveVersioned(T objectToSave, MongoPersistentEntity<?> entity, String collectionName) {
		...
		if (version == null) {
			...
			DBObject updateElements = Update.fromDBObject(dbObject, ID_FIELD).getUpdateObject();
...
//					if (writeResult.getN() == 0 && dbObjectContainsVersionProperty(queryObj, entity)) {
	...
	protected <S, T> List<T> doFind(String collectionName, DBObject query, DBObject fields, Class<S> entityClass,
		...
		DBObject mappedQuery = queryMapper.getMappedObject(query, entity);
		DBObject sortObj = preparer.getType() != null ? getMappedSortObject(preparer.getQuery(), preparer.getType()) : preparer.getQuery().getSortObject();
		DBObject hintObj = preparer.getQuery().getHintObject();
	...
	private static class FindCallback implements CollectionCallback<DBCursor> {
		...
		private final DBObject _matcher;
		private final DBObject _selector;
		private final DBObject _order;
		private final DBObject _hint;
		...
		public FindCallback(DBObject query, DBObject fields, DBObject order, DBObject hint,
