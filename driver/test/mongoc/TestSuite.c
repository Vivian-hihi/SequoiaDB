                ...
                char **argv)
{
   ...
   suite->testcasesnum = 0;
   suite->successfulnum = 0;
   suite->failurenum = 0;
   suite->errornum =0;
...
#endif
           if ((p = strstr(filename,".xml")) &&
                (*(p + strlen(".xml")) == 0)) {
               suite->generatexmlreport = true;
            }
            if (!suite->outfile) {
...
"    -t            Test Suite executive duration"
"    -F           Test Report Path(.xml/.txt)"
"\n"
"Tests:\n",
            suite->prgname);
...
static void
TestSuite_PrintXmlHeader(TestSuite *suite, /* IN */
                         FILE *stream)     /* IN */
{
...
static void
TestSuite_PrintXmlFooter (TestSuite *suite,
                          FILE *stream) /* IN */
{
   XmlNode *iter;
                    ...
                    const char *testname) /* IN */
{
   ...
   TestSuite_PrintJsonFooter (stdout);
   if (suite->outfile && !suite->generatexmlreport) {
      TestSuite_PrintJsonFooter (suite->outfile);
   }
...
TestSuite_Run (TestSuite *suite) /* IN */
{
   ...
   if (suite->outfile) {
      if (suite->generatexmlreport){
         TestSuite_PrintXmlHeader (suite, suite->outfile);
      }else{
...
void Add_XmlNode(TestSuite *suite, const char* content)
{
   ...
   for (iter =suite->reportnodes; NULL != iter->next; iter=iter->next);
   iter->next = node;
}
...
static pthread_once_t global_mutxt_once = PTHREAD_ONCE_INIT;
static Mutex mutex;
